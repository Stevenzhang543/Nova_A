...
- Rust **1.92.0** with Cargo (`rust-toolchain.toml`). The declared minimums are 1.77 for the core crates and 1.88 for the desktop app.
...
Run these commands from the repository root with Node.js and rustup installed. `rustup show active-toolchain` installs the Rust version, WASM target, clippy and rustfmt listed in `rust-toolchain.toml`. Skip the wasm-pack install if 0.14.0 is already installed.
...
- macOS 12 or newer (experimental): install Xcode, or Xcode Command Line Tools for desktop-only development, and complete the initial setup. See [platform notes](docs/PLATFORM_26_29.md).
- Debian/Ubuntu Linux:
...
Run `pnpm build:wasm:dev` before the first `pnpm check` to generate `nova_core/pkg/` and its TypeScript declarations. `pnpm test:native-headless` builds and tests the native physics process on your machine. Release checks are described below.

See [Contributing](CONTRIBUTING.md) for the code layout and PR instructions.
