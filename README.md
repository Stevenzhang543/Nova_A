...
<!-- NOVA_V2630_OVERVIEW -->
**Current development source 26.30 / Engine 26.30.0:** [feature inventory](docs/FEATURE_INVENTORY_26_30.md), [panel inventory](docs/PANEL_INVENTORY_26_30.md), [comparison/gaps](docs/COMPETITIVE_REVIEW_26_30.md), [issue closure](docs/ISSUE_CLOSURE_26_30.md), [lesson](docs/QUALIFICATION_LESSON_26_30.en.md). Final acceptance requires frozen-source reports/checksums. Native JSONL is physics-only, not a game server. Web installation is online; no offline reload.
...
[![Release](https://img.shields.io/badge/source-26.30-63c6ff)]()
...
**Current work:** [26.17 release notes](docs/RELEASE_NOTES_26_17.md) · [language contract](docs/VERSION_26_12_LANGUAGE.md) · [API support matrix](docs/SCRIPT_SUPPORT_MATRIX_26_12.md) · [every 26.17 edit](docs/EDIT_LEDGER_26_17.md) · [26.17 tracker](docs/IMPLEMENTATION_TRACKER_26_17.md) · [26.12–26.16 history](docs/IMPLEMENTATION_TRACKER_26_12_TO_26_16.md) · [26.11–26.20 coding manual](docs/ROADMAP_26_11_TO_26_20.md). The [26.11 audit](docs/AUDIT_26_11.md), [source map](docs/SOURCE_MAP_26_11.md), [feature inventory](docs/FEATURE_INVENTORY_26_11.md) and [edit ledger](docs/EDIT_LEDGER_26_11.md) remain the inherited baseline. Each milestone has its own source snapshot and release directory.
...
- Node.js **22.22.2**, as recorded in `.node-version` and `package.json`.
- pnpm **10.30.0**, as pinned by `package.json`.
- Rust **1.92.0** with Cargo, as pinned by `rust-toolchain.toml`. The core crate manifests declare a 1.77 MSRV and the desktop manifest declares 1.88; these minimum versions do not replace the development toolchain pin.
...
Run these commands from the repository root after installing Node.js and rustup. `rustup show active-toolchain` selects and installs the repository's pinned toolchain, including its WebAssembly target, clippy and rustfmt. If `wasm-pack` 0.14.0 is already installed, omit its installation command. Rust documents the WebAssembly target in the [`wasm32-unknown-unknown` target guide](https://doc.rust-lang.org/stable/rustc/platform-support/wasm32-unknown-unknown.html).
...
- macOS: install Xcode, or Xcode Command Line Tools for desktop-only development, then launch/initialize the selected toolchain. Nova_A's current experimental target declares macOS 12 or later; see [platform prerequisites and qualification limits](docs/PLATFORM_26_29.md).
- Debian/Ubuntu Linux:
...
On a fresh clone, build WASM before `pnpm check`: `nova_core/pkg/` and its TypeScript declarations are generated, not checked in. These commands run workspace tests, warnings-as-errors linting, Vue/TypeScript checking, the manual audit, native physics process tests and the production Web build. `pnpm test:native-headless` works on the current native host and is not the Windows release qualification gate. These development checks do not create the complete release-evidence reports required for packaging.

See [Contributing](CONTRIBUTING.md) for the repository map, focused validation and the fork/branch/pull-request workflow.
