# Nova_A 26.30 — 全 Vue 面板与条件状态源码清单

覆盖 85 个 src/**/*.vue；直接解析完整模板及各 scoped/global style 块。所有条件分支都保留；空/加载/错误/有数据标签只是源码索引，不能证明分支运行过。

本清单不声称逐一点击了所有面板，不将未出现显式 loading/error 分支解释为该能力已通过。浏览器证据仅来自独立的26.30定向用户报告。原生文件选择、设备输入、权限拒绝、远端服务和各操作系统外观仍需对应真实环境。

## 证据边界

全部模板条件、宿主和CSS属于静态索引。历史P26问题不能自动代表本版通过；当前浏览器结果由26.30独立用户报告记录。非原生点击候选需逐项由UI审计分类。
...
- 扩展：Host workspace/modal supplies available area; no separate local maximize control.
- 最小宽度/滚动/裁切：下面列出文件全部相关声明；无本地声明时由宿主及 main.css/editorReadability.css 决定，不能据此推断像素值。
- 键盘：原生控件 button=78, input=82, select=39, textarea=1, summary=14；直接键盘绑定 3；非原生点击候选 0。
- 状态索引：空/选择条件 23；加载/等待 0；错误/诊断 8；有数据循环 79。未命中标签不等于不存在状态，请以完整分支表为准。

| 行 | 元素/分支 | 原始条件或集合 |
|---|---|---|
| 15 | option v-for | asset in studioAssets |
| 21 | div v-if | document |
| 23 | details v-if | selectedAsset |
| 23 | code v-if | activeTrack |
| 23 | code v-if | activeTimelineClip |
| 24 | span v-if | draftConflict |
| 25 | button v-if | draftDirty |
| 26 | template v-if | clip \|\| controller \|\| timeline |
| 26 | button v-if | previewActive |
| 26 | label v-if | previewActive && !controller |
| 27 | template v-if | previewActive && timeline |
| 27 | button v-if | previewActive |
| 27 | p v-if | runtimeRecording.limited |
| 28 | p v-if | status |
| 28 | p v-if | studio.recordMode |
| 28 | p v-if | studio.recordIssue |
| 29 | details v-if | draftInspection.issues.length |
| 29 | p v-for | issue in draftInspection.issues.slice(0,100) |
| 31 | nav v-if | document |
| 32 | button v-for | mode in visibleModes |
| 35 | div v-if | clip && (studio.view === 'dope' \|\| studio.view === 'curve') |
| 45 | option v-for | mode in tangentModes |
| 52 | button v-for | (track, index) in clip.tracks |
| 66 | main v-if | studio.view === 'dope' |
| 68 | i v-for | tick in rulerTicks |
| 72 | article v-for | (track, trackIndex) in clip.tracks |
| 73 | button v-for | (keyframe, keyIndex) in track.keyframes |
| 75 | button v-for | (event, index) in clip.events |
| 77 | div v-if | selectionBox |
| 80 | main v-else | (fallback) |
| 81 | svg v-if | activeTrack |
| 82 | line v-for | x in 11 |
| 82 | line v-for | y in 9 |
| 83 | g v-if | clip.onionSkin |
| 83 | circle v-for | sample in onionSamples |
| 83 | line v-for | sample in onionSamples |
| 85 | circle v-for | (keyframe, index) in activeTrack.keyframes |
| 87 | aside v-if | selectedKeys[0] |
| 91 | option v-for | mode in tangentModes |
| 92 | option v-for | mode in interpolationModes |
| 93 | option v-for | mode in easingModes |
| 94 | label v-if | selectedKeys[0].keyframe.tangentMode === 'Free' |
| 98 | PanelResizeHandle v-if | !propertiesStacked |
| 98 | PanelResizeHandle v-else | (fallback) |
| 99 | details v-if | editingToolsOpen |
| 100 | template v-if | activeTrack |
| 102 | option v-for | property in properties |
| 103 | option v-for | entity in entities |
| 107 | label v-for | (event, index) in clip.events |
| 108 | label v-for | (marker, index) in clip.markers |
| 109 | article v-for | (track, trackIndex) in clip.commandTracks |
| 109 | option v-for | entity in entities |
| 109 | label v-for | (command,index) in track.commands |
| 110 | label v-for | (frame, index) in clip.spriteFrames |
| 110 | option v-for | asset in imageAssets |
| 112 | article v-for | issue in selectedAnimationIssues |
| 112 | p v-if | !selectedAnimationIssues.length |
| 116 | div v-else-if | controller |
| 119 | article v-for | (layer, index) in controller.layers |
| 120 | option v-for | asset in maskAssets |
| 120 | option v-for | candidate in controller.layers.filter(candidate=>candidate.id!==layer.id) |
| 120 | button v-if | index |
| 123 | label v-for | (parameter,index) in controller.parameters |
| 123 | input v-if | parameter.type === 'Float' \|\| parameter.type === 'Integer' |
| 123 | input v-else | (fallback) |
| 126 | line v-for | transition in controller.transitions |
| 127 | button v-for | state in controller.states |
| 127 | small v-if | state.blendTree |
| 130 | PanelResizeHandle v-if | !propertiesStacked |
| 130 | PanelResizeHandle v-else | (fallback) |
| 131 | template v-if | selectedState |
| 134 | option v-for | asset in clipAssets |
| 136 | option v-for | parameter in numericParameters |
| 141 | template v-if | selectedState.blendTree |
| 141 | option v-for | parameter in numericParameters |
| 141 | label v-if | selectedState.blendTree.type==='2D' |
| 141 | option v-for | parameter in numericParameters |
| 141 | label v-for | (child,index) in selectedState.blendTree.children |
| 141 | option v-for | asset in clipAssets |
| 141 | input v-if | selectedState.blendTree.type==='1D' |
| 141 | template v-else | (fallback) |
| 144 | article v-for | item in runtimeInspection |
| 144 | span v-for | layer in item.layers |
| 144 | p v-if | !runtimeInspection.length |
| 147 | article v-for | (transition,index) in controller.transitions |
| 147 | option v-for | state in controller.states |
| 147 | option v-for | state in controller.states |
| 147 | option v-for | value in interruptions |
| 147 | input v-if | transition.hasExitTime |
| 147 | option v-for | value in transitionSyncModes |
| 147 | label v-if | transition.syncMode==='Marker' |
| 147 | label v-for | (condition,conditionIndex) in transition.conditions |
| 147 | option v-for | parameter in controller.parameters |
| 147 | option v-for | operator in conditionOperators |
| 147 | input v-if | condition.operator !== 'trigger' |
| 153 | div v-else-if | rig |
| 154 | button v-for | bone in rig.bones |
| 154 | button v-for | chain in rig.ikChains |
| 154 | button v-for | constraint in rig.constraints |
| 154 | button v-for | attachment in rig.attachments |
| 155 | g v-for | bone in rig.bones |
| 155 | g v-for | chain in rig.ikChains |
| 156 | PanelResizeHandle v-if | !propertiesStacked |
| 156 | PanelResizeHandle v-else | (fallback) |
| 156 | template v-if | selectedBone |
| 156 | option v-for | bone in parentBoneOptions |
| 156 | template v-if | selectedIk |
| 156 | option v-for | asset in rigAssets.filter(asset=>asset.uuid!==selectedGuid) |
| 156 | article v-if | retargetSummary |
| 156 | small v-if | retargetSummary.missing.length |
| 156 | details v-if | rig.attachments.length |
| 156 | label v-for | attachment in rig.attachments |
| 156 | option v-for | bone in rig.bones |
| 159 | div v-else-if | skin |
| 159 | option v-for | asset in rigAssets |
| 159 | option v-for | bone in linkedRig?.bones \|\| [] |
| 159 | p v-if | autoWeightMessage |
| 159 | polygon v-for | (triangle,index) in skinTriangles |
| 159 | circle v-for | (point,index) in weightHeat |
| 159 | article v-for | (vertex,index) in skin.vertices |
| 159 | option v-for | bone in linkedRig?.bones \|\| [] |
| 161 | div v-else-if | timeline |
| 162 | option v-for | type in timelineTypes |
| 163 | div v-for | (track,index) in timeline.tracks |
| 164 | i v-for | tick in timelineRulerTicks |
| 164 | button v-for | marker in timeline.markers |
| 164 | article v-for | (track,trackIndex) in timeline.tracks |
| 164 | button v-for | (item,index) in track.clips |
| 165 | PanelResizeHandle v-if | !propertiesStacked |
| 165 | PanelResizeHandle v-else | (fallback) |
| 165 | template v-if | activeTimelineTrack |
| 165 | label v-for | (marker,index) in timeline.markers |
| 165 | option v-for | marker in timeline.markers |
| 165 | option v-for | marker in timeline.markers |
| 165 | template v-if | activeTimelineClip |
| 165 | label v-if | activeTimelineTrack?.type==='Camera' |
| 165 | option v-for | entity in entities |
| 165 | select v-if | activeTimelineTrack?.type==='Visibility' |
| 165 | input v-else | (fallback) |
| 165 | label v-if | timelineCompatibleAssets.length \|\| activeTimelineClip.asset |
| 165 | option v-for | asset in timelineCompatibleAssets |
| 165 | label v-if | activeTimelineTrack?.type==='Subtitle' |
| 165 | label v-if | activeTimelineTrack?.type==='Subtitle' |
| 165 | p v-for | issue in timelinePresentationState.diagnostics |
| 165 | strong v-if | timelinePresentationState.longTimelineWarning |
| 165 | article v-for | item in cinematicIssues |
| 168 | div v-else-if | mask |
| 168 | label v-for | property in properties |
| 169 | div v-else | (fallback) |
...
| .binding-details (base) | min-width:0 |
| .binding-details code (base) | white-space:normal |
| .animation-studio (base) | overflow:auto |
| .animation-studio>:is(.clip-workspace,.controller-workspace,.rig-workspace,.skin-workspace,.sequencer) (base) | min-height:360px |
| .create-menu (base) | min-width:0; max-width:100%; overflow-x:auto |
| .create-menu button (base) | white-space:nowrap |
| .sequencer>.transport (base) | min-height:max-content |

- 键盘 3: keydown → onKeyDown
- 键盘 85: keydown → selectKey(selectedTrack,index,$event)
- 键盘 85: keydown → selectKey(selectedTrack,index,$event)
...
- 扩展：Host workspace/modal supplies available area; no separate local maximize control.
- 最小宽度/滚动/裁切：下面列出文件全部相关声明；无本地声明时由宿主及 main.css/editorReadability.css 决定，不能据此推断像素值。
- 键盘：原生控件 button=9, input=37, select=12；直接键盘绑定 0；非原生点击候选 0。
- 状态索引：空/选择条件 4；加载/等待 1；错误/诊断 3；有数据循环 13。未命中标签不等于不存在状态，请以完整分支表为准。
...
| 28 | p v-for | message in prerequisites |
| 28 | pre v-if | prerequisiteDetail |
| 32 | article v-for | (uuid, index) in buildSettings.sceneOrder |
| 36 | section v-else-if | activeTab === 'platform' |
| 47 | label v-if | buildSettings.platform.signingMode === 'manual' |
| 48 | label v-if | buildSettings.target === 'macos' |
| 50 | p v-if | buildSettings.target === 'android' |
| 51 | AndroidDeliveryPanel v-if | buildSettings.target === 'android' |
| 54 | section v-else-if | activeTab === 'delivery' |
| 70 | option v-if | !compatibleTemplates.some(template => template.id === buildSettings.delivery.exportTemplate) |
| 70 | option v-for | template in compatibleTemplates |
| 70 | label v-if | buildSettings.delivery.deploymentMode === 'remote-hook' |
| 71 | label v-if | buildSettings.delivery.deploymentMode === 'remote-hook' |
| 74 | div v-if | buildSettings.delivery.telemetryEnabled |
| 75 | li v-for | line in privacySummary |
| 80 | section v-else-if | activeTab === 'diagnostics' |
| 82 | article v-for | platform in PLATFORM_SUPPORT_MATRIX |
| 83 | span v-for | contract in RELEASE_CANDIDATE_FREEZE.frozenContracts |
| 84 | article v-for | stage in NOVA_RELEASE_PIPELINE |
| 85 | article v-for | entry in buildHistory |
| 85 | small v-if | entry.cacheKey |
| 85 | p v-if | !buildHistory.length |
| 86 | section v-if | releaseEngineeringState.lastComparison |
| 89 | TeamWorkflowPanel v-else | (fallback) |
| 95 | code v-if | buildProgress.outputPath |
| 96 | article v-for | issue in issues |
| 96 | p v-if | !issues.length |
| 98 | section v-if | pluginBuildContributions.length |
| 98 | span v-for | item in pluginBuildContributions |
| 99 | button v-if | buildSettings.target === 'web' |
| 100 | p v-if | buildSettings.runtimeMode === 'headless-server' |
...
| .prerequisite-card > div (base) | min-width:0 |
| .prerequisite-card p, .prerequisite-card pre (base) | white-space:pre-wrap |
...
- 静态点击候选（本组件负责）：div:4。需要按当前实际交互分类；本清单不推断是否存在键盘缺陷。
...
| 243 | dl v-if | networkState |
| 246 | article v-for | event in networkState?.events.slice(-200).reverse() ?? [] |
| 249 | article v-for | packet in networkState?.packetSummaries.slice(-300).reverse() ?? [] |
| 251 | article v-for | entry in networkState?.rollbackTimeline.slice(-200).reverse() ?? [] |
| 252 | article v-for | diff in networkState?.replicationDiffs.slice(-200).reverse() ?? [] |

| 选择器及条件 | 尺寸、滚动、裁切声明 |
|---|---|
| .network-contract29 dl (base) | grid-template-columns:repeat(auto-fit,minmax(min(240px,100%),1fr)) |
...
- 扩展：Host workspace/modal supplies available area; no separate local maximize control.
- 最小宽度/滚动/裁切：下面列出文件全部相关声明；无本地声明时由宿主及 main.css/editorReadability.css 决定，不能据此推断像素值。
- 键盘：原生控件 button=42, input=49, select=22, textarea=1, summary=9；直接键盘绑定 3；非原生点击候选 0。
...
| 81 | label v-if | bus.id!=='Master' |
| 81 | option v-for | target in audioRouteOptions(bus) |
| 83 | div v-for | effect in bus.effects |
| 83 | label v-if | effect.kind==='LowPass'\|\|effect.kind==='HighPass' |
| 83 | label v-if | effect.kind==='LowPass'\|\|effect.kind==='HighPass' |
| 83 | label v-if | effect.kind==='Compressor' |
| 83 | label v-if | effect.kind==='Compressor' |
| 83 | label v-if | effect.kind==='Delay'\|\|effect.kind==='Reverb' |
| 83 | label v-if | effect.kind==='Delay' |
| 84 | div v-for | send in bus.sends |
| 84 | option v-for | target in audioRouteOptions(bus) |
| 85 | div v-for | (point,index) in bus.automation |
| 89 | option v-for | asset in audioAssets |
| 90 | rect v-if | waveformSelection |
| 90 | line v-if | selectedAudio |
| 90 | line v-if | selectedAudio && activeLoopEnd > 0 |
| 91 | template v-if | selectedAudio |
| 91 | option v-for | bus in audioSettings.mixer.buses |
| 91 | p v-if | selectedAudio.settings.audioSettings.streaming |
| 91 | option v-for | region in selectedAudio.settings.audioSettings.loopRegions |
| 91 | article v-for | region in selectedAudio.settings.audioSettings.loopRegions |
| 92 | article v-for | (clock,key) in audioDiagnostics.voiceClocks |
| 93 | article v-for | failure in audioDiagnostics.failures.slice(0,4) |
| 94 | article v-for | rule in audioSettings.mixer.ducking |
| 94 | option v-for | bus in audioSettings.mixer.buses |
| 94 | option v-for | bus in audioSettings.mixer.buses |
| 98 | div v-else | (fallback) |
| 99 | option v-for | action in physicsState.inputMap |
| 99 | button v-for | modality in inputModalities |
| 100 | button v-for | item in focusTraversal |
| 100 | article v-for | issue in accessibilityIssues |
| 100 | p v-if | !accessibilityIssues.length |
...
| .audio-workspace .bus-list article>header (base) | grid-template-columns:minmax(60px,1fr) auto repeat(3,32px) |
| .audio-workspace .bus-list article>header>strong (base) | white-space:normal |
| .audition-mix button (base) | min-height:32px |

- 键盘 90: keydown → nudgeWaveform(-1)
- 键盘 90: keydown → nudgeWaveform(1)
- 键盘 100: keydown → selectUiUuid(issue.entityUuid)
...
- 状态索引：空/选择条件 9；加载/等待 1；错误/诊断 3；有数据循环 28。未命中标签不等于不存在状态，请以完整分支表为准。
...
| 42 | tr v-for | pass in renderGraphState.passes |
| 46 | article v-for | metric in timingMetrics.filter(item => item.value > 0) |
| 80 | label v-for | option in debugOptions |
| 84 | div v-else-if | activeTab === 'memory' |
| 98 | option v-for | capture in tools.captures |
| 99 | option v-for | capture in tools.captures |
| 102 | dl v-if | tools.comparison |
| 103 | div v-if | selectedPerformanceCapture |
| 103 | article v-for | check in selectedPerformanceCapture.budget.checks |
| 108 | article v-for | event in recentLifetimeEvents |
| 113 | div v-else-if | activeTab === 'replay' |
| 126 | option v-for | asset in replayAssets |
| 128 | article v-for | mismatch in replayState.mismatches.slice(-32) |
| 132 | div v-else-if | activeTab === 'tests' |
| 135 | button v-for | test in settings.testing.tests |
| 136 | p v-if | !settings.testing.tests.length |
| 139 | template v-if | selectedTest |
| 141 | option v-for | scene in sceneManager.scenes |
| 143 | article v-for | (assertion, index) in selectedTest.assertions |
| 146 | p v-else | (fallback) |
| 150 | p v-if | testRunnerState.running |
| 151 | article v-for | result in testRunnerState.results |
| 151 | small v-if | result.error |
| 151 | small v-for | assertion in result.assertions.filter(item => !item.passed) |
| 151 | img v-if | result.screenshot |
| 155 | div v-else-if | activeTab === 'data' |
| 159 | option v-for | asset in schemaAssets |
| 160 | option v-for | asset in tableAssets |
| 164 | template v-if | schemaDraft |
| 166 | option v-for | field in schemaDraft.fields |
| 167 | article v-for | (field, index) in schemaDraft.fields |
| 176 | article v-for | issue in dataIssues.slice(0, 100) |
| 180 | div v-else-if | activeTab === 'jobs' |
| 196 | div v-else-if | activeTab === 'scripts' |
| 201 | article v-for | entry in profilerState.scriptFunctions |
| 202 | p v-if | !profilerState.scriptFunctions.length |
| 206 | option v-for | (capture, index) in profilerState.scriptCaptures |
| 207 | option v-for | (capture, index) in profilerState.scriptCaptures |
| 209 | article v-for | entry in scriptComparison |
| 213 | div v-else-if | activeTab === 'runtime' |
| 222 | article v-for | fault in faultCenterState.recent.slice(-20).reverse() |
| 227 | div v-else | (fallback) |
| 231 | button v-if | !networkPackageEnabled |
| 232 | template v-else | (fallback) |
| 235 | button v-if | !settings.networking.permissionGranted |
| 250 | dl v-if | networkState |
| 251 | p v-if | networkState?.lastError |
| 252 | article v-for | event in networkState?.events.slice(-30) ?? [] |
| 256 | article v-for | definition in settings.networking.replicatedEntities |
...
| .presentation-diagnostics (base) | min-width:0 |
| .presentation-diagnostics dl (base) | grid-template-columns:repeat(auto-fit,minmax(min(100%,220px),1fr)) |
...
- 静态点击候选（本组件负责）：div:36。需要按当前实际交互分类；本清单不推断是否存在键盘缺陷。
...
| 行 | 元素/分支 | 原始条件或集合 |
|---|---|---|
| 1526 | VirtualControlsOverlay v-if | editorState.currentPage === 'game' |
| 1527 | div v-if | editorState.currentPage === 'game' && accessibilityNodes.length |
| 1528 | div v-for | node in accessibilityNodes |
| 1549 | div v-if | editorState.currentPage === 'game' && (runtimeAccessibilitySettings.subtitles && timelinePresentationState.subtitles.length \|\| visibleCaptions.length) |
| 1550 | p v-for | subtitle in (runtimeAccessibilitySettings.subtitles ? timelinePresentationState.subtitles : []) |
| 1551 | p v-for | caption in visibleCaptions |
| 1553 | input v-if | focusedUiInput && editorState.currentPage === 'game' |
...
- 静态点击候选（本组件负责）：div:3。需要按当前实际交互分类；本清单不推断是否存在键盘缺陷。
...
- 扩展：Host workspace/modal supplies available area; no separate local maximize control.
- 最小宽度/滚动/裁切：下面列出文件全部相关声明；无本地声明时由宿主及 main.css/editorReadability.css 决定，不能据此推断像素值。
- 键盘：原生控件 button=18, input=28, select=19, summary=2；直接键盘绑定 0；非原生点击候选 0。
...
| 54 | section v-show | showCard('performanceProfiles reduceMotion idle preview budget editor performance', 'editor') |
| 69 | PhysicsSettingsPanel v-show | showCard('physicsSettings globalGravity collisionLayers physicsMaterials conformance', 'project') |
| 71 | section v-show | showCard('scriptingSettings scriptApiVersion debugger hotReload formatting lint indexing testing remoteDebugging', 'project') |
| 91 | section v-show | showCard('audioSettings masterVolume musicVolume sfxVolume uiVolume sampleRate', 'project') |
| 100 | DeviceInputPanel v-show | showCard('deviceInput virtualControls touch gesture gamepad calibration safeArea orientation sensors haptics mobile', 'project') |
| 101 | section v-show | showCard('inputMap inputDevice bindingCode gamepad keyboard', 'project') |
| 104 | option v-for | device in inputDevices |
| 105 | span v-for | device in connectedInputDevices |
| 106 | div v-if | inputConflicts.length |
| 106 | span v-for | conflict in inputConflicts |
| 108 | article v-for | ({ action, actionIndex }) in visibleInputActions |
| 119 | details v-if | !compactInputMap |
| 127 | label v-if | action.interaction === 'hold' |
| 128 | label v-if | action.interaction === 'tap' \|\| action.interaction === 'multiTap' |
| 129 | label v-if | action.interaction === 'multiTap' |
| 135 | div v-for | (binding, bindingIndex) in action.bindings |
| 137 | option v-for | device in inputDevices |
| 140 | template v-if | action.kind === 'vector2' |
| 144 | label v-else | (fallback) |
| 145 | label v-if | binding.device.startsWith('gamepad') |
| 146 | label v-if | binding.device === 'gamepad-axis' |
| 148 | details v-if | !compactInputMap |
| 156 | section v-show | showCard('canvasSettings gridSize snapToGrid zoomSensitivity showConnections renderQuality', 'editor') |
| 173 | section v-show | showCard('packages plugins saveData engineDiagnostics projectHealth', 'all') |
| 181 | section v-show | showCard('projectSettings autosave autosaveInterval confirmDestructive restoreAutosave', 'project') |
| 189 | section v-show | showCard('defaultsSettings defaultDensity defaultRestitution defaultFriction', 'editor') |
