...
`pnpm stability:v3` runs a bounded local smoke covering repeated play/stop isolation, scene load/unload toggles, content-hash-stable asset reimports, corrupted JSON, and WebAssembly plugin trap isolation followed by a healthy Plugin API 2 start. The report is written to `release-audits/v3.0.0-stability-smoke.json`.

The 24-hour gate is separate and explicit:

```text
node scripts/stability-v3.mjs --duration-hours=24 --output=release-audits/v3.0.0-stability-24h.json
```
