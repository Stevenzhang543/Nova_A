# Nova_A 26.29 / 26.29.0 Web hosting
...
Capabilities, not a browser name, control startup. Legacy browsers receive a readable unsupported notice before application module execution. Internet Explorer is unsupported. Consult the actual 26.29 browser report for tested engine/version; unavailable Firefox/WebKit and physical iPhone/Android remain unqualified.
...
## Optional services and platform delivery

26.29 / 26.29.0 does not add a mandatory backend. A disabled networking configuration must continue to support local authoring and games. Explicitly enabled multiplayer/authentication/relay adapters require their own compatible endpoints, HTTPS/WSS and server Origin/CORS policy where applicable. Static hosting cannot supply a native UDP listener. Do not embed service secrets in public Web files. See [platform prerequisites and recipes](PLATFORM_26_29.md) and [network boundaries](NETWORK_26_29.md).

The targeted static-host check loads both root and nested editor URLs, fetches actual relative manifest/icons, checks the declared online-only policy and absence of service-worker registration, then downloads and starts an actual Web game. It does not claim browser home-screen installation or physical phone qualification. See `release-audits/v26.29-static-host-user.json` after that command has actually passed.

Deutsch: Kein verpflichtendes Backend. Nur ausdrücklich aktivierte Netzwerkdienste benötigen eigene kompatible Endpunkte, HTTPS/WSS und gegebenenfalls Origin-/CORS-Freigaben. Ein statischer Server bietet keinen nativen UDP-Listener. Die PWA bleibt online; eine Manifest-Prüfung ist keine echte Geräteinstallation.

中文：没有强制后端。仅明确启用的联网服务需要兼容端点、HTTPS/WSS 及适用的 Origin/CORS 配置；静态网站不是原生 UDP 监听器。此 PWA 仍仅在线，清单检查不等于真实设备安装验收。
