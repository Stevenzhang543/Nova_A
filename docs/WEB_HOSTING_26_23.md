# Hosting Nova_A 26.23 on a website
...
The locally executed audit uses the recorded Edge/Chromium build. It loads the editor and exported game under separate nested paths, blocks hidden paths, downloads through the explicit ZIP control, verifies package contents, and starts the downloaded player. This is evidence for that environment. Firefox, Safari, real iPhone/Android use, landscape gating and installed home-screen behavior are separate roadmap checks; they are not claimed as qualified by this Windows audit. Internet Explorer is not a supported target.
