...
Open a Rhai asset in **Script → Script Studio → Contract**. Existing scripts need no changes. Choose **Add contract header** to insert a strict, deterministic header with bounded commands and logs. Add a line such as `// @requires component RigidBody2D`, `// @requires input Jump`, `// @requires asset Assets/Prefabs/Ball.nova-prefab`, or `// @requires package top.whitelists.example` when the behavior cannot work without it.

The Contract page lists API calls with their module, callback/fixed-step/worker rule, deterministic/seeded/host-dependent classification, and permissions. A deterministic contract cannot call raw host-dependent `mouse_x`, `mouse_y`, `wheel_x`, or `wheel_y`; use action input or world-pointer APIs appropriate to the gameplay contract. Budgets are per callback and may only lower Nova_A's global bridge limits.

On Play, Nova_A checks requirements against the attached object, Input Map, Asset Database, and enabled project packages. The same issue appears in **Manage → Project Health** and **Manage → Build Settings**. Fix the named dependency or contract line, save, and run again. Visual Graph and linked Rhai remain synchronized; Project Format 2/schema 29 is unchanged.

## 6.1.0 portable workspace and responsive editor

Nova_A may be moved to another drive as a complete repository. Keep `package.json`, `pnpm-lock.yaml`, and `pnpm-workspace.yaml` together, then run `pnpm install --frozen-lockfile` once after the move so pnpm recreates location-specific dependency links. Build, WASM, Tauri, export, and release paths are repository-relative; historical audit JSON may still mention the machine where that evidence was recorded.

The desktop editor opens maximized with normal window decorations. You can restore, resize, move, minimize, or use the separate true-fullscreen command. **Settings → Launch maximized** controls the next launch; when enabled it intentionally overrides a saved smaller window size.

The default Balanced profile now coalesces high-frequency pointer samples to the display frame and uses low-overhead profiling. Full profiler tracing remains available in **Profiler → Mode → Full**, and every workspace, visual animation, runtime tick, export path, and rendering feature remains available.

## 6.0.4 linked logic and lock-safe rebuilding

To connect text and visual programming, open a `.nova-graph`, select **Generated Rhai**, then choose **Create / update linked Rhai**. Save the graph to regenerate that linked script. Save the linked script in Script Studio to update the graph. Literal/API argument edits become normal node values; source without a standard visual equivalent appears in a visible **Rhai Module**, **Rhai Statement**, or linked override node, so it is never silently lost. Independent Rhai files remain independent.

For Windows output, choose **Build Settings → Windows x86-64 → Package into executable → Build**. Nova_A stages and verifies the player before publishing it. You may leave a previously exported game running: if Windows locks that filename, the new build is written beside it with its build ID in the filename.

On a slower device, choose **Settings → Performance profiles → Low-end**. This reduces redundant idle editor frames, graph recompiles, and diagnostic sampling; it does not change project data, exported-game quality, tools, features, or animations.
