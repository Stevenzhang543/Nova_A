  ...
  <section class="physics-workspace" @change="commit">
    ...
    <div v-if="tab === 'simulation'" class="physics-grid">
      <article class="physics-card">
        ...
        <label><span>{{ t('physicsTickRate') }}</span><div><input v-model.number="physics.globalSettings.tickRate" type="number" min="1" max="1000" step="1"><em>Hz</em></div></label>
        <label><span>{{ t('maxCatchUpSteps') }}</span><input v-model.number="physics.globalSettings.maxCatchUpSteps" type="number" min="1" max="240" step="1"></label>
        <label><span>{{ t('physicsInterpolation') }}</span><select v-model="physics.globalSettings.interpolation"><option value="Interpolate">{{ t('interpolate') }}</option><option value="None">{{ t('noInterpolation') }}</option></select></label>
      </article>
...
import { PHYSICS_CONFORMANCE_CASES, PHYSICS_SHAPE_SUPPORT, PHYSICS_UNITS, defaultPhysicsMaterial, normalizePhysicsMaterial, type PhysicsCombineMode, type PhysicsMaterialAsset2D } from '../runtime/physicsProduction'
...
function commit() { normalizeGlobalSettings(); pushHistory('Edit physics settings') }
