<template>
  ...
  <section v-if="emitter" class="world-component"><header><strong>{{ t('particleEmitter2D') }}</strong><span>{{ emitter.simulationBackend }}</span></header><label><span>{{ t('particleSystemAsset') }}</span><input v-model="emitter.particleSystemAsset" placeholder="asset://…"></label><label><span>{{ t('simulationBackend') }}</span><select v-model="emitter.simulationBackend"><option>Auto</option><option>CPU</option><option>GPU</option></select></label><label><span>{{ t('emissionRate') }}</span><input v-model.number="emitter.emissionRate" type="number" min="0" max="100000"></label><label><span>{{ t('burst') }}</span><input v-model.number="emitter.burst" type="number" min="0" max="100000"></label><label><span>{{ t('lifetime') }}</span><input v-model.number="emitter.lifetime" type="number" min=".0001" max="86400" step=".1"></label><label><span>{{ t('maxParticles') }}</span><input v-model.number="emitter.maxParticles" type="number" min="0" max="100000"></label><label><span>{{ t('collisionMode') }}</span><select v-model="emitter.collisionMode"><option>None</option><option>Bounce</option><option>Stop</option></select></label><label><span>{{ t('eventSignal') }}</span><input v-model="emitter.eventSignal" maxlength="128"></label><label><span>{{ t('trailLength') }}</span><span class="inline"><input v-model="emitter.trailEnabled" type="checkbox"><input v-model.number="emitter.trailLength" type="number" min="2" max="32"></span></label><label><span>{{ t('trailWidth') }}</span><input v-model.number="emitter.trailWidth" type="number" min=".001" step=".01"></label><small>{{ t('particleGraphHint') }}</small></section>
</template>
