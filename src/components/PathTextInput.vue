...
const copy=computed(/** 依据语言及点或切线模式选择语法和数量说明。 */ ()=>({en:{syntax:'Every entry must contain finite coordinates. Use x,y for points or x,y:x,y for tangents. Escape restores the value.',count:props.kind==='points'?'Enter 2–10,000 points.':'Use at most one tangent pair per point; blank uses automatic tangents.'},de:{syntax:'Jeder Eintrag muss endliche Koordinaten enthalten: x,y für Punkte oder x,y:x,y für Tangenten. Escape stellt den Wert wieder her.',count:props.kind==='points'?'2–10.000 Punkte eingeben.':'Höchstens ein Tangentenpaar je Punkt; leer verwendet automatische Tangenten.'},zh:{syntax:'每项必须包含有限坐标：点使用 x,y，切线使用 x,y:x,y。按 Escape 恢复原值。',count:props.kind==='points'?'请输入 2–10,000 个点。':'每个点最多一对切线；留空使用自动切线。'}})[preferencesState.locale])
/** 从模型恢复路径文本并清除脏状态与表单错误。 */ function reset(){draft.value=props.modelValue;dirty=false;error.value='';input.value?.setCustomValidity('')}
/** 保存用户输入为草稿并清除旧校验错误。 */ function edit(event:Event){draft.value=(event.target as HTMLTextAreaElement).value;dirty=true;error.value='';input.value?.setCustomValidity('')}
/** 解析已修改路径，按问题类型更新本地化错误及表单有效性。 */ function validate(){if(!dirty)return true;const result=parsePathText(draft.value,props.kind,props.maximum);error.value=result.issue?copy.value[result.issue]:'';input.value?.setCustomValidity(error.value);return !error.value}
/** 校验解析成功后才提交路径；失败阻止事件传播并保留输入。 */ function commit(event?:Event){if(!dirty)return true;if(!validate()){event?.stopPropagation();return false}const result=parsePathText(draft.value,props.kind,props.maximum);if(!result.value)return false;dirty=false;emit('commit',result.value,event);return true}
onUnmounted(registerEditorDraft({validate,commit:/** 在外部保存边界提交当前路径草稿。 */ ()=>commit(),cancel:reset}))
