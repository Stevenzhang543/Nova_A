<template>
  ...
  <section v-if="slider && componentVisible('Slider', t('uiSlider'))" class="runtime-component"><header><strong>{{ t('uiSlider') }}</strong><button @click="remove('Slider')">×</button></header><ValueRange :component="slider" /><label><span>{{ t('wholeNumbers') }}</span><input v-model="slider.wholeNumbers" type="checkbox"></label><label><span>{{ t('interactable') }}</span><input v-model="slider.interactable" type="checkbox"></label></section>
  <section v-if="progress && componentVisible('ProgressBar', t('uiProgressBar'))" class="runtime-component"><header><strong>{{ t('uiProgressBar') }}</strong><button @click="remove('ProgressBar')">×</button></header><ValueRange :component="progress" /><label><span>{{ t('fillColor') }}</span><input type="color" :value="rgbHex(progress.fillColor)" @input="setColor(progress.fillColor, $event)"></label><label><span>{{ t('backgroundColor') }}</span><input type="color" :value="rgbHex(progress.backgroundColor)" @input="setColor(progress.backgroundColor, $event)"></label></section>
  <section v-if="checkbox && componentVisible('Checkbox', t('uiCheckbox'))" class="runtime-component"><header><strong>{{ t('uiCheckbox') }}</strong><button @click="remove('Checkbox')">×</button></header><label><span>{{ t('label') }}</span><input v-model="checkbox.label"></label><label><span>{{ t('checked') }}</span><input v-model="checkbox.checked" type="checkbox"></label><label><span>{{ t('interactable') }}</span><input v-model="checkbox.interactable" type="checkbox"></label></section>
  <section v-if="textInput && componentVisible('TextInput', t('uiTextInput'))" class="runtime-component"><header><strong>{{ t('uiTextInput') }}</strong><button @click="remove('TextInput')">×</button></header><label><span>{{ t('value') }}</span><input v-model="textInput.value"></label><label><span>{{ t('placeholder') }}</span><input v-model="textInput.placeholder"></label><label><span>{{ t('maxLength') }}</span><input v-model.number="textInput.maxLength" type="number" min="0"></label><label><span>{{ t('password') }}</span><input v-model="textInput.password" type="checkbox"></label></section>
...
<style scoped>
.runtime-component { margin-bottom: 9px; border: 1px solid var(--border-subtle); border-radius: 10px; overflow: hidden; background: var(--surface-2); }.runtime-component header { min-height: 34px; padding: 0 9px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--border-subtle); }.runtime-component header strong { color: var(--text-primary); font-size: 10.5px; }.runtime-component header button { width: 25px; height: 25px; border: 0; border-radius: 6px; color: var(--danger); background: transparent; }.runtime-component header button:hover { background: var(--danger-soft); }.runtime-component label, .range-values label { min-height: 34px; padding: 5px 9px; display: flex; align-items: center; justify-content: space-between; gap: 8px; border-bottom: 1px solid var(--border-subtle); color: var(--text-muted); font-size: 9.5px; line-height: 1.3; }.runtime-component label:last-child { border-bottom: 0; }.runtime-component label > span { min-width: 0; }.runtime-component label > input:not([type='checkbox']):not([type='color']), .runtime-component label > select { width: 55%; min-width: 0; min-height: 27px; }.runtime-component label > input[type='color'] { width: 42px; height: 25px; }.runtime-component label > div { width: 55%; display: flex; gap: 4px; }.runtime-component label > div input { min-width: 0; width: 50%; min-height: 27px; }.runtime-component label > .quad { display: grid; grid-template-columns: 1fr 1fr; }.runtime-component .stacked { align-items: stretch; flex-direction: column; }.runtime-component textarea { width: 100%; min-height: 58px; resize: vertical; }.range-values { display: grid; }.component-import { width: calc(100% - 18px); min-height: 28px; margin: 7px 9px; border: 1px solid var(--border-subtle); border-radius: 7px; color: var(--accent); background: var(--surface-3); font-size: 9px; }.ui-palette { padding: 11px; border: 1px dashed var(--border-strong); border-radius: 10px; }.ui-palette > strong { color: var(--text-secondary); font-size: 10px; }.ui-palette > p { margin: 4px 0 0; color: var(--text-muted); font-size: 8.5px; line-height: 1.4; }.ui-palette > div { margin-top: 8px; display: grid; grid-template-columns: 1fr 1fr; gap: 6px; }.ui-palette button { min-width: 0; min-height: 31px; overflow: hidden; border: 1px solid var(--border-subtle); border-radius: 7px; color: var(--accent); background: var(--surface-3); font-size: 9px; text-overflow: ellipsis; white-space: nowrap; }
