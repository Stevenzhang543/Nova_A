<template>
  ...
  <section v-if="audioSource && componentVisible('AudioSource', t('audioSource'))" class="runtime-component">
    ...
    <label><span>{{ t('startOffset') }}</span><input v-model.number="audioSource.startOffsetSeconds" type="number" min="0" step=".01"></label>
    <label><span>{{ t('fadeInOut') }}</span><div><input v-model.number="audioSource.fadeInSeconds" type="number" min="0" step=".01"><input v-model.number="audioSource.fadeOutSeconds" type="number" min="0" step=".01"></div></label>
    <label><span>{{ t('dopplerScale') }}</span><input v-model.number="audioSource.dopplerScale" type="number" min="0" max="4" step=".05" title="Stereo Web Audio reports Doppler as a limited capability."></label>
  ...
  <section v-if="particleEmitter && componentVisible('ParticleEmitter2D', t('particleEmitter2D'))" class="runtime-component">
    ...
    <label v-if="particleEmitter.collisionMode === 'Bounce'"><span>Restitution</span><input v-model.number="particleEmitter.collisionRestitution" type="range" min="0" max="1" step=".01"></label>
    <label v-if="particleEmitter.collisionMode !== 'None'"><span>Collision layer mask</span><input v-model.number="particleEmitter.collisionLayerMask" type="number" min="0" max="4294967295"></label>
  ...
  <section v-if="light && componentVisible('Light2D', t('light2D'))" class="runtime-component">
    ...
    <label><span>{{ t('shadowSoftness') }}</span><input v-model.number="light.shadowSoftness" type="range" min="0" max="1" step="0.05" :disabled="light.lightType === 'Directional'" :aria-describedby="light.lightType === 'Directional' ? `directional-shadow-${entity.uuid}` : undefined"></label>
  ...
  <section v-if="rectTransform && componentVisible('RectTransform', t('rectTransform'))" class="runtime-component">
    ...
    <label><span>{{ t('zOrder') }}</span><input v-model.number="rectTransform.zOrder" type="number" step="1"></label>
