  ...
  <main class="project-manager">
    ...
    <section class="manager-shell">
      ...
      <section class="creation-card">
        ...
        <details class="template-details"><summary>{{ t('templateDetails') }}</summary><strong>{{ selectedTemplateRecord ? templateName(selectedTemplateRecord.id, selectedTemplateRecord.name) : '' }}</strong><p>{{ selectedTemplateRecord ? templateDescription(selectedTemplateRecord.id, selectedTemplateRecord.description) : '' }}</p><span>{{ selectedTemplateRecord ? templateFeatures(selectedTemplateRecord.id, selectedTemplateRecord.features).join(' · ') : '' }}</span></details>
...
const icons: Record<ProjectTemplateId, string> = { empty: '◇', 'mouse-knockout': '✣', snake: '⌇', pong: '↔', breakout: '▦', platformer: '▰', 'top-down': '◉', 'physics-sandbox': '⌁', 'collision-lab': '◎', 'rendering-lab': '◈', 'ui-showcase': '▣', 'networked-optional': '⇄' }
...
.template-grid { margin-top: 8px; display: grid; grid-template-columns: 1fr 1fr; gap: 9px; }.template-grid button { min-height: 150px; padding: 15px; display: flex; flex-direction: column; align-items: flex-start; gap: 7px; border: 1px solid var(--border-subtle); border-radius: 13px; color: var(--text-primary); background: var(--surface-2); text-align: left; }.template-grid button:hover, .template-grid button.selected { transform: translateY(-2px); border-color: var(--accent); background: var(--accent-soft); box-shadow: 0 10px 28px color-mix(in srgb, var(--accent) 12%, transparent); }.template-icon { width: 34px; height: 34px; display: grid; place-items: center; border-radius: 10px; color: var(--accent); background: var(--surface-3); font-size: 20px; }.template-grid strong { font-size: 12px; }.template-grid small { color: var(--text-muted); font-size:11px; line-height: 1.45; }.feature-list { margin-top: auto; color: var(--accent); font-size:11px; }.create-button { width: 100%; margin-top: 12px; }
...
@media (max-width: 580px) { .manager-header { padding: 0 12px; }.identity strong, .version { display: none; }.manager-shell { width: min(100% - 20px, 560px); }.creation-card header { align-items: stretch; flex-direction: column; }.creation-card label { width: 100%; }.template-categories button{grid-template-columns:1fr;place-items:center;text-align:center}.template-categories button strong{white-space:normal}.template-categories button small{display:none}.template-grid { grid-template-columns: 1fr; }.template-grid button { min-height: 126px; } }
