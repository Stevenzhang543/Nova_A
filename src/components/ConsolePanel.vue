...
.log-entry { width: 100%; min-height: 31px; padding: 4px 9px; display: grid; grid-template-columns: 76px 62px 78px minmax(160px, 1fr) minmax(0, auto); gap: 7px; align-items: center; border: 0; border-bottom: 1px solid var(--border-subtle); border-radius: 0; color: var(--text-secondary); background: transparent; text-align: left; font-size: 11px; }
.log-entry:hover { background: var(--surface-hover); }.log-entry time, .log-entry code { overflow: hidden; color: var(--text-muted); text-overflow: ellipsis; white-space: nowrap; }.log-entry b { color: var(--text-muted); }.log-entry strong { color: var(--accent); }.log-entry.warning b, .log-entry.warning strong { color: var(--warning); }.log-entry.error b, .log-entry.error strong, .log-entry.fatal b, .log-entry.fatal strong { color: var(--danger); }.log-entry.fatal { background: var(--danger-soft); }
