<template>
  <section class="project-health">
    ...
    <div class="health-browser">
      ...
      <article v-if="selectedHealth" class="health-detail"><span>{{ selectedHealth.label }}</span><strong>{{ selectedHealth.value }}</strong><p>{{ selectedHealth.detail }}</p></article>
    </div>
...
import { t } from '../i18n'
import { assetState as assets, queueTextureAtlasRebuild } from '../assets/AssetDatabase'
...
import { getSceneJSON, physicsState, pushHistory, replaceAssetReferences, sceneManager } from '../store/physics'
...
import { openEditorTool, openManageSection } from '../editor/workspaces'
...
const healthRows = computed(() => [
  ...
  { id: 'renderer', label: t('rendererBackend'), value: `${rendererCapability.value.backend} · ${rendererCapability.value.tier}`, detail: `${rendererCapability.value.maximumTextureSize || 'n/a'} px · ${rendererCapability.value.target}`, status: rendererIssues.value.length ? 'warning' : 'ok' },
...
const issues = computed(() => [
  ...
  ...(validation.value?.issues.filter(item => item.severity === 'error').map(item => `${item.path}: ${item.message}`) ?? []), ...packageProblems.value, ...buildIssues.value.map(item => item.message), ...rendererIssues.value
].slice(0, 16))
...
.repair-mode{margin:8px 0 2px;display:flex;align-items:center;gap:7px;color:var(--text-muted)}.repair-mode input{width:16px;height:16px}
</style>
