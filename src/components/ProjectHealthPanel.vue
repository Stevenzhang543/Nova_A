...
const scriptMetrics = computed(() => {
  ...
  const contracts = scripts.map(asset => parseScriptContract(readTextAsset(asset.uuid) ?? ''))
  const graphs = assets.records.filter(asset => asset.assetType === 'visualScript').map(asset => { try { return compileGraphSource(readTextAsset(asset.uuid) ?? '') } catch (error) { return { valid: false, diagnostics: [{ severity: 'error', message: error instanceof Error ? error.message : String(error) }] } } })
  return { scripts: scripts.length, graphs: graphs.length, apiV1: scripts.filter(asset => asset.script?.apiVersion === 1).length, deprecated: analyses.reduce((sum, analysis) => sum + analysis.diagnostics.filter(item => item.code === 'NOVA-COMPAT-001').length, 0), contractErrors: contracts.reduce((sum, contract) => sum + contract.diagnostics.filter(item => item.severity === 'error').length, 0), errors: analyses.reduce((sum, analysis) => sum + analysis.diagnostics.filter(item => item.severity === 'error').length, 0) + contracts.reduce((sum, contract) => sum + contract.diagnostics.filter(item => item.severity === 'error').length, 0) + graphs.reduce((sum, graph) => sum + graph.diagnostics.filter(item => item.severity === 'error').length, 0) }
})
