...
const scriptMetrics = computed(() => {
  ...
  return { scripts: scripts.length, graphs: graphs.length, apiV1: scripts.filter(asset => asset.script?.apiVersion === 1).length, deprecated: analyses.reduce((sum, analysis) => sum + analysis.diagnostics.filter(item => item.code === 'NOVA-COMPAT-001').length, 0), errors: analyses.reduce((sum, analysis) => sum + analysis.diagnostics.filter(item => item.severity === 'error').length, 0) + graphs.reduce((sum, graph) => sum + graph.diagnostics.filter(item => item.severity === 'error').length, 0) }
})
