...
const issues = computed(() => [...graph.value.missingReferences.slice(0, 8).map(item => `${t('missingReferences')}: ${item.reference}`), ...(validation.value?.issues.filter(item => item.severity === 'error').map(item => `${item.path}: ${item.message}`) ?? []), ...packageProblems.value, ...buildIssues.value.map(item => item.message)].slice(0, 16))
