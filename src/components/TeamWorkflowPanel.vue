    ...
    <div class="team-grid">
      <section class="changes-card">
        ...
        <div class="change-list">
          <article v-for="change in changes" :key="`${change.kind}:${change.id}`"><span :class="change.change">{{ change.change.slice(0, 1).toUpperCase() }}</span><div><strong>{{ change.path }}</strong><small>{{ change.kind }} · {{ t(`source_${change.change}`) }}</small></div></article>
          <p v-if="!changes.length">{{ t('workingTreeCleanHint') }}</p>
        </div>
        <footer><button @click="downloadNovaIgnoreFile">{{ t('generateIgnore') }}</button><button @click="openDiff">{{ t('openExternalDiff') }}</button></footer>
        ...
        <div v-if="team.incomingSource" class="conflict-summary"><span>{{ t('conflictsFound', { count: team.conflicts.length }) }}</span><button :disabled="!team.mergeTool.trim()" @click="openMerge">{{ t('openExternalMerge') }}</button></div>
      </section>
...
import { t } from '../i18n'
import { getSceneJSON } from '../store/physics'
import { projectSessionState as project } from '../projects/projectSession'
import { acquireProjectLock, downloadNovaIgnoreFile, downloadProjectLock, openExternalDiff, openExternalMerge, persistTeamWorkflowSettings, refreshSourceStatus, releaseProjectLock, setIncomingProject, teamWorkflowState as team } from '../runtime/teamWorkflow'
