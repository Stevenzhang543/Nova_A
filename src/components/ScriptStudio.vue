  ...
  <section class="script-studio" @keydown.ctrl.s.prevent="saveActive" @keydown.meta.s.prevent="saveActive">
    <header class="studio-toolbar">
      ...
      <div class="toolbar-actions">
        ...
        <button @click="toggleScriptExplorer">{{ scriptStudioState.layout.explorerVisible ? t('hideExplorer') : t('showExplorer') }}</button>
    ...
    <div :class="['studio-grid', { 'detail-bottom': scriptStudioState.layout.detailDock === 'bottom', 'explorer-hidden': !scriptStudioState.layout.explorerVisible, 'detail-hidden': !scriptStudioState.layout.detailVisible }]">
      <aside v-show="scriptStudioState.layout.explorerVisible" class="project-scripts">
      ...
      <aside v-show="scriptStudioState.layout.detailVisible" class="studio-inspector">
...
import { closeScriptAsset, openScriptAsset, scriptStudioState, toggleScriptDetailDock, toggleScriptExplorer } from '../editor/scriptStudioState'
...
async function analyzeCurrent() { const revision = ++analysisRevision; const next = await service.analyze(draft.value, { apiVersion: activeAsset.value?.script?.apiVersion ?? scriptProjectSettings.apiVersion, revision }); if (revision === analysisRevision) analysis.value = applyScriptLintPolicy(next, scriptProjectSettings.lint) }
...
function formatActive() { if (!activeAsset.value) return; const formatted = formatScript(draft.value, scriptProjectSettings.formatting); if (formatted !== draft.value) { draft.value = formatted; sourceChanged(); pushHistory('Format script', `script-format:${activeAsset.value.uuid}`) } }
