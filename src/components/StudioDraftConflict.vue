<!-- 工作室草稿冲突提示：并列预览已保存版本与草稿，把保留或放弃决定交给父组件。 -->
<template>
...
const labels=computed(/* 返回 copy[preferencesState.locale] 的当前值。 */ ()=>copy[preferencesState.locale])
/** 为冲突预览格式化较小的有效 JSON，保留无效原文；缺失内容显示提示，长文本截取前两万字符。 */ function preview(value:string|null){if(value!==null&&props.format==='json'&&value.length<=1000000){try{value=JSON.stringify(JSON.parse(value),null,2)}catch{/* Invalid source remains visible verbatim. */}}return value===null?labels.value.missing:value.length>20000?value.slice(0,20000)+'\n…\n'+labels.value.preview:value}
