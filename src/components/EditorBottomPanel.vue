<template>
  <section class="bottom-panel" :class="{ collapsed: !estate.bottomPanelOpen }" :style="panelStyle">
    ...
    <div v-if="estate.bottomPanelOpen" class="panel-content">
      <div v-if="estate.bottomPanelTab === 'assets'" class="asset-browser">
        ...
        <section class="asset-workspace">
          <header class="asset-toolbar">
            ...
            <span class="path">{{ assets.currentFolder }}</span>
            <select v-model="assets.typeFilter" :aria-label="t('assetType')">
              <option value="all">{{ t('allAssets') }}</option><option value="image">{{ t('images') }}</option>
              <option value="audio">{{ t('audioAssets') }}</option><option value="font">{{ t('fontAssets') }}</option>
              <option value="scene">{{ t('scenes') }}</option><option value="prefab">{{ t('prefabs') }}</option>
              <option value="script">{{ t('scripts') }}</option>
              <option value="material">{{ t('materials') }}</option><option value="other">{{ t('otherAssets') }}</option>
            </select>
            <input v-model="assets.search" type="search" :placeholder="t('searchAssets')">
            <input ref="assetInput" hidden type="file" multiple accept="image/png,image/jpeg,image/webp,image/gif,image/svg+xml,audio/wav,audio/ogg,audio/mpeg,font/ttf,font/otf,font/woff,font/woff2,.ttf,.otf,.woff,.woff2,.rhai,.nova-prefab,.nova-scene" @change="importFiles">
          </header>
      ...
      <div v-else-if="estate.bottomPanelTab === 'project'" class="project-summary">
        <article><span>{{ t('engineVersion') }}</span><strong>1.6.0</strong></article>
        <article><span>{{ t('formatVersion') }}</span><strong>v10</strong></article>
      ...
      <div v-else class="future-panel"><strong>{{ t(estate.bottomPanelTab === 'animation' ? 'animation' : 'buildPanel') }}</strong><p>{{ t(estate.bottomPanelTab === 'animation' ? 'animationPlanned' : 'buildPlanned') }}</p></div>
    </div>
  </section>
</template>
...
function assetIcon(type: AssetType): string { return type === 'audio' ? '♫' : type === 'font' ? 'Aa' : type === 'scene' ? '◇' : type === 'prefab' ? '⬡' : type === 'material' ? '◩' : '◆' }
...
@media (max-width: 1050px) {
  .asset-browser { grid-template-columns: 105px minmax(140px, 1fr) 195px; }
  ...
  .asset-toolbar .path { display: none; }
}
