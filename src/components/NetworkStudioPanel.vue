<template>
  <section class="network-studio">
    ...
    <main v-else-if="activeTab === 'orchestration'" class="studio-grid orchestration-grid">
      <section class="card">
      ...
      <section class="card span-two">
        ...
        <button class="primary" :disabled="networkBusy || !canLaunchInstances" @click="buildAndLaunchInstances">{{ t('buildLaunchPeers') }}</button>
        <div class="instance-grid"><article v-for="instance in launchedInstances" :key="instance.id"><strong>{{ instance.playerName }}</strong><span>{{ instance.role }} · PID {{ instance.processId }}</span><code>{{ instance.logScope || t('sharedLogs') }}</code></article></div>
        <p v-if="multiInstanceError" class="danger">{{ multiInstanceError }}</p>
      </section>
    </main>
...
import { buildGame, sanitizeGameName } from '../runtime/gameExporter'
...
interface LaunchedNetworkInstance { id: string; role: string; playerName: string; sessionName: string; logScope: string; processId: number }
...
const replayA = ref(''), replayB = ref(''), saveAsset = ref(''), interestX = ref(0), interestY = ref(0), authorityEntity = ref(''), authorityPeer = ref(''), handoffScene = ref(''), handoffPeer = ref(''), handoffSpawnTag = ref(''), multiInstanceError = ref(''), launchedInstances = ref<LaunchedNetworkInstance[]>([])
...
const canLaunchInstances = computed(() => settings.networking.enabled && settings.networking.permissionGranted && settings.networking.autoStart && buildSettings.target === 'windows' && buildSettings.runtimeMode === 'game' && buildSettings.packageIntoExecutable && '__TAURI_INTERNALS__' in window)
...
async function buildAndLaunchInstances() {
  multiInstanceError.value = ''; networkBusy.value = true
  try {
    if (!canLaunchInstances.value) throw new Error(t('multiInstancePrerequisites'))
    const plan = createNetworkPlayPlan(settings.networking.multiInstance.peerCount, settings.networking.sessionName), result = await buildGame(false), executable = `${result.outputPath}\\${sanitizeGameName(buildSettings.gameName)}.exe`
    const { invoke } = await import('@tauri-apps/api/core')
    launchedInstances.value = await invoke<LaunchedNetworkInstance[]>('launch_network_instances', { request: { executable, workingDirectory: result.outputPath, sessionName: settings.networking.sessionName, count: plan.length, separateLogs: settings.networking.multiInstance.separateLogs } })
...
<style scoped>
.network-studio{height:100%;min-height:0;display:flex;flex-direction:column;overflow:hidden;background:var(--surface-1);container-type:inline-size}.studio-header{min-height:52px;padding:7px 10px;display:grid;grid-template-columns:minmax(180px,.5fr) minmax(540px,1.65fr) auto;gap:10px;align-items:center;border-bottom:1px solid var(--border-subtle)}.studio-header>div{display:flex;flex-direction:column;min-width:0}.studio-header>div span,.card p,.peers-card small{color:var(--text-muted);font-size:11px;line-height:1.45}.studio-header nav{display:grid;grid-template-columns:repeat(6,minmax(78px,1fr));gap:5px}.studio-header button,.card button,.empty-state button{min-height:30px;padding:0 9px;border:1px solid var(--border-subtle);border-radius:8px;color:var(--text-secondary);background:var(--surface-2);font-size:11px}.studio-header button.active,.primary{color:var(--accent-contrast)!important;border-color:var(--accent)!important;background:var(--accent)!important}.studio-header>output{padding:5px 9px;border:1px solid var(--border-subtle);border-radius:999px;color:var(--text-muted);font-size:11px}.studio-header>output.connected{color:var(--success);border-color:color-mix(in srgb,var(--success) 55%,var(--border-subtle))}.studio-header>output.error,.danger,.error{color:var(--danger)!important}.warning{color:var(--warning)!important}.studio-grid{min-height:0;flex:1;padding:9px;display:grid;grid-template-columns:repeat(3,minmax(250px,1fr));gap:9px;align-items:start;overflow:auto}.wide-grid{grid-template-columns:repeat(3,minmax(270px,1fr))}.card{min-width:0;padding:10px;border:1px solid var(--border-subtle);border-radius:11px;background:var(--surface-2);overflow:hidden}.card>header{min-height:32px;display:flex;justify-content:space-between;align-items:center;gap:8px}.card>header span{color:var(--text-muted);font-size:11px}.card>label,.limits-card label{min-height:35px;display:flex;justify-content:space-between;align-items:center;gap:8px;border-bottom:1px solid var(--border-subtle);color:var(--text-muted);font-size:11px}.card>label input:not([type=checkbox]),.card>label select{width:56%;min-width:0}.button-row{margin:7px 0;display:flex;gap:6px;flex-wrap:wrap}.danger-button{color:var(--danger)!important;border-color:var(--danger)!important}.span-two{grid-column:span 2}.span-three{grid-column:1/-1}.table-scroll,.event-list,.packet-list,.timeline-list{max-height:310px;overflow:auto}.channel-row,.rpc-row,.replication-row{min-width:760px;padding:6px 0;display:grid;gap:6px;align-items:center;border-bottom:1px solid var(--border-subtle);font-size:11px}.channel-row{grid-template-columns:120px 170px 1fr 1fr 1fr 28px}.rpc-row{grid-template-columns:120px 105px 145px 90px 85px 90px 75px 28px}.replication-row{min-width:940px;grid-template-columns:minmax(120px,1fr) 90px minmax(210px,1.4fr) 88px 72px 105px 86px 28px}.channel-row label,.replication-row label{min-height:28px;display:flex;align-items:center;gap:5px;color:var(--text-muted)}.channel-row label{justify-content:space-between}.channel-row label input{width:82px}.replication-row>div{display:flex;gap:8px;flex-wrap:wrap}.peers-card article,.lobby-list article{padding:7px;display:flex;justify-content:space-between;gap:8px;border-bottom:1px solid var(--border-subtle)}.peers-card article span,.lobby-list article span{display:flex;flex-direction:column;min-width:0}.peers-card code{max-width:55%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.metrics-card dl{display:grid;grid-template-columns:1fr 1fr;gap:5px}.metrics-card dl div{padding:6px;display:flex;justify-content:space-between;gap:6px;border-radius:6px;background:var(--surface-3);font-size:11px}.metrics-card dt{color:var(--text-muted)}.metrics-card dd{margin:0}.event-list article,.packet-list article,.timeline-list article{min-height:29px;padding:4px 6px;display:grid;gap:7px;align-items:center;border-bottom:1px solid var(--border-subtle);font-size:11px}.event-list article{grid-template-columns:72px 58px minmax(150px,1fr)}.packet-list article{grid-template-columns:80px 90px minmax(100px,1fr) 70px 24px}.timeline-list article{grid-template-columns:64px minmax(130px,.8fr) minmax(170px,1.5fr)}.event-list article.warning{color:var(--warning)}.event-list article.error,.packet-list .rejected{color:var(--danger)}.coordinate-row{margin:8px 0;display:grid;grid-template-columns:1fr 1fr auto;gap:6px}.ownership-list{margin-top:8px;display:grid;gap:4px;max-height:130px;overflow:auto}.ownership-list code{padding:5px 7px;border-radius:6px;background:var(--surface-3);overflow:hidden;text-overflow:ellipsis}.instance-grid{margin-top:9px;display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:7px}.instance-grid article{padding:8px;display:flex;flex-direction:column;gap:3px;border:1px solid var(--border-subtle);border-radius:8px;background:var(--surface-3)}.instance-grid span,.instance-grid code{color:var(--text-muted);font-size:11px}.empty-state{margin:auto;padding:24px;max-width:520px;text-align:center}.empty-state p{color:var(--text-muted)}
@container(max-width:1000px){.studio-header{grid-template-columns:1fr auto}.studio-header nav{grid-column:1/-1;grid-row:2;grid-template-columns:repeat(3,1fr)}.studio-grid{grid-template-columns:1fr 1fr}.span-three{grid-column:1/-1}.span-two{grid-column:span 2}}@container(max-width:620px){.studio-header{display:flex;flex-wrap:wrap}.studio-header>div{width:calc(100% - 90px)}.studio-header nav{width:100%;grid-template-columns:repeat(2,1fr)}.studio-grid{display:flex;flex-direction:column}.card{width:100%}.span-two,.span-three{grid-column:auto}.metrics-card dl{grid-template-columns:1fr}.table-scroll{max-height:360px}.channel-row,.rpc-row,.replication-row{min-width:0;display:flex;flex-wrap:wrap}.channel-row>*:not(button),.rpc-row>*:not(button),.replication-row>*:not(button){flex:1 1 130px}.replication-row>strong,.replication-row>div{flex-basis:100%}.event-list article,.timeline-list article{grid-template-columns:62px 1fr}.event-list article span,.timeline-list article span{grid-column:1/-1}.coordinate-row{grid-template-columns:1fr 1fr}.coordinate-row button{grid-column:1/-1}}
</style>
