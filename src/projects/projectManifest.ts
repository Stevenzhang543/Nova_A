...
export function normalizeProjectManifest(value: unknown, metadata: ProjectMetadata = projectSessionState): ProjectManifest {
  ...
  const compatibility = source.engineCompatibility && typeof source.engineCompatibility === 'object' ? source.engineCompatibility : { minimum: '3.9.0', maximumExclusive: '7.0.0' }
  ...
  return {
    ...
    engineCompatibility: {
      ...
      maximumExclusive: typeof compatibility.maximumExclusive === 'string' ? (compatibility.maximumExclusive === '6.0.0' ? '7.0.0' : compatibility.maximumExclusive.slice(0, 40)) : '7.0.0'
    },
    schemaVersion: NOVA_PROJECT_SCHEMA_VERSION,
