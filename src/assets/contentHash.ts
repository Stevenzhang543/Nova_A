...
export function sha256Bytes(source: Uint8Array): string {
  ...
  bytes.set(source); bytes[byteLength] = 0x80
  const view = new DataView(bytes.buffer)
  const bitLength = byteLength * 8
  view.setUint32(paddedLength - 8, Math.floor(bitLength / 0x1_0000_0000), false)
  view.setUint32(paddedLength - 4, bitLength >>> 0, false)
  const hash = new Uint32Array(INITIAL), words = new Uint32Array(64)
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index++) {
      ...
      const temporary1 = (h + sum1 + choice + ROUND[index] + words[index]) >>> 0
...
export function assetSourceBytes(source: string): Uint8Array {
  ...
  try {
    const metadata = source.slice(0, comma), payload = source.slice(comma + 1)
    ...
    return bytes
  } catch { return new TextEncoder().encode(source) }
}
