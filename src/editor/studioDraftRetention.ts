...
export type StudioDraftKind = 'theme' | 'localization' | 'code' | 'graph' | 'events' | 'blueprint' | 'resource' | 'animation' | 'controller' | 'animationMask' | 'rig' | 'skin' | 'timeline'
...
const assetTypes:Record<StudioDraftKind,string>={theme:'uiTheme',localization:'localization',code:'script',graph:'visualScript',events:'eventSheet',blueprint:'objectBlueprint',resource:'resource',animation:'animation',controller:'controller',animationMask:'animationMask',rig:'rig',skin:'skin',timeline:'timeline'}
let nextRevision=0
...
function captureOwners(projectId:string,records:readonly StudioDraftRecord[]){const live=new Set(records),active=new Set<StudioDraftRecord>();for(const owner of owners){const candidate=owner.read();if(candidate&&candidate.projectId===projectId&&live.has(candidate.record)){retainStudioDraft(candidate);active.add(candidate.record)}}return active}
export function listPendingAuthoringDrafts(projectId:string,records:readonly StudioDraftRecord[]):PendingAuthoringDraft[]{const active=captureOwners(projectId,records);return records.flatMap(record=>{const entry=drafts.get(record);return entry?.projectId===projectId&&entry.assetUuid===record.uuid&&record.assetType===assetTypes[entry.kind]?[{assetUuid:record.uuid,kind:entry.kind,name:record.name,revision:entry.revision,sourceFingerprint:entry.sourceFingerprint,active:active.has(record)}]:[]}).sort((a,b)=>a.assetUuid.localeCompare(b.assetUuid))}
...
export function discardAuthoringDrafts(projectId:string,records:readonly StudioDraftRecord[]){const live=new Set(records);for(const owner of owners){const candidate=owner.read();if(candidate?.projectId===projectId&&live.has(candidate.record))owner.discard()}for(const record of records)clearStudioDraft(record,projectId)}
export function snapshotAuthoringDrafts(projectId:string,records:readonly StudioDraftRecord[],readSource:(record:StudioDraftRecord)=>string|null):StudioDraftSnapshot[]{captureOwners(projectId,records);return records.flatMap(record=>{const entry=drafts.get(record);return entry?.projectId===projectId?[{...entry,assetType:record.assetType,name:record.name,savedSourceAtSnapshot:readSource(record)}]:[]})}
