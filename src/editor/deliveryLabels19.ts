...
/* 返回 copy[key][preferencesState.locale === 'de' ? 1 : preferencesState.locale === 'zh' ? 2 : 0] 的当前值。 */ export function deliveryLabel19(key: keyof typeof copy): string { return copy[key][preferencesState.locale === 'de' ? 1 : preferencesState.locale === 'zh' ? 2 : 0] }
/* 根据 value === undefined 的真假，分别返回 deliveryLabel19('missing') 或 JSON.stringify(value, null, 2)。 */ export function conflictValue19(value: unknown): string { return value === undefined ? deliveryLabel19('missing') : JSON.stringify(value, null, 2) }
