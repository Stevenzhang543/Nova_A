...
function normalizeSyntaxConfig(value: unknown, depth = 0): GraphValue {
  ...
  if (Array.isArray(value)) { if (value.length > 1_024) throw new Error('Rhai structure metadata exceeds its array limit.'); return value.map(item => normalizeSyntaxConfig(item, depth + 1)) }
  if (value && typeof value === 'object') { const entries = Object.entries(value); if (entries.length > 1_024) throw new Error('Rhai structure metadata exceeds its object limit.'); return Object.fromEntries(entries.map(([key, item]) => [key, normalizeSyntaxConfig(item, depth + 1)])) }
...
function normalizeReroutes(value: unknown): GraphPoint[] {
  ...
  return boundedArray(value,MAX_EDGE_REROUTES,'Graph wire reroutes').map(entry=>{
