...
export const NOVA_STABLE_CONTRACTS: readonly StableContract[] = Object.freeze([
  ...
  Object.freeze({ id: 'package', version: String(NOVA_PACKAGE_MANIFEST_VERSION), compatibility: 'Package Manifest 1 with semantic engine ranges' }),
  Object.freeze({ id: 'cli', version: String(NOVA_BUILD_CLI_VERSION), compatibility: 'Build CLI 1; additive flags only within Nova_A 3.x' })
])
...
export function stableContractDiagnostics(): string {
  return JSON.stringify({
    ...
    contracts: NOVA_STABLE_CONTRACTS,
