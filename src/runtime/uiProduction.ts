...
/** 构造并返回记录 { code, severity, entityUuid: entity.uuid, entityName: entity.name, source: `scene/entity/${entity.uuid}/${property}`, message, preset }，字段按当前实参及捕获状态求值。 */ function issue(entity: Entity, code: UiValidationCode | string, severity: AccessibilityIssueSeverity, property: string, message: string, preset: string): UiValidationIssue {
...
/** 结构说明（自动提取）：validationTextContext；无显式参数；直接调用 getContext、document.createElement；写入 textContext；返回路径包含 textContext。 */ function validationTextContext(): CanvasRenderingContext2D | null { if (textContext === undefined) textContext = typeof document === 'undefined' ? null : document.createElement('canvas').getContext('2d'); return textContext }

/** 结构说明（自动提取）：validateUiForPreset；输入 entities、preset、direction、minimumTargetSize、layoutOptions；直接调用 validationTextContext、resolveUiLayout、Map、layout.items.map、rects.map 等；写入 context.font；包含循环处理。 */ export function validateUiForPreset(entities: Entity[], preset: UiDevicePreset, direction: TextDirection = 'ltr', minimumTargetSize = 44, layoutOptions: UiLayoutOptions = {}): UiMatrixResult {
  const context = validationTextContext(), layout = resolveUiLayout(preset.width, preset.height, entities, { locale: localizationSettings.previewLocale, direction, safeArea: preset.safeArea, localeDirection: activeTextDirection, measureText: /* 调用 measureUiText(text, scale, locale, width, context) 并返回调用结果。 */ (_entity, text, scale, locale, width) => measureUiText(text, scale, locale, width, context), ...layoutOptions }), actualItems = new Map(layout.items.map(/* 返回按声明顺序构造的数组 [item.entity.uuid, item]。 */ item => [item.entity.uuid, item]))
  const rects = layout.items.map(/** 构造并返回记录 { entityUuid: item.entity.uuid, ...item.rect, visible: item.visible }，字段按当前实参及捕获状态求值。 */ item => ({ entityUuid: item.entity.uuid, ...item.rect, visible: item.visible })), resolved = new Map(rects.map(/* 返回按声明顺序构造的数组 [rect.entityUuid, rect]。 */ rect => [rect.entityUuid, rect])), byUuid = new Map(entities.map(/* 返回按声明顺序构造的数组 [entity.uuid, entity]。 */ entity => [entity.uuid, entity])), issues: UiValidationIssue[] = layout.issues.map(/** 结构说明（自动提取）：layout.issues.map 回调；输入 finding；直接调用 byUuid.get。 */ finding => { const entity = byUuid.get(finding.entityUuid); return { ...finding, entityName: entity?.name ?? '', source: `scene/entity/${finding.entityUuid}/RectTransform`, severity: 'error', preset: preset.id } })
  for (const entity of entities) {
    ...
    const ranges = [...rect.breakpoints].sort(/* 计算表达式 a.minWidth - b.minWidth 并返回结果，沿用操作数的原有类型规则。 */ (a, b) => a.minWidth - b.minWidth)
    ranges.forEach(/** 结构说明（自动提取）：ranges.forEach 回调；输入 range、index；直接调用 Number.isFinite、issues.push、issue。 */ (range, index) => {
    ...
    if (text?.enabled) {
      ...
      const measured = layoutUiText(style.value, { width: actual.width, height: actual.height, lineHeight: style.lineHeight, wrap: text.wrap, overflow: text.overflow, measure: /* 当 context?.measureText(value).width 为 null 或 undefined 时返回 [...value].length * style.size * .62，否则保留左侧值。 */ value => context?.measureText(value).width ?? [...value].length * style.size * .62 })
      context?.restore()
    ...
    if (Object.values(rect.margins).some(/* 比较 value 与 0，返回小于的判断结果。 */ value => value < 0) || entity.getComponent<Panel>('Panel') && Object.values(entity.getComponent<Panel>('Panel')!.padding).some(/* 比较 value 与 0，返回小于的判断结果。 */ value => value < 0)) issues.push(issue(entity, 'NOVA-UI-NEGATIVE-INSET', 'warning', 'RectTransform.margins', 'Container padding and margin insets are clamped to zero when negative; use signed position or anchor offsets for overlap.', preset.id))
    ...
    if (['Button', 'Slider', 'Checkbox', 'TextInput'].some(/* 调用 entity.hasComponent(kind as 'Button') 并返回调用结果。 */ kind => entity.hasComponent(kind as 'Button')) && (actual.width < minimumTargetSize || actual.height < minimumTargetSize)) issues.push(issue(entity, 'NOVA-A11Y-TARGET', 'warning', 'RectTransform.size', `Rendered interactive target is smaller than ${minimumTargetSize} × ${minimumTargetSize} logical pixels.`, preset.id))
  ...
  for (const accessibility of auditUiAccessibility(entities, minimumTargetSize).filter(/* 先计算 finding.code !== 'NOVA-A11Y-TARGET'；仅当其为真值时求右侧 resolved.get(finding.entityUuid)?.visible，返回短路求值结果。 */ finding => finding.code !== 'NOVA-A11Y-TARGET' && resolved.get(finding.entityUuid)?.visible)) issues.push({ ...accessibility, preset: preset.id })
  for (const canvasEntity of entities.filter(/* 调用 entity.hasComponent('Canvas') 并返回调用结果。 */ entity => entity.hasComponent('Canvas'))) {
  ...
  return { preset, direction, rects, issues: issues.sort(/* 先计算 a.source.localeCompare(b.source)；仅当其为假值时求右侧 a.code.localeCompare(b.code)，返回短路求值结果。 */ (a, b) => a.source.localeCompare(b.source) || a.code.localeCompare(b.code)) }
}

/* 调用 presets.map(preset => validateUiForPreset(entities, preset, direction, minimumTargetSize)) 并返回调用结果。 */ export function validateResponsiveUi(entities: Entity[], presets: readonly UiDevicePreset[] = UI_DEVICE_PRESETS, direction: TextDirection = 'ltr', minimumTargetSize = 44): UiMatrixResult[] {
  return presets.map(/* 调用 validateUiForPreset(entities, preset, direction, minimumTargetSize) 并返回调用结果。 */ preset => validateUiForPreset(entities, preset, direction, minimumTargetSize))
}

/** 结构说明（自动提取）：uiPerformanceSnapshot；输入 entities；直接调用 Map、entities.map、entities.filter、controls.filter、controls.reduce 等。 */ export function uiPerformanceSnapshot(entities: Entity[]): { controls: number; interactive: number; maximumDepth: number; breakpoints: number; estimatedLayoutOperations: number } {
  const byUuid = new Map(entities.map(/* 返回按声明顺序构造的数组 [entity.uuid, entity]。 */ entity => [entity.uuid, entity]))
  const depth = /** 结构说明（自动提取）：depth；输入 entity；直接调用 Set、visited.has、visited.add、byUuid.get；写入 current；返回路径包含 value；包含循环处理。 */ (entity: Entity): number => { let value = 1, current = entity; const visited = new Set<string>(); while (current.parentUuid && !visited.has(current.parentUuid)) { visited.add(current.parentUuid); const parent = byUuid.get(current.parentUuid); if (!parent) break; value++; current = parent } return value }
  const controls = entities.filter(/* 调用 entity.hasComponent('RectTransform') 并返回调用结果。 */ entity => entity.hasComponent('RectTransform'))
  const interactive = controls.filter(/** 结构说明（自动提取）：controls.filter 回调；输入 entity；直接调用 entity.hasComponent；返回表达式求值结果。 */ entity => entity.hasComponent('Button') || entity.hasComponent('Slider') || entity.hasComponent('Checkbox') || entity.hasComponent('TextInput')).length
  const breakpoints = controls.reduce(/* 计算表达式 sum + (entity.getComponent<RectTransform>('RectTransform')?.breakpoints.length ?? 0) 并返回结果，沿用操作数的原有类型规则。 */ (sum, entity) => sum + (entity.getComponent<RectTransform>('RectTransform')?.breakpoints.length ?? 0), 0)
  const maximumDepth = controls.reduce(/* 调用 Math.max(maximum, depth(entity)) 并返回调用结果。 */ (maximum, entity) => Math.max(maximum, depth(entity)), 0)
