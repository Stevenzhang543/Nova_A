...
function labelFor(entity: Entity, rect: RectTransform): string { return rect.accessibilityLabel || entity.getComponent<Text>('Text')?.text || entity.getComponent<Checkbox>('Checkbox')?.label || entity.getComponent<TextInput>('TextInput')?.placeholder || entity.name }
function roleFor(entity: Entity): string { return entity.hasComponent('Button') ? 'button' : entity.hasComponent('Slider') ? 'slider' : entity.hasComponent('Checkbox') ? 'checkbox' : entity.hasComponent('TextInput') ? 'textbox' : '' }

/** Applies safe authoring defaults while preserving deliberate skip-navigation and semantic-group choices. */
export function configureUiAccessibility(entity: Entity, entities: Entity[] = []): void {
  ...
  if (interactive(entity)) {
    ...
    if (rect.focusable && !rect.skipNavigation && rect.readingOrder === 0 && entities.length) {
      rect.readingOrder = entities.reduce((maximum, candidate) => {
