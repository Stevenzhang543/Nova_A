...
function cloneInput(input: InputSnapshot): InputSnapshot {
  return {
    ...
    mousePosition: [input.mousePosition[0], input.mousePosition[1]], wheel: [input.wheel[0], input.wheel[1]]
  }
}
...
export function exportReplay(tickRate: number): ReplayDocument {
  return {
    format: 'nova-replay', version: 1, engineVersion: '3.0.0', seed: replayState.seed,
...
export function normalizeReplayDocument(value: unknown): ReplayDocument {
  ...
  return {
    format: 'nova-replay', version: 1, engineVersion: '3.0.0', seed: Number.isFinite(source.seed) ? Number(source.seed) >>> 0 : productionSettings.replay.seed,
