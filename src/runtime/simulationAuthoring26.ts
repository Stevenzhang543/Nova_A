...
const finite = /* 调用 Number.isFinite(value) 并返回调用结果。 */ (value: number): boolean => Number.isFinite(value)
const fixed = /* 计算表达式 Math.round((Number.isFinite(value) ? value : 0) * 1e9) / 1e9 并返回结果，沿用操作数的原有类型规则。 */ (value: number): number => Math.round((Number.isFinite(value) ? value : 0) * 1e9) / 1e9

/** 结构说明（自动提取）：stableFingerprint；输入 value；直接调用 JSON.stringify、source.charCodeAt、Math.imul、padStart、first.toString 等；写入 first、second；包含循环处理。 */ function stableFingerprint(value: unknown): string {
...
/** 结构说明（自动提取）：simulationStateChecksum；输入 entities、connections；直接调用 stableFingerprint、map、sort。 */ export function simulationStateChecksum(entities: readonly Entity[], connections: readonly Connection[]): string {
  return stableFingerprint({
    entities: [...entities].sort(/* 调用 a.uuid.localeCompare(b.uuid) 并返回调用结果。 */ (a, b) => a.uuid.localeCompare(b.uuid)).map(/** 结构说明（自动提取）：map 回调；输入 entity；直接调用 worldTransform、entity.getComponent、map、sort、Object.entries 等。 */ entity => {
      ...
      const board = Object.entries(blackboardSnapshot(entity.uuid)).sort(/* 调用 first.localeCompare(second) 并返回调用结果。 */ ([first], [second]) => first.localeCompare(second)).map(/* 返回按声明顺序构造的数组 [key, typeof value === 'number' ? fixed(value) : value]。 */ ([key, value]) => [key, typeof value === 'number' ? fixed(value) : value])
      return [
        ...
        agent ? [agent.pathStatus, agent.pathIndex, fixed(agent.velocity.x), fixed(agent.velocity.y), fixed(agent.targetPosition.x), fixed(agent.targetPosition.y), agent.targetEntityUuid ?? '', agent.path.map(/* 返回按声明顺序构造的数组 [fixed(point.x), fixed(point.y)]。 */ point => [fixed(point.x), fixed(point.y)])] : null,
        behavior ? [behavior.currentNode, board] : null, machine?.currentState ?? ''
      ]
    }),
    connections: [...connections].sort(/* 调用 a.uuid.localeCompare(b.uuid) 并返回调用结果。 */ (a, b) => a.uuid.localeCompare(b.uuid)).map(/** 结构说明（自动提取）：map 回调；输入 connection；直接调用 fixed、connection.ropeNodes.map；返回表达式求值结果。 */ connection => [connection.uuid, connection.enabled, connection.componentType, connection.breakState, connection.breakLink, fixed(connection.tension), fixed(connection.strain), connection.ropeNodes.map(/* 返回按声明顺序构造的数组 [fixed(node.position.x), fixed(node.position.y), fixed(node.velocity.x), fixed(node.velocity.y)]。 */ node => [fixed(node.position.x), fixed(node.position.y), fixed(node.velocity.x), fixed(node.velocity.y)])])
  })
}

/** 结构说明（自动提取）：ropeNetworks；输入 connections；直接调用 connections.filter、Map、rope.anchors.map、adjacency.get、Set 等；写入 edges；返回路径包含 networks；包含循环处理。 */ function ropeNetworks(connections: readonly Connection[]): number {
  const ropes = connections.filter(/* 先计算 connection.enabled && connection.componentType === 'Rope2D'；仅当其为真值时求右侧 connection.breakState === 'intact'，返回短路求值结果。 */ connection => connection.enabled && connection.componentType === 'Rope2D' && connection.breakState === 'intact'), adjacency = new Map<number, Set<number>>()
  for (const rope of ropes) { const ids = rope.anchors.map(/* 返回 anchor.entityId 的当前值。 */ anchor => anchor.entityId); if (ids.length !== 2) continue; const first = adjacency.get(ids[0]) ?? new Set<number>(); first.add(ids[1]); adjacency.set(ids[0], first); const second = adjacency.get(ids[1]) ?? new Set<number>(); second.add(ids[0]); adjacency.set(ids[1], second) }
...
/** 结构说明（自动提取）：textDocument；输入 reference；直接调用 resolveAsset、readTextAsset、JSON.parse。 */ function textDocument<T>(reference: string | null): T | null { const asset = resolveAsset(reference), source = readTextAsset(reference); if (!asset || !source) return null; try { return JSON.parse(source) as T } catch { return null } }

/** 结构说明（自动提取）：polygonArea；输入 points；直接调用 Math.abs；写入 twiceArea；包含循环处理。 */ function polygonArea(points: readonly { x: number; y: number }[]): number {
...
/* 先计算 Number.isInteger(value) && value >= 0；仅当其为真值时求右侧 value <= 0xffff_ffff，返回短路求值结果。 */ function validMask(value: number): boolean { return Number.isInteger(value) && value >= 0 && value <= 0xffff_ffff }
/* 调用 Boolean(value && finite(value.x) && finite(value.y)) 并返回调用结果。 */ function finiteVector(value: { x: number; y: number } | undefined): boolean { return Boolean(value && finite(value.x) && finite(value.y)) }
/* 先计算 value === Number.POSITIVE_INFINITY；仅当其为假值时求右侧 finite(value) && value >= 0，返回短路求值结果。 */ function finiteOrUnbounded(value: number): boolean { return value === Number.POSITIVE_INFINITY || finite(value) && value >= 0 }
...
/** 结构说明（自动提取）：buildSimulationProductionReport；输入 entities、connections、settings；直接调用 Set、entities.filter、entities.flatMap、finite、add 等；写入 children；包含循环处理。 */ export function buildSimulationProductionReport(entities: readonly Entity[], connections: readonly Connection[], settings: { gravity: number; tickRate: number; timeScale: number }): SimulationProductionReport {
  const issues: SimulationProductionReport['issues'] = [], issueKeys = new Set<string>()
  const add = /** 结构说明（自动提取）：add；输入 code、status、message、fix、params；直接调用 issueKeys.has、issueKeys.add、issues.push。 */ (code: string, status: 'review' | 'blocked', message: string, fix: string, params: Record<string, string | number> = {}) => {
  ...
  const bodies = entities.filter(/* 先计算 entity.enabled；仅当其为真值时求右侧 entity.hasComponent('RigidBody2D')，返回短路求值结果。 */ entity => entity.enabled && entity.hasComponent('RigidBody2D')), colliders = entities.flatMap(/** 结构说明（自动提取）：entities.flatMap 回调；输入 entity；直接调用 entity.getCollider。 */ entity => { const collider = entity.getCollider(); return collider?.enabled ? [{ entity, collider }] : [] })
  ...
  for (const entity of bodies) {
    ...
    if (values.some(/* 返回 finite(value) 的逻辑取反结果。 */ value => !finite(value))) add('SIM-NONFINITE-BODY', 'blocked', `${entity.name} contains a non-finite transform or physical property.`, 'Restore finite Inspector values before Play or export.', { name: entity.name })
  ...
  for (const { entity, collider } of colliders) {
    const descriptors = [{ kind: collider.shapeModel, points: collider.vertices }, ...collider.shapes.filter(/* 返回 shape.enabled 的当前值。 */ shape => shape.enabled).map(/** 构造并返回记录 { kind: shape.kind, points: shape.points }，字段按当前实参及捕获状态求值。 */ shape => ({ kind: shape.kind, points: shape.points }))]
  ...
  for (const connection of connections.filter(/* 返回 value.enabled 的当前值。 */ value => value.enabled)) {
    if (connection.anchors.length !== 2 || connection.anchors.some(/* 返回 entities.some(entity => entity.id === anchor.entityId) 的逻辑取反结果。 */ anchor => !entities.some(/* 比较 entity.id 与 anchor.entityId，返回严格相等的判断结果。 */ entity => entity.id === anchor.entityId))) add('SIM-CONSTRAINT-ANCHOR', 'blocked', `${connection.name} has a missing owner anchor.`, 'Reconnect the two existing objects in Connection Builder.', { name: connection.name })
  ...
  for (const { entity, value } of regions) {
    ...
    if (value.links.some(/* 先计算 !finiteVector(link.start) || !finiteVector(link.end) || !finite(link.cost)；仅当其为假值时求右侧 link.cost <= 0，返回短路求值结果。 */ link => !finiteVector(link.start) || !finiteVector(link.end) || !finite(link.cost) || link.cost <= 0)) add('SIM-NAV-LINK', 'blocked', `${entity.name} contains an invalid navigation link.`, 'Give every link finite endpoints and a positive traversal cost.', { name: entity.name })
    if (value.costAreas.some(/** 结构说明（自动提取）：value.costAreas.some 回调；输入 area；直接调用 finiteVector、finite；返回表达式求值结果。 */ area => !finiteVector(area.center) || !finiteVector(area.size) || !finite(area.radius) || !finite(area.multiplier) || area.multiplier <= 0 || area.navigationLayer < 1 || area.navigationLayer > 32)) add('SIM-NAV-TERRAIN', 'blocked', `${entity.name} contains an invalid terrain-cost area.`, 'Use finite shapes, positive multipliers, and layers from 1 through 32.', { name: entity.name })
  ...
  for (const entity of agents) {
    ...
    if (!finiteVector(agent.targetPosition) || !finiteVector(agent.velocity) || [agent.speed, agent.acceleration, agent.radius, agent.stoppingDistance, agent.avoidanceRadius, agent.repathInterval, agent.avoidancePriority].some(/* 返回 finite(value) 的逻辑取反结果。 */ value => !finite(value)) || agent.speed < 0 || agent.acceleration < 0 || agent.radius <= 0 || agent.stoppingDistance < 0 || agent.avoidanceRadius < 0 || agent.repathInterval < .02 || agent.avoidancePriority < 0 || agent.avoidancePriority > 1 || !validMask(agent.navigationMask)) add('SIM-NAV-AGENT', 'blocked', `${entity.name} has invalid steering, avoidance, target, layer, or mask values.`, 'Repair the NavigationAgent2D values in World Studio.', { name: entity.name })
    if (agent.targetEntityUuid && !entities.some(/* 比较 candidate.uuid 与 agent.targetEntityUuid，返回严格相等的判断结果。 */ candidate => candidate.uuid === agent.targetEntityUuid)) add('SIM-NAV-TARGET', 'blocked', `${entity.name} targets a missing navigation object.`, 'Choose an existing target object or use a world-space target position.', { name: entity.name })
    if (!regions.some(/** 结构说明（自动提取）：regions.some 回调；输入 { value }；返回表达式求值结果。 */ ({ value }) => value.navigationLayer === agent.navigationLayer && (agent.navigationMask & (1 << ((value.navigationLayer - 1) & 31))) !== 0)) add('SIM-NAV-COVERAGE', 'review', `${entity.name} has no compatible enabled navigation region.`, 'Add/enable a matching region or align its navigation layer and mask.', { name: entity.name })
  }
  ...
  const aiEntities = entities.filter(/* 先计算 entity.getComponent('BehaviorTree2D')?.enabled；仅当其为假值时求右侧 entity.getComponent('StateMachine2D')?.enabled，返回短路求值结果。 */ entity => entity.getComponent('BehaviorTree2D')?.enabled || entity.getComponent('StateMachine2D')?.enabled)
  ...
  const clothNetworks = ropeNetworks(connections), status = /** 结构说明（自动提取）：status；输入 codes；直接调用 issues.some；返回表达式求值结果。 */ (codes: string[]): SimulationCheckStatus => issues.some(/* 先计算 codes.includes(issue.code)；仅当其为真值时求右侧 issue.status === 'blocked'，返回短路求值结果。 */ issue => codes.includes(issue.code) && issue.status === 'blocked') ? 'blocked' : issues.some(/* 调用 codes.includes(issue.code) 并返回调用结果。 */ issue => codes.includes(issue.code)) ? 'review' : 'ready'
  const localized = /** 构造并返回记录 { key, params }，字段按当前实参及捕获状态求值。 */ (key: string, params: Record<string, string | number> = {}): SimulationLocalizedText => ({ key, params })
  const checks: SimulationCheck[] = [
    ...
    {
      id: 'determinism', status: issues.some(/* 比较 issue.status 与 'blocked'，返回严格相等的判断结果。 */ issue => issue.status === 'blocked') ? 'blocked' : 'ready',
  ...
  return { generatedAt: new Date().toISOString(), checks, issues, metrics: { bodies: bodies.length, colliderChildren: children, ropePaths: connections.filter(/* 比较 connection.componentType 与 'Rope2D'，返回严格相等的判断结果。 */ connection => connection.componentType === 'Rope2D').length, clothNetworks, navigationRegions: regions.length, navigationAgents: agents.length, aiAgents: aiEntities.length }, ready: checks.every(/* 比较 check.status 与 'blocked'，返回严格不等的判断结果。 */ check => check.status !== 'blocked'), checksum: simulationStateChecksum(entities, connections) }
}

/** 结构说明（自动提取）：simulationPreflight；输入 entities、connections、settings；直接调用 buildSimulationProductionReport、report.issues.filter。 */ export function simulationPreflight(entities: readonly Entity[], connections: readonly Connection[], settings: { gravity: number; tickRate: number; timeScale: number }): { report: SimulationProductionReport; blocked: SimulationProductionReport['issues']; reviews: SimulationProductionReport['issues'] } {
  const report = buildSimulationProductionReport(entities, connections, settings)
  return { report, blocked: report.issues.filter(/* 比较 issue.status 与 'blocked'，返回严格相等的判断结果。 */ issue => issue.status === 'blocked'), reviews: report.issues.filter(/* 比较 issue.status 与 'review'，返回严格相等的判断结果。 */ issue => issue.status === 'review') }
}
...
/** 结构说明（自动提取）：captureSimulationEvidence；输入 frame、entities、connections；直接调用 simulationStateChecksum、simulationEvidenceState.frames.find、simulationEvidenceState.frames.push；写入 simulationEvidenceState.lastChecksum、simulationEvidenceState.divergentFrame；返回路径包含 simulationEvidenceState.lastChecksum、checksum。 */ export function captureSimulationEvidence(frame: number, entities: readonly Entity[], connections: readonly Connection[]): string {
  ...
  const existing = simulationEvidenceState.frames.find(/* 比较 item.frame 与 frame，返回严格相等的判断结果。 */ item => item.frame === frame)
...
/** 结构说明（自动提取）：clearSimulationEvidence；无显式参数；直接调用 simulationEvidenceState.frames.splice；写入 simulationEvidenceState.lastChecksum、simulationEvidenceState.divergentFrame。 */ export function clearSimulationEvidence(): void { simulationEvidenceState.frames.splice(0); simulationEvidenceState.lastChecksum = ''; simulationEvidenceState.divergentFrame = -1 }
