...
export function analyzeProjectUpgrade(source: string): UpgradePreview {
  ...
  const maximumEngine = String(engineRange.maximumExclusive ?? '7.0.0')
  ...
  const engineRangeSupported = currentEngineInRange || v39BoundarySeal || v49BoundarySeal || v59BoundarySeal
  if (v39BoundarySeal) warnings.push('This 3.x project has an archived engine ceiling; v6 seals it to the frozen <7.0.0 range without changing schema 29.')
  if (v49BoundarySeal) warnings.push('This 4.x project has an archived engine ceiling; v6 seals it to the frozen <7.0.0 range without changing schema 29.')
  if (v59BoundarySeal) warnings.push('This 5.x project has the archived <6.0.0 ceiling; v6 seals it to the frozen <7.0.0 range without changing schema 29.')
  ...
  const engineUpgradeRequired = sourceEngine !== NOVA_ENGINE_VERSION || v39BoundarySeal || v49BoundarySeal || v59BoundarySeal
  ...
  if (engineUpgradeRequired && sourceSchema === NOVA_PROJECT_SCHEMA_VERSION) migrationSteps.push({ fromSchema: sourceSchema, toSchema: sourceSchema, name: '6.0 compatibility metadata seal (schema remains frozen)' })
