...
import { readUiTheme, themeStyle, type UiThemeDocument } from './uiTheme'
...
export interface UiAccessibilityNode { uuid: string; role: string; label: string; description: string; rect: UiRect; tabIndex: number; focused: boolean; disabled: boolean }
...
class GameUiRuntime {
  ...
  accessibilityNodes(): UiAccessibilityNode[] {
    ...
    return this.focusableItems(true).flatMap(item => {
      ...
      return [{ uuid: item.entity.uuid, role, label, description: rect.accessibilityDescription, rect: { ...item.rect }, tabIndex: rect.tabIndex, focused: item.entity === this.focused, disabled: isDisabled(item.entity) }]
    })
  }
  ...
  keyDown(event: KeyboardEvent): boolean {
    ...
    if (event.key === 'Escape' && this.awaitingRemap) { this.awaitingRemap = null; return true }
  ...
  reset(): void {
    ...
    this.resolved = []; this.hovered = null; this.pressed = null; this.focused = null; this.focusedInput = null; this.awaitingRemap = null; this.previousGamepadButtons.clear(); this.previousGamepadDirection = ''
  }
  ...
  private resolve(width: number, height: number, entities: Entity[]): ResolvedUi[] {
    ...
    const rectFor = (entity: Entity): UiRect | null => {
      ...
      const ownScale = canvas?.scaleWithScreen ? Math.min(width / Math.max(1, canvas.referenceSize.x), height / Math.max(1, canvas.referenceSize.y)) : parentScale
      ...
      if (parentPanel && parentPanel.layout !== 'None') {
        ...
        const sourceIndex = Math.max(0, siblings.indexOf(entity)), index = direction === 'rtl' && (parentPanel.layout === 'Horizontal' || parentPanel.layout === 'Grid') ? siblings.length - 1 - sourceIndex : sourceIndex
        ...
        if (parentPanel.layout === 'Vertical') {
        ...
        } else if (parentPanel.layout === 'Horizontal' && !parentPanel.wrap) {
          ...
          const columns = Math.max(1, Math.min(Math.max(1, siblings.length), Math.round(parentPanel.columns))), rows = Math.max(1, Math.ceil(siblings.length / columns)), column = index % columns, row = Math.floor(index / columns)
      ...
      const inheritedTheme = parent ? themeCache.get(parent.uuid) ?? null : null; themeCache.set(entity.uuid, canvas?.themeAsset ? readUiTheme(canvas.themeAsset) : inheritedTheme)
      return result
    }
  ...
  private draw(context: CanvasRenderingContext2D, item: ResolvedUi, editor: boolean): void {
    const { entity, rect, theme } = item, panel = entity.getComponent<Panel>('Panel'), button = entity.getComponent<Button>('Button')
      ...
      const fill = typeof style.background === 'string' ? style.background : panel ? color(panel.color, panel.opacity) : color(stateColor ?? { r: 45, g: 106, b: 214 })
    ...
    if (checkbox) {
      ...
      context.fillStyle = String(style.foreground ?? '#f5f7fb'); context.font = `${Number(style.fontWeight ?? 600)} ${Number(style.fontSize ?? 16) * item.scale}px Nunito Sans, Segoe UI, sans-serif`; context.textAlign = activeTextDirection() === 'rtl' ? 'right' : 'left'; context.textBaseline = 'middle'
    ...
    if (input) {
      ...
      context.fillStyle = String(style.foreground ?? (input.value ? '#f5f7fb' : '#7e899c')); context.font = `${Number(style.fontWeight ?? 500)} ${Number(style.fontSize ?? 16) * item.scale}px Nunito Sans, Segoe UI, sans-serif`; context.textAlign = activeTextDirection() === 'rtl' ? 'right' : 'left'; context.textBaseline = 'middle'; context.fillText(shown, activeTextDirection() === 'rtl' ? rect.x + rect.width - 12 : rect.x + 12, rect.y + rect.height / 2, Math.max(0, rect.width - 24))
    }
    const text = entity.getComponent<Text>('Text')
    if (text) {
      const fontAsset = resolveAsset(text.fontAsset), fallbacks = activeFontFallbackFamilies(), displayText = localize(text.localizationKey, text.localizationVariables, text.text), rtl = activeTextDirection() === 'rtl'
      context.fillStyle = color(text.color, text.opacity); context.font = `${text.fontWeight} ${Math.max(1, text.fontSize * item.scale)}px ${fontAsset?.assetType === 'font' && fontAsset.fontFamily ? fontAsset.fontFamily : text.fontFamily}${fallbacks.length ? `, ${fallbacks.join(', ')}` : ''}`
      ...
      const x = align === 'left' || align === 'start' ? rect.x : align === 'right' || align === 'end' ? rect.x + rect.width : rect.x + rect.width / 2; context.fillText(displayText, x, rect.y + rect.height / 2, rect.width)
    }
  }
  ...
  private focusableItems(includeNonInteractive = false): ResolvedUi[] { return this.resolved.filter(item => { const rect = item.entity.getComponent<RectTransform>('RectTransform'); return rect?.focusable && !rect.accessibilityHidden && (includeNonInteractive || interactive(item.entity)) && !isDisabled(item.entity) }).sort((first, second) => (first.entity.getComponent<RectTransform>('RectTransform')?.tabIndex ?? 0) - (second.entity.getComponent<RectTransform>('RectTransform')?.tabIndex ?? 0) || first.order - second.order) }
  private setFocus(entity: Entity): boolean { const item = this.resolved.find(candidate => candidate.entity === entity), rect = entity.getComponent<RectTransform>('RectTransform'); if (!item || !rect?.focusable || rect.accessibilityHidden || isDisabled(entity)) return false; this.focused = entity; if (runtimeAccessibilitySettings.announceFocusChanges) this.callback?.(entity, 'on_focus_enter'); return true }
  ...
  private activate(entity: Entity): void {
    ...
    if (button?.interactable) { if (rect?.remapAction) this.awaitingRemap = { entity, action: rect.remapAction, bindingIndex: Math.max(0, Math.round(rect.remapBindingIndex)) }; else this.callback?.(entity, button.onPressed || 'on_pressed') }
    else if (checkbox?.interactable) { checkbox.checked = !checkbox.checked; this.callback?.(entity, 'on_pressed') }
