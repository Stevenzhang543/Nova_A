...
export interface ProjectTestReport { format: 'nova-test-report'; version: 1; engineVersion: '3.2.0'; startedAt: string; durationMs: number; passed: number; failed: number; results: ProjectTestResult[] }
...
export async function runProjectTests(testId?: string): Promise<ProjectTestReport> {
  ...
  try {
    ...
    const report: ProjectTestReport = { format: 'nova-test-report', version: 1, engineVersion: '3.2.0', startedAt, durationMs: performance.now() - started, passed: results.filter(result => result.status === 'passed').length, failed: results.filter(result => result.status !== 'passed').length, results }
