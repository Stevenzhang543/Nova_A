...
const clamp = (value: number, minimum: number, maximum: number): number => Math.min(Math.max(positive(minimum), positive(maximum)), Math.max(positive(minimum), positive(value)))
...
export function intersectUiRects(first: UiRect, second: UiRect): UiRect { const x = Math.max(first.x, second.x), y = Math.max(first.y, second.y); return { x, y, width: Math.max(0, Math.min(first.x + first.width, second.x + second.width) - x), height: Math.max(0, Math.min(first.y + first.height, second.y + second.height) - y) } }
export function uiClipContains(clip: { rect: UiRect; rounded: number }, point: { x: number; y: number }): boolean {
...
export function uiItemVisibleArea(item: UiLayoutItem): UiRect { return item.clips.reduce((rect, clip) => intersectUiRects(rect, clip.rect), item.rect) }

/** Pure geometry shared by the actual renderer and responsive preflight; does not mutate scene components. */
export function resolveUiLayout(width: number, height: number, entities: readonly Entity[], options: UiLayoutOptions = {}): UiLayoutResult {
  ...
  const byUuid = new Map(entities.map(entity => [entity.uuid, entity])), indices = new Map(entities.map((entity, index) => [entity.uuid, index])), children = new Map<string, Entity[]>(), cache = new Map<string, UiLayoutItem | null>(), active = new Set<string>(), slots = new Map<string, Map<string, UiRect>>()
  ...
  const report = (entity: Entity, code: string, message: string) => { if (issues.length < 2000) issues.push({ entityUuid: entity.uuid, code, message }) }
  const breakpointFor = (rect: RectTransform) => rect.layoutMode === 'Fixed' ? undefined : rect.breakpoints.slice(0, 256).find(item => width >= finite(item.minWidth) && width <= finite(item.maxWidth))
  const desired = (entity: Entity, rect: RectTransform, available: UiRect, inheritedScale: number, locale: string, container = false): { width: number; height: number } => {
  ...
  const childSlots = (parent: UiLayoutItem, panel: Panel): Map<string, UiRect> => {
    ...
    const siblings = (children.get(parent.entity.uuid) ?? []).filter(entity => { const rect = entity.getComponent<RectTransform>('RectTransform'); return entity.enabled && rect?.enabled && (!entity.getComponent<Panel>('Panel')?.enabled || entity.getComponent<Panel>('Panel')!.visible) && breakpointFor(rect)?.visible !== false })
    const items = siblings.map(entity => { const rect = entity.getComponent<RectTransform>('RectTransform')!; return { entity, rect, size: desired(entity, rect, inner, parent.scale, parent.locale, true) } })
    const put = (item: typeof items[number], box: UiRect) => { const value = inset(box, item.rect.margins, parent.scale); result.set(item.entity.uuid, parent.direction === 'rtl' && item.rect.mirrorInRtl ? { ...value, x: inner.x + inner.width - (value.x - inner.x) - value.width } : value) }
    if (layout === 'Overlay' || layout === 'Margin') for (const item of items) put(item, inner)
      ...
      items.forEach((item, index) => put(item, { x: inner.x + index % columns * (cellWidth + gap), y: inner.y + Math.floor(index / columns) * (cellHeight + gap), width: cellWidth, height: cellHeight }))
    } else {
      ...
      for (const line of lines) {
        ...
        const fillers = line.filter(item => item.rect[mainPolicy] === 'Fill'), occupied = line.reduce((sum, item) => sum + (item.rect[mainPolicy] === 'Fill' ? 0 : item.size[main]), 0) + gap * (line.length - 1), fill = Math.max(0, inner[main] - occupied) / Math.max(1, fillers.length)
        const sizes = line.map(item => item.rect[mainPolicy] === 'Fill' ? clamp(fill, (vertical ? item.rect.minSize.y : item.rect.minSize.x) * parent.scale, (vertical ? item.rect.maxSize.y : item.rect.maxSize.x) * parent.scale) : item.size[main])
        const free = Math.max(0, inner[main] - sizes.reduce((sum, value) => sum + value, 0) - gap * (line.length - 1)), spacing = gap + (panel.justify === 'SpaceBetween' && line.length > 1 ? free / (line.length - 1) : 0)
        let position = panel.justify === 'Center' ? free / 2 : panel.justify === 'End' ? free : 0
        const crossSize = wrap ? Math.max(0, ...line.map(item => item.size[cross])) : inner[cross]
        line.forEach((item, index) => { const extent = panel.align === 'Stretch' || item.rect[vertical ? 'horizontalPolicy' : 'verticalPolicy'] === 'Fill' ? crossSize : item.size[cross], offset = panel.align === 'Center' ? (crossSize - extent) / 2 : panel.align === 'End' ? crossSize - extent : 0; put(item, vertical ? { x: inner.x + crossPosition + offset, y: inner.y + position, width: extent, height: sizes[index] } : { x: inner.x + position, y: inner.y + crossPosition + offset, width: sizes[index], height: extent }); position += sizes[index] + spacing })
        crossPosition += crossSize + gap
      }
    }
    return result
  }
  const resolve = (entity: Entity, depth = 0): UiLayoutItem | null => {
