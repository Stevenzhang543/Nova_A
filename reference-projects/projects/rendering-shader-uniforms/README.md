...
Demonstrates: typed reflected shader uniforms, includes, variants and compile cache, source-linked safe-shader diagnostics and material serialization. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/rendering-shader-uniforms/project.nova --target web --profile release --output ./Builds/reference-rendering-shader-uniforms --cache validate --jsonl
```
