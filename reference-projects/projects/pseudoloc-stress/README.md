# Nova_A Pseudolocalization Stress

Engine **4.4.0**, Project Format 2, schema 29.

## Expected behavior

Demonstrates: 35% string expansion, clipping and missing-key audit. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/pseudoloc-stress/project.nova --target web --profile release --output ./Builds/reference-pseudoloc-stress --cache validate --jsonl
```
