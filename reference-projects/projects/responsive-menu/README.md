# Nova_A Responsive Menu

Engine **5.0.1**, Project Format 2, schema 29.

## Expected behavior

Demonstrates: responsive anchors, safe areas and DPI previews, row, column, grid, flow and overlay layout containers. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/responsive-menu/project.nova --target web --profile release --output ./Builds/reference-responsive-menu --cache validate --jsonl
```
