...
Demonstrates: complete 16-mask terrain rules, weighted deterministic variants, terrain validation and preview. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/tilemap-terrain-rules/project.nova --target web --profile release --output ./Builds/reference-tilemap-terrain-rules --cache validate --jsonl
```
