# Script v4.6 Language Services

Engine **5.3.0**, Project Format 2, schema 29.

## Purpose

Large-symbol completion, hover, signature help, definition, references, rename, formatting, module assistance, cancellation and persisted workspace indexing.

## Test procedure

1. Open `project.nova` and switch to **Script**.
2. Follow `test-controls.json` and compare the result with `expected-output.json`.
3. Run `pnpm nova script-test ./reference-projects/projects/script-v46-language-services/project.nova --format json` when the project path is supported, or use the listed `Assets/Scripts` fixture sources directly.
