...
Demonstrates: 2D listener, panning and distance attenuation, polyphony, deterministic pitch/volume variation, voice priority and limits. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/audio-positional/project.nova --target web --profile release --output ./Builds/reference-audio-positional --cache validate --jsonl
```
