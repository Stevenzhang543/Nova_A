# Nova_A Runtime Rebinding

Engine **5.0.1**, Project Format 2, schema 29.

## Expected behavior

Demonstrates: runtime rebinding UI, binding conflict detection and persisted modifiers/chords. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/runtime-rebinding/project.nova --target web --profile release --output ./Builds/reference-runtime-rebinding --cache validate --jsonl
```
