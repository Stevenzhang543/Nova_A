...
Demonstrates: Prefabs, Scene switch, Triggers, Particles, Save API. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/top-down/project.nova --target web --profile release --output ./Builds/reference-top-down --cache validate --jsonl
```
