...
Demonstrates: bus routing, compressor and delay effects, snapshots, sends, meters and ducking, deterministic voice limits. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/audio-bus-effects/project.nova --target web --profile release --output ./Builds/reference-audio-bus-effects --cache validate --jsonl
```
