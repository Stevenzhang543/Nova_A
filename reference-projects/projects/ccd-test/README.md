...
Demonstrates: continuous collision detection, thin-wall tunneling reference. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/ccd-test/project.nova --target web --profile release --output ./Builds/reference-ccd-test --cache validate --jsonl
```
