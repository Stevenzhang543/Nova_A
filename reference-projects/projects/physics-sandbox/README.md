...
Demonstrates: Rigid bodies, Materials, Rope2D, Joints, Debugger. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/physics-sandbox/project.nova --target web --profile release --output ./Builds/reference-physics-sandbox --cache validate --jsonl
```
