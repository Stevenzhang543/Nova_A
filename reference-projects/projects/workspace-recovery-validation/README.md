...
Demonstrates: workspace import/export, docking, 100-step history, autosave recovery, safe layout, read-only recovery. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/workspace-recovery-validation/project.nova --target web --profile release --output ./Builds/reference-workspace-recovery-validation --cache validate --jsonl
```
