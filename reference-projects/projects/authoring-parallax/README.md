...
Demonstrates: parallax parent layer, camera-relative motion scale, repeat metadata, hierarchy reparenting. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/authoring-parallax/project.nova --target web --profile release --output ./Builds/reference-authoring-parallax --cache validate --jsonl
```
