...
Demonstrates: nearest filtering, pixel-perfect camera, sprite asset drop, selection outlines at multiple zooms. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/authoring-pixel-art/project.nova --target web --profile release --output ./Builds/reference-authoring-pixel-art --cache validate --jsonl
```
