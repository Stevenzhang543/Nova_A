# Resolution Independent Authoring

Engine **5.3.0**, Project Format 2, schema 29.

## Expected behavior

Demonstrates: world-unit scaling, common-resolution overlays, vector shapes, camera aspect previews. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/authoring-resolution-independent/project.nova --target web --profile release --output ./Builds/reference-authoring-resolution-independent --cache validate --jsonl
```
