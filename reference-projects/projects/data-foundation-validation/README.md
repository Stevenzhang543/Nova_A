...
Demonstrates: manifest and directory policy, nested scenes, nested prefabs, per-property overrides, imported artifact hashes, dependency graph, missing-resource repair. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/data-foundation-validation/project.nova --target web --profile release --output ./Builds/reference-data-foundation-validation --cache validate --jsonl
```
