...
Demonstrates: animated tiles, custom metadata, scene/prefab placement fields, atlas sources. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/tilemap-animated/project.nova --target web --profile release --output ./Builds/reference-tilemap-animated --cache validate --jsonl
```
