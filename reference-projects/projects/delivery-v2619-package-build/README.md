...
4. Open Manage → Packages → Browse; review and install Nova Navigation 2D. Disable, enable and uninstall it, then undo and save/reopen. **Expected:** The offline bundled entry shows publisher, permissions, dependency and hash information. No registry download is needed. Package and plugin state follow successful lifecycle actions.

5. For a separately reviewed signed update manifest, opt in and stage it; disable updates during verification. **Expected:** A cancelled check cannot stage an update. Staging downloads and installs nothing; native installer lifecycle needs separate disposable-host evidence.
