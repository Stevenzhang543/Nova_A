...
Demonstrates: cancellable tasks, timers, error propagation and ownership. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/script-async-tasks/project.nova --target web --profile release --output ./Builds/reference-script-async-tasks --cache validate --jsonl
```
