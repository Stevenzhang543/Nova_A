...
Demonstrates: deterministic save migrations, checksum and transaction journal, backup and corruption recovery, async progress and cancellation. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/save-migration/project.nova --target web --profile release --output ./Builds/reference-save-migration --cache validate --jsonl
```
