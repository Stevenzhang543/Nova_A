...
Demonstrates: experimental transport/RPC/authority contracts, replication, prediction and diagnostics, headless fixed-tick interface, security and packet-loss gates. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/headless-networking/project.nova --target web --profile release --output ./Builds/reference-headless-networking --cache validate --jsonl
```

## Requirements

- Required packages: top.whitelists.novaa.networking@2.9.0.
