# Nova_A Empty 2D Reference

Engine **4.4.0**, Project Format 2, schema 29.

## Expected behavior

Demonstrates: Camera2D, Input map, Build settings. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/empty/project.nova --target web --profile release --output ./Builds/reference-empty --cache validate --jsonl
```
