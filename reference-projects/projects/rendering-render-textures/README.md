# Render Textures and Cameras

Engine **5.3.0**, Project Format 2, schema 29.

## Expected behavior

Demonstrates: multiple cameras and viewports, named render-texture target, post-processing render graph and fallback. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/rendering-render-textures/project.nova --target web --profile release --output ./Builds/reference-rendering-render-textures --cache validate --jsonl
```
