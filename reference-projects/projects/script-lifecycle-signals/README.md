# Nova_A Lifecycle and Signals

Engine **4.4.0**, Project Format 2, schema 29.

## Expected behavior

Demonstrates: all lifecycle callbacks, typed exported property metadata, editor-visible signals. Open the project, run Play/Pause/Step/Stop, and compare the scene/entity minimums and diagnostics with `expected-output.json`.
...
```powershell
pnpm nova export --project ./reference-projects/projects/script-lifecycle-signals/project.nova --target web --profile release --output ./Builds/reference-script-lifecycle-signals --cache validate --jsonl
```
