/** 验证脚本（v26.23-template-output）：组织对应功能与边界场景检查，断言行为并汇总验证结果。 */
import assert from 'node:assert/strict'
// Retained 26.20 regression implementation, executed against actual 26.23 source.
...
const release = '26.23', development = process.argv.includes('--development')
if (!development) assert.equal(engineVersion, '26.23.0', 'Template output qualification requires actual 26.23 source')
...
const report = {
  format: 'nova-v26.23-template-output-verification',
  version: 1,
  release,
  engineVersion,
  development,
...
await writeFile(join(root, 'release-audits', 'v26.23-template-output.json'), `${JSON.stringify(report, null, 2)}\n`)
if (failed.length) { console.error(failed); process.exit(1) }
console.log(`Nova_A 26.23 template output verification passed: ${templateResults.length} templates, ${checks.length} checks.`)
