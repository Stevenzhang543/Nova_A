...
export function inspectRenderingGame(bytes,template,savedProject){
 ...
 for(const entry of index.entries){
  assert.ok(typeof entry.path==='string'&&entry.path&&!entry.path.includes('\\')&&!entry.path.includes(':')&&!entry.path.split('/').some(part=>!part||part==='.'||part==='..')&&!files.has(entry.path),'Safe unique embedded path')
...
export function validateRenderingUserEvidence(report,nativeBuildCompletedAt){
 ...
 assert.ok(Array.isArray(report.checks)&&report.checks.length>=4&&report.checks.every(check=>check.status==='passed'),'Actual browser authoring must pass first')
 ...
 const project=report.observations?.find(item=>item.name==='downloaded-lit-project'),web=report.observations?.find(item=>item.name==='actual-export-player')
 ...
 assert.ok(web?.engineVersion==='26.15.0'&&Array.isArray(web.pixels)&&web.pixels.length===3&&web.pixels.every(pixel=>Array.isArray(pixel)&&pixel.length===4&&pixel.every(value=>Number.isInteger(value)&&value>=0&&value<=255)),'Three real browser player pixel oracles are required')
 return{project,web}
}

export function selectRenderingExecutable(report){
 const files=report.files?.filter(file=>typeof file?.path==='string'&&/^[^/\\]+\.exe$/i.test(file.path))??[]
