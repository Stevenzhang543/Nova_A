...
export async function openMediaAuditModules(context,entries){
 ...
 const close=async()=>{const suffix=relative(join(root,'.cache'),temporary);assert.ok(suffix.startsWith('nova-v2616-media-corpus-')&&!isAbsolute(suffix)&&!suffix.startsWith('..'));if(process.env.NOVA_AUDIT_NODE_OPERATION_COVERAGE==='1')await captureNodeBundle22(temporary);await rm(temporary,{recursive:true,force:true})}
 const inputs=Object.fromEntries(Object.entries(entries).map(([name,path])=>[name,locate(path)]));if(context.initializeBundledWasm)inputs.__wasm=join(root,'nova_core/pkg/nova_core.js')
 ...
 const modules=Object.fromEntries(await Promise.all(Object.keys(inputs).map(async name=>[name,await import(pathToFileURL(join(temporary,name+'.mjs')).href)])))
 ...
 globalThis.window={setTimeout:()=>0,clearTimeout(){},addEventListener(){},removeEventListener(){},...(context.initializeBundledWasm?{performance:globalThis.performance}:{})}
