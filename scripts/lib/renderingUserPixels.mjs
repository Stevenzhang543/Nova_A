...
/** 解码有尺寸限制的八位非交错 RGB 或 RGBA 截图，重建过滤行并统一输出 RGBA。 */ export function decodeAuditPng(bytes){
 ...
 const paeth=/** 按 PNG Paeth 预测规则选择最接近预测值的邻近字节。 */ (a,b,c)=>{const p=a+b-c,pa=Math.abs(p-a),pb=Math.abs(p-b),pc=Math.abs(p-c);return pa<=pb&&pa<=pc?a:pb<=pc?b:c}
 for(let y=0;y<height;y++){const filter=raw[y*(stride+1)];for(let x=0;x<stride;x++){const value=raw[y*(stride+1)+1+x],left=x>=channels?reconstructed[y*stride+x-channels]:0,above=y?reconstructed[(y-1)*stride+x]:0,diagonal=y&&x>=channels?reconstructed[(y-1)*stride+x-channels]:0;reconstructed[y*stride+x]=(value+(filter===0?0:filter===1?left:filter===2?above:filter===3?Math.floor((left+above)/2):filter===4?paeth(left,above,diagonal):(/** 遇到未知 PNG 行过滤类型时拒绝解码。 */ ()=>{throw Error('Invalid PNG filter')})()))&255}}
...
/** 将像素坐标取整并限制在图像内，返回四个颜色通道。 */ export function auditPixel(image,x,y){const start=(Math.max(0,Math.min(image.height-1,Math.round(y)))*image.width+Math.max(0,Math.min(image.width-1,Math.round(x))))*4;return [...image.rgba.subarray(start,start+4)]}
