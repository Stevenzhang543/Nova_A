...
export function validateNativeRendererVersion(version) {
...
export function validateNativeRendererReceipt(receipt,digest,sha256,bytes) {
 ...
 const matching=(receipt.gates??[]).filter(gate=>gate.id==='native-build'),gate=matching[0]
 ...
 const artifacts=(gate.artifacts??[]).filter(artifact=>artifact.name==='windows-editor')
...
export function validateRendererBrowserComparison(browser,digest) {
 if(browser?.sourceInputDigest!==digest||browser.sourceMode!=='staged'||!Array.isArray(browser.checks)||!browser.checks.length||browser.checks.some(check=>check.status!=='passed')||!Array.isArray(browser.captures)||!browser.captures.length||new Set(browser.captures.map(capture=>capture.name)).size!==browser.captures.length)throw Error('Browser comparison must pass against the exact same frozen source digest and contain unique actual captures.')
}

export async function qualifyNativeRendererInput(root,sourceRoot,executable,snapshotPath,comparisonPath,receiptPath) {
...
export async function compareRendererCaptures(nativeReport,nativeInput,evaluate) {
 const pairs=[]
 for(const capture of nativeReport.captures) {
  const baseline=nativeInput.browser.captures.find(item=>item.name===capture.name)
