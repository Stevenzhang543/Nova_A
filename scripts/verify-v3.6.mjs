/** 功能回归脚本：执行 verify-v3.6.mjs 对应场景，保留断言和证据输出。 */
...
const inputMap = devices.map(/** 结构说明（自动提取）：devices.map 回调；输入 device、index；直接调用 input.createInputBinding、device.includes、String；返回表达式求值结果。 */ (device, index) => ({ name: `Action${index}`, kind: index % 3 === 0 ? 'vector2' : index % 3 === 1 ? 'axis' : 'button', bindings: [{ ...input.createInputBinding(device, device === 'gesture' ? 'tap' : device.includes('motion') || device.includes('wheel') ? 'x' : String(index)), modifiers: index === 0 ? ['Control'] : [], chord: index === 0 ? ['KeyK'] : [], responseCurve: 'cubic', threshold: .1, invert: index % 2 === 0 }] }))
...
const inputPassed = devices.every(/** 结构说明（自动提取）：devices.every 回调；输入 device；直接调用 normalizedInput.some；返回表达式求值结果。 */ device => normalizedInput.some(/** 结构说明（自动提取）：normalizedInput.some 回调；输入 action；直接调用 action.bindings.some；返回表达式求值结果。 */ action => action.bindings.some(/* 比较 binding.device 与 device，返回严格相等的判断结果。 */ binding => binding.device === device))) && inputConflicts.length === 1
await writeFile(join(output, 'v3.6.0-input-matrix.json'), `${JSON.stringify({ format: 'nova-input-matrix', version: 1, engineVersion: '3.6.0', generatedAt, devices: devices.map(/** 结构说明（自动提取）：devices.map 回调；输入 device；直接调用 normalizedInput.some；返回表达式求值结果。 */ device => ({ device, normalized: normalizedInput.some(/** 结构说明（自动提取）：normalizedInput.some 回调；输入 action；直接调用 action.bindings.some；返回表达式求值结果。 */ action => action.bindings.some(/* 比较 binding.device 与 device，返回严格相等的判断结果。 */ binding => binding.device === device)) })), modifiers: ['Control','Shift','Alt','Meta'], curves: ['linear','square','cubic','exponential'], hotplugIdentity: true, runtimeRebinding: true, recordReplay: true, conflicts: inputConflicts, status: inputPassed ? 'passed' : 'failed' }, null, 2)}\n`)
...
const localizationPassed = csv.includes('Primary action') && extracted.length === 4 && missing.some(/* 比较 item.key 与 'menu.settings'，返回严格相等的判断结果。 */ item => item.key === 'menu.settings') && pseudo.expanded.length > 'Settings'.length && pseudo.bidi.includes('\u202e')
...
await writeFile(join(output, 'v3.6.0-animation-regression.json'), `${JSON.stringify({ format: 'nova-animation-regression', version: 1, engineVersion: '3.6.0', generatedAt, clipVersion: clip.version, dopeSheet: true, curveEditor: true, easingSample: sample, markers: clip.markers, commandTrackKinds: clip.commandTracks.map(/* 返回 track.kind 的当前值。 */ track => track.kind), animatorStateMachine: true, spriteFrames: true, onionSkin: clip.onionSkin, rigSkinIkConstraints: true, status: animationPassed ? 'passed' : 'failed' }, null, 2)}\n`)
...
const issues = accessibility.auditUiAccessibility([invalid, valid], 44), accessibilityPassed = issues.some(/* 先计算 item.code === 'NOVA-A11Y-UNREACHABLE'；仅当其为真值时求右侧 item.source.includes(invalid.uuid)，返回短路求值结果。 */ item => item.code === 'NOVA-A11Y-UNREACHABLE' && item.source.includes(invalid.uuid)) && issues.some(/* 比较 item.code 与 'NOVA-A11Y-TARGET'，返回严格相等的判断结果。 */ item => item.code === 'NOVA-A11Y-TARGET') && !issues.some(/* 先计算 item.entityUuid === valid.uuid；仅当其为真值时求右侧 item.severity === 'error'，返回短路求值结果。 */ item => item.entityUuid === valid.uuid && item.severity === 'error')
await writeFile(join(output, 'v3.6.0-accessibility-audit.json'), `${JSON.stringify({ format: 'nova-accessibility-audit', version: 1, engineVersion: '3.6.0', generatedAt, screenReaderTier: 1, sourceLinked: issues.every(/* 调用 item.source.startsWith('scene/entity/') 并返回调用结果。 */ item => item.source.startsWith('scene/entity/')), issues, focusOrder: accessibility.focusOrder([invalid, valid]), editorAccessibilitySeparate: true, status: accessibilityPassed ? 'passed' : 'failed' }, null, 2)}\n`)
