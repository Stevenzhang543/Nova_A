/** 功能回归脚本：执行 verify-v5.0-windows.mjs 对应场景，保留断言和证据输出。 */
...
const paths = {
  ...
  msi: join(root, 'src-tauri', 'target', 'release', 'bundle', 'msi', 'Nova_A_5.0.0_x64_en-US.msi'),
  setup: join(root, 'src-tauri', 'target', 'release', 'bundle', 'nsis', 'Nova_A_5.0.0_x64-setup.exe')
}
const artifacts = {}
...
try {
  ...
  const report = {
    format: 'nova-v5.0-windows-smoke', version: 1, engineVersion: '5.0.0', generatedAt, host: `${process.platform}-${process.arch}`,
  ...
  await writeFile(join(root, 'release-audits', 'v5.0.0-windows-smoke.json'), `${JSON.stringify(report, null, 2)}\n`)
  if (status !== 'passed') process.exitCode = 1
  console.log(`Nova_A v5.0 Windows smoke ${status}.`)
} finally {
