/** 功能回归脚本：执行 verify-v26.20-template-library.mjs 对应场景，保留断言和证据输出。 */
...
await withBrowserAudit({release:'26.20',name:'template-library',root,development:process.env.NOVA_AUDIT_DEVELOPMENT==='1',expectedRelease:process.env.NOVA_AUDIT_EXPECTED_RELEASE || '26.20',width:1600,height:1000}, /** 结构说明（自动提取）：withBrowserAudit 回调；输入 audit；直接调用 client.on、client.send、until、evaluate、check 等；包含循环处理；等待异步结果。 */ async audit => {
