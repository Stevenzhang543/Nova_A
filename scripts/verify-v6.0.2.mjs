/** 功能回归脚本：执行 verify-v6.0.2.mjs 对应场景，保留断言和证据输出。 */
...
try {
  ...
  const templateResults = templates.PROJECT_TEMPLATES.map(/** 结构说明（自动提取）：templates.PROJECT_TEMPLATES.map 回调；输入 item；直接调用 templates.createTemplateProject、templates.auditTemplateProject。 */ item => {
  ...
  check('V602-TEMPLATES', templateResults.every(/* 比较 item.failures.length 与 0，返回严格相等的判断结果。 */ item => item.failures.length === 0), 'Every startup template creates and passes the production template audit.', { templates: templateResults.length, failures: templateResults.filter(/* 返回 item.failures.length 的当前值。 */ item => item.failures.length) })
  ...
  check('V602-PACK-ROUNDTRIP', parsed.index.engineVersion === '6.0.2' && parsed.files.has('project.nova') && parsed.index.entries.every(/* 调用 parsed.files.has(entry.path) 并返回调用结果。 */ entry => parsed.files.has(entry.path)), 'Nova package creation, checksum validation and parsing round-trip every entry.', { bytes: packageBytes.byteLength, entries: parsed.index.entries.length })
  ...
  const invalidCodes = builds.validateBuildSettings(invalidSettings, { host: 'windows', architecture: 'x86_64', androidAvailable: false, androidReason: 'not installed' }).map(/* 返回 issue.code 的当前值。 */ issue => issue.code)
  ...
  const androidIssues = builds.validateBuildSettings(androidSettings, { host: 'windows', architecture: 'x86_64', androidAvailable: true, androidReason: '' }).map(/* 返回 issue.code 的当前值。 */ issue => issue.code)
  check('V602-BUILD-VALIDATION', ['scene', 'identifier', 'version'].every(/* 调用 invalidCodes.includes(code) 并返回调用结果。 */ code => invalidCodes.includes(code)) && !androidIssues.includes('architecture') && !androidIssues.includes('host'), 'Build validation rejects incomplete configuration while allowing an installed Android cross-target template.', { invalidCodes, androidIssues })
  ...
  check('V602-PORTABLE-FOOTER', magic === 'NOVAPK2!' && embeddedLength > 16 && embeddedHash === actualHash && !portableFiles.some(/* 调用 path.endsWith('game.nova-pak') 并返回调用结果。 */ path => path.endsWith('game.nova-pak')), 'Single-file CLI output embeds the package behind the player with exact length and SHA-256 footer.', { executableBytes: executable.length, embeddedLength, files: portableFiles.map(/* 调用 relative(output, path) 并返回调用结果。 */ path => relative(output, path)) })
  ...
  const maliciousAsset = malicious.assets.find(/* 比较 asset.assetType 与 'script'，返回严格相等的判断结果。 */ asset => asset.assetType === 'script')
  ...
  check('V602-NATIVE-BOUNDS', ['MAX_WEB_EXPORT_FILES', 'MAX_WEB_EXPORT_FILE_BYTES', 'MAX_WEB_EXPORT_TOTAL_BYTES', 'decode_base64_limited', 'nova-write-', 'nova-backup-'].every(/* 调用 native.includes(marker) 并返回调用结果。 */ marker => native.includes(marker)), 'Native export bounds web payloads and performs rollback-capable replacement.')
  ...
  check('V602-CLI-HARDENING', ['NOVAPK2!', 'MAX_PACKAGE_BYTES', 'dynamicImports', 'The game output cannot overwrite its player template'].every(/* 调用 cliSource.includes(marker) 并返回调用结果。 */ marker => cliSource.includes(marker)), 'Headless export has bounded embedded packages, dynamic web chunk traversal and overwrite protection.')
} finally {
...
/** 结构说明（自动提取）：allFiles；输入 directory；直接调用 readdir、join、entry.isDirectory、files.push、allFiles；返回路径包含 files；包含循环处理；等待异步结果。 */ async function allFiles(directory) { const files=[];for(const entry of await readdir(directory,{withFileTypes:true})){const path=join(directory,entry.name);entry.isDirectory()?files.push(...await allFiles(path)):files.push(path)}return files }
