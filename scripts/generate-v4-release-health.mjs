...
const names = (await readdir(directory)).filter(/* 先计算 name.startsWith('v4.0.0-') && name.endsWith('.json')；仅当其为真值时求右侧 name !== 'v4.0.0-release-health.json'，返回短路求值结果。 */ name => name.startsWith('v4.0.0-') && name.endsWith('.json') && name !== 'v4.0.0-release-health.json')
const reports = []
...
const failed = reports.filter(/* 比较 item.status 与 'failed'，返回严格相等的判断结果。 */ item => item.status === 'failed'), external = reports.filter(/* 调用 String(item.status).includes('external') 并返回调用结果。 */ item => String(item.status).includes('external'))
...
const rows = reports.map(/** 生成发布健康报告表格行。 */ item=>`<tr><td>${item.name}</td><td>${item.format}</td><td data-status="${item.status}">${item.status}</td></tr>`).join('')
