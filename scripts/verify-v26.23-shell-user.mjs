...
for(const index of await readdir(registry)) {const entries=await readdir(join(registry,index));const opener=entries.find(name=>name==='tauri-plugin-opener-2.5.3');if(opener){interception=await readFile(join(registry,index,opener,'src/init-iife.js'),'utf8');break}}
assert.ok(interception,'The installed native opener interceptor is required for this regression.')
await withBrowserAudit({release:'26.23',name:'shell-user',development:expected!=='26.23',expectedRelease:expected},async a=>{
 const u=worldUserControls17(a)
 await a.check('Launcher opens the complete library only on New and retains cancelled drafts',async()=>{
 ...
 const install=async()=>{await a.evaluate(interception+';true');await a.evaluate(`(()=>{window.__novaOpenCalls=[];window.__novaOpenDeny=false;window.__TAURI_INTERNALS__={invoke:(command,args)=>{window.__novaOpenCalls.push({command,url:args?.url});return window.__novaOpenDeny&&command==='plugin:opener|open_url'?Promise.reject(new Error('Permission denied by test host')):Promise.resolve(null)}};return true})()`)}
 for(const[label,selector]of[['launcher','.project-manager .identity'],['editor','.top-bar .brand']]){
  ...
  await a.check(label+' native brand link invokes once and a denial remains recoverable',async()=>{
 ...
 await a.check('Rapid top-menu switching retains at most one interactive dropdown and dismisses safely',async()=>{
 ...
 await a.check('Shared workspace popover supports keyboard retention, Escape and touch outside dismissal',async()=>{
