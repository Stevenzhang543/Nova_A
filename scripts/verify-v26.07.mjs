/** 验证脚本（v26.07）：组织对应功能与边界场景检查，断言行为并汇总验证结果。 */
...
check('V2607-TRANSPORT-LIFECYCLE', ['snapshotAccumulator', 'scheduledDeliveries', 'connectionGeneration', 'peerSources', 'reliableBuffers.clear()', 'remoteInputs.clear()'].every(/* 调用 networking.includes(token) 并返回调用结果。 */ token => networking.includes(token)), 'Fixed cadence, delayed-send cancellation, endpoint binding and disconnect cleanup are present.')
check('V2607-SECURITY-BOUNDS', ['maximumPendingReliable', 'maximumPacketBytes', 'maximumMessagesPerSecond', 'authenticationRejected', 'replayRejected', 'schemaRejected'].every(/* 调用 networking.includes(token) 并返回调用结果。 */ token => networking.includes(token)), 'Packet, reliable-window, rate, authentication, replay and schema boundaries remain observable and bounded.')
check('V2607-OPTIONAL-SERVICES', ['registerReviewedNetworkService', 'networkServiceReviewIssues', 'networkServiceSelectionIssues', 'openReviewedNetworkService'].every(/* 调用 services.includes(token) 并返回调用结果。 */ token => services.includes(token)) && build.includes('networkServiceSelectionIssues'), 'Identity, lobby and relay extension points require reviewed metadata and participate in Build validation.')
...
const nativeTools = ['launch_network_instances', 'network_instance_status', 'stop_network_instance', 'stop_network_instances', 'validate_network_player_build', 'nova-build-report.json', 'embedded_package(executable)', 'network_policy_from_project', 'endpoint: host_endpoint.clone()', 'bind_address'].every(/* 调用 native.includes(token) 并返回调用结果。 */ token => native.includes(token))
const panelTools = ['openInstanceLogs', 'openInstanceInspector', 'stopNetworkInstance', 'instance.endpoint', 'instance.bindAddress', 'selectedInstance.endpoint', 'selectedInstance.bindAddress'].every(/* 调用 panel.includes(token) 并返回调用结果。 */ token => panel.includes(token))
...
for (const [id, expectedMode, expectedRole] of [['multiplayer-v2607-coop-rollback', 'game', 'host'], ['multiplayer-v2607-headless-authority', 'headless-server', 'server']]) {
  ...
  check(`V2607-REFERENCE-${expectedRole.toUpperCase()}`, project?.engineVersion === engineVersion && project?.formatVersion === 29 && project?.projectSettings?.build?.runtimeMode === expectedMode && project?.projectSettings?.build?.platform?.version === engineVersion && network?.enabled === true && network?.permissionGranted === true && network?.autoStart === true && network?.sessionMode === 'direct' && network?.role === expectedRole && network?.protocolVersion === 2 && network?.replicatedEntities?.length >= 2 && network?.rpcContracts?.length >= 1 && installed.some(/** 检查联机包在项目中启用、通过安全验证且具备客户端与监听权限。 */ item => item?.manifest?.id === 'top.whitelists.novaa.networking' && item.enabled === true && item.project === true && item.securityStatus === 'verified' && item.grantedPermissions?.includes('network.client') && item.grantedPermissions?.includes('network.listen')) && locked.some(/* 比较 item?.id 与 'top.whitelists.novaa.networking'，返回严格相等的判断结果。 */ item => item?.id === 'top.whitelists.novaa.networking'), `The ${expectedRole} reference is authored, permission-granted, package-locked and configured for its actual ${expectedMode} workflow.`, { error, replicatedEntities: network?.replicatedEntities?.length ?? 0, rpcContracts: network?.rpcContracts?.length ?? 0 })
}
...
/** 构造三种联机开关和包安装组合的Web导出样本，验证条件依赖裁剪并清理临时目录。 */ async function verifyConditionalWebExport() {
  ...
  try {
    ...
    for (const fixture of cases) {
      ...
      const exists = /** 尝试读取导出资源，以判断该资源是否存在且可读。 */ async name => { try { await readFile(join(output, 'assets', name)); return true } catch { return false } }
    ...
    result.passed = result.cases.length === 3 && result.cases.every(/* 返回 item.passed 的当前值。 */ item => item.passed)
  } catch (error) {
