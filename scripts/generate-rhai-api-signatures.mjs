...
const run = (command, args) => new Promise((resolve, reject) => {
  ...
  let stdout = '', stderr = ''; child.stdout.on('data', value => { stdout += value }); child.stderr.on('data', value => { stderr += value; process.stderr.write(value) }); child.on('error', reject)
...
try {
  ...
  const bindingTool = process.argv.find(value => value.startsWith('--wasm-bindgen='))?.slice(15) ?? 'wasm-bindgen'
  ...
  console.log(JSON.stringify({ rhaiVersion: version, host: inventory.signatures.filter(item => item.origin === 'host').length, signatures: inventory.signatures.length, packages: inventory.packages.length, path: 'src/visual/rhaiApiSignatures.generated.json' }))
