/** 版本26.06：读取功能注册与源码信息，生成当前版本的功能清单文档。 */
...
const escape = /* 调用 String(value).replaceAll('|', '\\|') 并返回调用结果。 */ value => String(value).replaceAll('|', '\\|')
...
try {
  ...
  for (const [key, items] of groups) lines.push(`| ${escape(key)} | ${items.length} | ${items.filter(/* 比较 item.dimensions.runtimeExport.status 与 'covered'，返回严格相等的判断结果。 */ item => item.dimensions.runtimeExport.status === 'covered').length} | ${items.filter(/* 比较 item.dimensions.tests.status 与 'external'，返回严格相等的判断结果。 */ item => item.dimensions.tests.status === 'external').length} |`)
  ...
  for (const [key, items] of groups) {
    ...
    for (const item of items) {
      const cell = /* 调用 escape(`${item.dimensions[dimension].status}: ${item.dimensions[dimension].route}`) 并返回调用结果。 */ dimension => escape(`${item.dimensions[dimension].status}: ${item.dimensions[dimension].route}`)
