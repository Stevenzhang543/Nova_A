...
const root=process.cwd(),option=name=>process.argv.find(value=>value.startsWith('--'+name+'='))?.slice(name.length+3)
...
const check=async(name,operation)=>{try{await operation();checks.push({name,status:'passed'})}catch(error){failures.push(error);checks.push({name,status:'failed',error:String(error),stack:error.stack})}}
const bundle=async(directory,target)=>{
  const overlay={name:'isolated-projection-source',enforce:'pre',resolveId(source,importer){
    ...
    const prefix=[join(target,'src'),join(root,'src')].find(prefix=>raw.startsWith(prefix+sep))
  ...
  const input=Object.fromEntries(Object.entries(paths).map(([name,path])=>[name,join(existsSync(join(target,'src',path+'.ts'))?target:root,'src',path+'.ts')]))
  ...
  return Object.fromEntries(await Promise.all(Object.keys(input).map(async name=>[name,await import(pathToFileURL(join(directory,name+'.mjs')).href)])))
}
try{
  ...
  const execute=source=>{const runtime=new wasm.WasmScriptRuntime();try{const {commands,logs,properties}=JSON.parse(runtime.execute_json(source,'start','{"entity":"projection-probe"}'));return{commands,logs,properties}}catch(error){return{error:String(error)}}finally{runtime.free()}}
  const compile=(modules,graph)=>{const compiled=modules.compiler.compileGraph(graph);assert.equal(compiled.valid,true,JSON.stringify(compiled.diagnostics));return compiled.source}
  const equivalent=(expected,actual,fixture)=>{if(fixture.error){assert.match(expected.error??'',fixture.error);assert.match(actual.error??'',fixture.error)}else{assert.equal(expected.error,undefined);assert.deepEqual(actual,expected)}}
  const fields=graph=>graph.nodes.map(node=>({id:node.config.astId,type:node.type,fields:node.config.fields,spans:node.config.fieldSpans,source:node.config.source,comments:node.config.ownedComments,binding:node.config.bindingId,scope:node.config.syntaxScopeId})).sort((a,b)=>a.id.localeCompare(b.id))
  for(const fixture of rhaiCorpus)await check('independent baseline/staged projection and full regeneration: '+fixture.name,()=>{
  ...
  await check('exact source ranges choose the matching syntax kind for equal-span wrappers',()=>{
    for(const source of ['fn value(){42} fn start(){print(value());}','fn start(){let x=1;{print(x)}}','fn start(){print(`value ${1+2}`);}']){
      const graph=current.syntax.projectRhaiSyntax(source),assessment=current.syntax.assessRhaiConversion(source,'ranges',graph),byId=new Map(graph.nodes.map(node=>[node.uuid,node]))
  ...
  await check('operator spans and owned comments survive indexed lookup, edits and regeneration',()=>{
    ...
    const binary=graph.nodes.find(node=>node.type==='rhai.Binary'&&node.config.fields.operator==='+')
    current.schema.setSyntaxNodeField(binary,'operator','*',graph)
    for(const node of graph.nodes)node.config.structureChanged=true
    const generated=compile(current,graph),result=execute(generated)
    assert.deepEqual(result.logs.map(log=>log.message),['2','ok'])
    for(const comment of ['module','body','left','right','nested'])assert.equal((generated.match(new RegExp('/[/*] '+comment,'g'))??[]).length,1,comment)
  })
  await check('edited and linked source ranges remain current instead of using original spans',()=>{
    const graph=current.syntax.projectRhaiSyntax('fn start(){let x=1;print(x);}'),literal=graph.nodes.find(node=>node.type==='rhai.Literal')
    current.schema.setSyntaxNodeField(literal,'raw','123456789',graph)
    const source='// linked source prefix\n'+compile(current,graph),assessment=current.syntax.assessRhaiConversion(source,'edited',graph),region=assessment.regions.find(region=>region.nodeUuid===literal.uuid)
  ...
  await check('waypoints round-trip without affecting compiler source or actual VM channels',()=>{
    ...
    graph.edges.forEach((edge,index)=>{edge.reroutes=[{x:index+.5,y:-index-.25},{x:1_000_000,y:-1_000_000}]})
    const saved=current.types.serializeGraphDocument(graph),restored=current.types.parseGraphDocument(saved)
    const byId=new Map(restored.edges.map(edge=>[edge.uuid,edge]));for(const edge of graph.edges)assert.deepEqual(byId.get(edge.uuid).reroutes,edge.reroutes)
  ...
  await check('malformed/nonfinite/out-of-range and oversized waypoint lists reject without truncation',()=>{
    const graph=current.syntax.projectRhaiSyntax('fn start(){print(1);}'),edge=graph.edges[0]
    for(const value of [null,{},'route',[null],[{x:'1',y:2}],[{x:NaN,y:2}],[{x:Infinity,y:2}],[{x:1_000_001,y:2}],Array(65).fill({x:0,y:0})]){edge.reroutes=value;assert.throws(()=>current.types.normalizeGraphDocument(graph),/reroute|finite|64/)}
    edge.reroutes=Array.from({length:64},(_,index)=>({x:index,y:-index}));assert.equal(current.types.normalizeGraphDocument(graph).edges[0].reroutes.length,64)
  ...
  await check('aggregate waypoint/node/edge limits reject before normalizing unrelated metadata',()=>{
    const graph=current.syntax.projectRhaiSyntax('fn start(){print(1);}'),point={x:0,y:0},edge=graph.edges[0],nodes=graph.nodes
    const poisoned={...nodes[0],get config(){throw Error('POISON_CONFIG_WAS_READ')}}
    const routed={...graph,nodes:[poisoned],edges:Array.from({length:1563},()=>({...edge,reroutes:Array(64).fill(point)}))}
    assert.throws(()=>current.types.normalizeGraphDocument(routed),/100,000/)
    assert.throws(()=>current.types.normalizeGraphDocument({...graph,nodes:Array(5000).fill(poisoned),routines:[{nodes:Array(5001).fill(poisoned),edges:[]}]}),/total node or edge/)
    assert.throws(()=>current.types.normalizeGraphDocument({...graph,nodes:[poisoned],edges:Array(20001).fill(edge)}),/20,000|20000/)
    const boundary={...graph,edges:[...Array.from({length:1562},()=>({...edge,uuid:current.types.graphUuid(),reroutes:Array(64).fill(point)})),{...edge,uuid:current.types.graphUuid(),reroutes:Array(32).fill(point)}]}
    assert.equal(current.types.normalizeGraphDocument(boundary).edges.reduce((sum,edge)=>sum+edge.reroutes.length,0),100000)
  })
  await check('stale endpoint or pin identities remain invalid even with valid waypoints',()=>{
    const graph=current.syntax.projectRhaiSyntax('fn start(){print(1);}'),edge=graph.edges[0]
  ...
  const array=count=>'['+Array(count).fill('0').join(',')+']'
  const benchmarkSources=[{nodes:100,source:'fn start(){let values='+array(91)+';}'},{nodes:1000,source:'fn start(){let values='+array(955)+';}'},{nodes:10000,source:'fn start(){'+Array.from({length:10},(_,index)=>'let values'+index+'='+array(955)+';').join('')+';'.repeat(25)+'}'}]
  for(const fixture of benchmarkSources)await check('bounded '+fixture.nodes+' node projection and assessment',()=>{
    for(const [profile,modules] of [['baseline',baseline],['current',current]]){
      ...
      assert.equal(graph.nodes.length,fixture.nodes);assert.ok(graph.nodes.every(node=>node.pins.length<=128));assert.ok(fixture.source.length<=64000)
  ...
  console.log(JSON.stringify({status:failures.length?'failed':'passed',checks:checks.length,benchmarks,failed:checks.filter(item=>item.status==='failed'),output},null,2))
