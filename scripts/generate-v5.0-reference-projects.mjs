/** 版本5.0：生成参考项目与对应资源，供功能演示和版本验证使用。 */
...
/** 升级5.0项目与包兼容性，并同步锁文件及内嵌包引擎范围。 */ function refreshProject(project) {
  project.engineVersion = '5.0.0'
...
/** 更新5.0参考身份、发布工程字段和默认团队工作设置。 */ function updateProject(project, slug, title, features) {
  ...
  project.projectSettings.build.releaseEngineering = {
    release: '5.0.0', schema: 29, rhaiApi: 2, pluginApi: 2, packageManifest: 1, buildCli: 1,
...
for (const [slug, source, title, features] of fixtures) {
  ...
  await writeFile(join(directory, 'expected-output.json'), json({
    engineVersion: '5.0.0', schema: 29, projectName: title, expectedValidation: 'pass', features,
  ...
  await writeFile(join(directory, 'test-controls.json'), json({
    engineVersion: '5.0.0', open: 'Project Manager > Open project.nova',
...
for (const entry of entries) {
  ...
  try {
    ...
    await writeFile(readmePath, readme.replace(/Engine \*\*\d+\.\d+\.\d+\*\*/g, 'Engine **5.0.0**'))
    for (const name of ['expected-output.json', 'test-controls.json']) {
      try {
        const path = join(directory, name)
        const document = JSON.parse(await readFile(path, 'utf8'))
        document.engineVersion = '5.0.0'
...
console.log('Generated five Nova_A v5.0 release-system references and refreshed all reference metadata.')
