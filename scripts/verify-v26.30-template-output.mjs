/** 26.30 保留回归：复用 26.23 已审计的 template-output 检查，在当前版本执行并单独记录证据。 */
import assert from 'node:assert/strict'
// Retained 26.20 regression implementation, executed against actual 26.30 source.
...
const release = '26.30', development = process.argv.includes('--development')
if (!development) assert.equal(engineVersion, '26.30.0', 'Template output qualification requires actual 26.30 source')
...
try {
  await viteBuild({
    ...
    build: {
      ...
      outDir: compiled,
      emptyOutDir: true,
      rollupOptions: { input: { templates: join(root, 'src', 'projects', 'templates.ts'), pak: join(root, 'src', 'runtime', 'novaPak.ts') }, output: { entryFileNames: '[name].mjs' } }
    }
  })
  const templates = await import(`${pathToFileURL(join(compiled, 'templates.mjs')).href}?v=${Date.now()}`)
  const {parseNovaPak}=await import(pathToFileURL(join(compiled,'pak.mjs')).href)
  const productionPlayerPath=join(root,'src-tauri/target/release/nova_a.exe'),productionPlayer=await readFile(productionPlayerPath)
  assert.ok(productionPlayer.length>1000000,'A real current native player is required; frozen release build verifies its machine version')
  ...
  for (const descriptor of templates.PROJECT_TEMPLATES) {
    ...
    try {
      const web = await exportProject(projectPath, webOutput, 'web', 'web-es2022-v1', ['--dist', join(root, 'dist')])
      ...
      const decoded=await parseNovaPak(webPack)
      assert.equal(decoded.index.engineVersion,engineVersion)
      assert.ok(decoded.files.has('project.nova'))
      // 解包验证每个实际包含资源的校验和，并比较作者原始数据而非只检查魔数。
      for(const entry of decoded.index.entries){
        if(!entry.assetUuid)continue
        const authored=project.assets.find(/* UUID 定位被打包的作者资源。 */ asset=>asset.uuid===entry.assetUuid)
        assert.ok(authored,'Exported asset has authored identity')
        const source=authored.source??''
        const comma=source.indexOf(',')
        const bytes=source.startsWith('data:')?(source.slice(0,comma).includes(';base64')?Buffer.from(source.slice(comma+1),'base64'):Buffer.from(decodeURIComponent(source.slice(comma+1)))):Buffer.from(source)
        assert.deepEqual(Buffer.from(decoded.files.get(entry.path)),bytes,'Exported bytes '+descriptor.id+' '+entry.path)
      }
      ...
      const native = await exportProject(projectPath, nativeOutput, 'windows', 'windows-x64-v1', ['--single-file', '--player', productionPlayerPath])
      ...
      const nativeOk = native.exportTemplate === 'windows-x64-v1'
        ...
        && executable.subarray(0, productionPlayer.length).equals(productionPlayer)
  ...
  const first = await exportProject(sampleProject, incrementalOutput, 'web', 'web-es2022-v1', ['--dist', join(root, 'dist')])
  const second = await exportProject(sampleProject, incrementalOutput, 'web', 'web-es2022-v1', ['--dist', join(root, 'dist')])
  ...
  let unknownFailure = ''
  try {
    await exportProject(sampleProject, join(workspace, 'unknown-template'), 'web', 'unregistered-output-template', ['--dist', join(root, 'dist')])
  } catch (error) {
  ...
  let reservedFailure = ''
  try {
    await exportProject(reservedProjectPath, join(workspace, 'reserved-package-path'), 'web', 'web-es2022-v1', ['--dist', join(root, 'dist')])
  } catch (error) {
  ...
  await exportProject(cleanProjectPath, cleanOutput, 'web', 'web-es2022-v1', ['--dist', join(root, 'dist')])
...
const report = {
  format: 'nova-v26.30-template-output-verification',
  version: 1,
  release,
  engineVersion,
  development,
  expectedRelease: development ? engineVersion.split('.').slice(0, 2).join('.') : release,
  qualifiedRelease: development ? null : release,
  generatedAt: new Date().toISOString(),
  scope: { launcherTemplates: templateResults.length, webRuntime: 'Actual current production dist copied into each export; rendering is qualified by the separate browser library and export-user suites.', targets: ['web-production-asset-integrity', 'windows-production-player-packaging'], realPlayerLaunch: 'not-executed-by-this-headless-check' },
  checks,
  severity0Open: 0,
...
await writeFile(join(root, 'release-audits', 'v26.30-template-output.json'), `${JSON.stringify(report, null, 2)}\n`)
if (failed.length) { console.error(failed); process.exit(1) }
console.log(`Nova_A 26.30 template output verification passed: ${templateResults.length} templates, ${checks.length} checks.`)
