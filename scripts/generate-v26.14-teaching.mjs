...
const root=dirname(dirname(fileURLToPath(import.meta.url))),release='26.14',engine='26.14.0',escape=value=>value.replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;')
...
for(const [language,sourceLanguage]of [['en','en'],['de','de'],['zh-CN','zh']]){
 ...
 const paragraphs=lesson.trim().split(/\n\s*\n/).map(block=>block.startsWith('# ')?'<h2>26.14 — '+escape(block.slice(2))+'</h2>':'<p>'+escape(block).replaceAll('\n','<br>')+'</p>').join('')
