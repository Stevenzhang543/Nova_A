...
const replaceJsonVersion = source => source.replace(/("version"\s*:\s*")[0-9]+\.[0-9]+\.[0-9]+("\s*,)/, `$1${engineVersion}$2`)
const definitions = [
  ...
  ['Cargo.toml', source => source.replace(/(\[workspace\.package\][\s\S]*?version\s*=\s*")[0-9]+\.[0-9]+\.[0-9]+("\s*)/, `$1${engineVersion}$2`), source => new RegExp(`\\[workspace\\.package\\][\\s\\S]*?version\\s*=\\s*"${engineVersion.replaceAll('.', '\\.')}"`).test(source)],
  ['src-tauri/Cargo.toml', source => source.replace(/(\[package\][\s\S]*?version\s*=\s*")[0-9]+\.[0-9]+\.[0-9]+("\s*)/, `$1${engineVersion}$2`), source => new RegExp(`\\[package\\][\\s\\S]*?version\\s*=\\s*"${engineVersion.replaceAll('.', '\\.')}"`).test(source)],
  ['Cargo.lock', source => source.replace(/(name = "nova_[^"]+"\r?\nversion = ")[0-9]+\.[0-9]+\.[0-9]+("\r?\n)/g, `$1${engineVersion}$2`), source => { const versions = [...source.matchAll(/^name = "nova_[^"]+"\r?\nversion = "([^"]+)"/gm)].map(match => match[1]); return versions.length >= 7 && versions.every(version => version === engineVersion) }],
  ...
  ['src/projects/projectFormat.ts', source => source.replace(/NOVA_ENGINE_VERSION = '[0-9]+\.[0-9]+\.[0-9]+'/, `NOVA_ENGINE_VERSION = '${engineVersion}'`).replace(/NOVA_RELEASE_NAME = '[0-9]{2}\.[0-9]{2}'/, `NOVA_RELEASE_NAME = '${release}'`), source => source.includes(`NOVA_ENGINE_VERSION = '${engineVersion}'`) && source.includes(`NOVA_RELEASE_NAME = '${release}'`)],
  ['crates/nova_format/src/lib.rs', source => source.replace(/CURRENT_ENGINE_VERSION: &str = "[0-9]+\.[0-9]+\.[0-9]+"/, `CURRENT_ENGINE_VERSION: &str = "${engineVersion}"`), source => source.includes(`CURRENT_ENGINE_VERSION: &str = "${engineVersion}"`)],
  ['tests/fixtures/migrations/public-schema-expected.json', source => source.replace(/("targetEngine"\s*:\s*")[0-9]+\.[0-9]+\.[0-9]+("\s*,)/, `$1${engineVersion}$2`), source => JSON.parse(source).targetEngine === engineVersion],
  ['src/i18n.ts', source => source.replace(/Nova_A v[0-9]{2}\.[0-9]{2}/g, `Nova_A v${release}`), source => source.includes(`Nova_A v${release}`)]
]
...
const authorities = {
  ...
  wasm: changes.some(item => item.path === 'nova_core/pkg/package.json') ? JSON.parse(await readFile(join(root, 'nova_core/pkg/package.json'), 'utf8')).version : engineVersion
}
...
for (const [path, , valid] of definitions.filter(([path]) => changes.some(item => item.path === path))) if (!valid(await readFile(join(root, path), 'utf8'))) throw new Error(`Post-commit authority validation failed for ${path}.`)
