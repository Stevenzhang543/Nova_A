/** 26.24 保留工作室行为回归：真实连线、撤销、草稿冲突、窄面板和事件编辑；按当前启动页与固定布局契约执行。 */
...
await withBrowserAudit({release:'26.24',name:'studio-authoring',expectedRelease,development:expectedRelease!=='26.24'},/** 结构说明（自动提取）：withBrowserAudit 回调；输入 audit；直接调用 check；等待异步结果。 */ async audit=>{
  ...
  const setCode=/** 通过真实输入替换编辑器源码，失焦后等待草稿更新。 */ async text=>{await evaluate("(()=>{const e=document.querySelector('.editor-shell textarea');e.focus();e.select()})()");await client.send('Input.insertText',{text});await evaluate("document.querySelector('.editor-shell textarea').blur()");await wait(250)}
  const commands=/** 展开图或代码更多菜单执行指定命令，随后收起菜单。 */ async(label,graph=true)=>{const menu=graph?'.graph-more':'.studio-more';if(!await evaluate(`document.querySelector(${JSON.stringify(menu)}).open`))await click(menu+'>summary');await clickText(menu+' button',label);if(await evaluate(`document.querySelector(${JSON.stringify(menu)}).open`))await click(menu+'>summary')}
  const graphPositions=/** 读取所有渲染图节点的样式位置并按节点标识建立映射。 */ ()=>evaluate("Object.fromEntries([...document.querySelectorAll('.graph-node')].map(e=>[e.dataset.nodeUuid,{x:parseFloat(e.style.left),y:parseFloat(e.style.top)}]))")
  const graphView=/* 调用 evaluate("document.querySelector('.nodes-layer').style.transform") 并返回调用结果。 */ ()=>evaluate("document.querySelector('.nodes-layer').style.transform")
  const showNode=/** 展开图详情并搜索节点种类，选择首项以定位目标节点。 */ async type=>{if(await evaluate("!!document.querySelector('.open-details')"))await click('.open-details');await fill('.graph-symbol-search input',type);await until("document.querySelectorAll('.graph-symbol-search button').length>0");await click('.graph-symbol-search button');await wait(180)}
  const focus=/* 调用 evaluate(`document.querySelector(${JSON.stringify(selector)}).focus({preventScroll:true})`) 并返回调用结果。 */ selector=>evaluate(`document.querySelector(${JSON.stringify(selector)}).focus({preventScroll:true})`)
  const graphUndo=/** 聚焦图编辑器并发送撤销快捷键，等待状态恢复。 */ async()=>{await focus('.graph-editor');await press('z',2);await wait(350)}
  await check('Create a disposable project and source script through real controls',/** 结构说明（自动提取）：check 回调；无显式参数；直接调用 click、until、fill、wait、evaluate 等；等待异步结果。 */ async()=>{
    await click('.quick-actions .new-project');await until("!!document.querySelector('.creation-card')");await fill('.creation-card header input','26.24 Studio Development Audit');await click('[data-template-id="empty"]');await click('.create-button');await until("!!document.querySelector('.editor-root')",30000);await wait(300)
  ...
  await check('Unsaved source survives a real workspace unmount and return',/** 验证未保存源码经过工作区卸载与重新进入仍完整保留。 */ async()=>{
  ...
  await check('Narrow code view retains editor height and restores responsive explorer selection',/** 验证窄窗代码编辑区最小尺寸、资源浏览器切换及键盘调整，宽窗恢复检查器。 */ async()=>{
  ...
  await check('Bottom inspector resizes by keyboard and code focus restores preferred panes',/** 验证底部详情区可通过键盘调整，专注模式退出后恢复尺寸与偏好布局。 */ async()=>{
  ...
  await check('Convert source into typed cards with visible keyboard-accessible pins',/** 验证首次实测图布局无重叠，语法卡标题完整且键盘引脚可见。 */ async()=>{
  ...
  await check('Measured full layout preserves selected identity and viewport and is one Undo operation',/** 结构说明（自动提取）：check 回调；无显式参数；直接调用 audit.point、client.send、wait、graphPositions、graphView 等；等待异步结果。 */ async()=>{
    ...
    await commands('Rearrange including pinned nodes');await until("!document.querySelector('.graph-layout-status')?.textContent.includes('Arranging nodes')");await wait(700)
  ...
  await check('Selected layout leaves every unselected position unchanged',/** 结构说明（自动提取）：check 回调；无显式参数；直接调用 evaluate、audit.point、observations.push、client.send、wait 等；包含循环处理；等待异步结果。 */ async()=>{
    const nodes=await evaluate("[...document.querySelectorAll('.graph-node')].slice(0,2).map(e=>e.dataset.nodeUuid)")
    for(let index=0;index<nodes.length;index++){const point=await audit.point(`[data-node-uuid="${nodes[index]}"] header strong`);observations.push({name:'selection-hit-'+index,expected:nodes[index],point,hit:await evaluate(`document.elementFromPoint(${point.x},${point.y})?.closest('[data-node-uuid]')?.dataset.nodeUuid`)});await client.send('Input.dispatchMouseEvent',{type:'mousePressed',...point,button:'left',clickCount:1,modifiers:index?2:0});await client.send('Input.dispatchMouseEvent',{type:'mouseReleased',...point,button:'left',clickCount:1,modifiers:index?2:0});await wait(120)}
  ...
  await check('Actual keyboard pin activation replaces a compatible structural connection',/** 通过键盘激活引脚重建兼容结构连接，验证边数量不增加且显示连接成功。 */ async()=>{
  ...
  let routeBeforeSave
  await check('Wire route edits affect real paths, reject invalid coordinates and undo precisely',/** 验证手工路由点编辑、撤销、非法坐标拒绝和键盘微调，并保存待核对路由。 */ async()=>{
  ...
  await check('Dirty graph, selection, viewport, route edits and Undo survive workspace unmount',/** 验证未保存图在工作区卸载后保留位置、视口、选择、路由及撤销重做历史。 */ async()=>{
  ...
  await check('Cancel an actual node drag without changing positions or selection',/** 通过真实拖动后按Escape取消，验证全部节点位置恢复。 */ async()=>{
  ...
  await check('Save and reopen graph authoring preserves manual points and regenerates fixed-node routes',/** 保存并往返代码图模式，验证手工转折点持久化及固定节点路由重建为折线路径。 */ async()=>{
    ...
    await until("document.querySelector('.graph-canvas').getAttribute('aria-busy')!=='true'",20000);const paths=await evaluate("[...document.querySelectorAll('.wire-hit')].map(e=>e.getAttribute('d')).filter(Boolean)");assert.ok(paths.length>0);assert.ok(paths.every(/* 返回 path.includes('C') 的逻辑取反结果。 */ path=>!path.includes('C')),paths.join('\n'))
  })
  await check('Narrow graph leaves useful canvas width and retains access to both pane controls',/** 验证窄窗保留可用图画布，工具库可键盘调整且详情仍可访问，外层不横向溢出。 */ async()=>{
  ...
  await check('Mode help remains fully readable through a reachable narrow disclosure',/** 验证窄窗模式帮助弹层具有足够宽度、位于视口内且文本可读。 */ async()=>{
  ...
  await check('Saved-source changes expose a recovered graph conflict and require an explicit decision',/** 结构说明（自动提取）：check 回调；无显式参数；直接调用 viewport、graphView、graphPositions、evaluate、commands 等；等待异步结果。 */ async()=>{
    ...
    await clickText('.workspace-list button','Design');await until("!document.querySelector('.script-workspace')");if(await evaluate("document.querySelector('.compact-tab-select').getBoundingClientRect().width>0"))await audit.select('.compact-tab-select','assets');else await clickText('.bottom-panel .panel-tab','Assets');if(!await evaluate("!!document.querySelector('.bottom-panel .panel-content')"))await click('.panel-controls>button:last-child');await until("!!document.querySelector('.asset-browser')");await clickText('.folder-tree button','Assets/Scripts');await until("[...document.querySelectorAll('.asset-grid article')].some(e=>e.querySelector('small')?.textContent.startsWith('script'))")
  ...
  await check('Event Sheet changes survive workspace unmount and validated Save clears recovery',/** 结构说明（自动提取）：check 回调；无显式参数；直接调用 viewport、click、until、fill、clickText 等；等待异步结果。 */ async()=>{
    ...
    await click('.sheet-toolbar button.primary');await wait(250)
    const savedName=await evaluate("document.querySelector('.sheet-browser>button.active small').textContent")
    await click('.sheet-browser>header button');await until(`document.querySelector('.sheet-browser>button.active small')?.textContent!==${JSON.stringify(savedName)} && !document.querySelector('[role=alertdialog]')`)
    await clickText('.sheet-browser>button',savedName);await until("document.querySelector('.event-copy input')?.value==='Unsaved workspace event'")
    assert.equal(await evaluate("document.querySelector('.callback input').value"),'start')
    await clickText('.workspace-list button','Design');await until("!document.querySelector('.script-workspace')");await clickText('.workspace-list button','Script');await until("!!document.querySelector('.event-studio')");await wait(300);assert.equal(await evaluate("document.querySelector('.event-copy input').value"),'Unsaved workspace event');assert.equal(await evaluate("document.querySelector('.callback input').value"),'start');await capture('events-recovered')
  })
