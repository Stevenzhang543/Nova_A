/** 功能回归脚本：执行 verify-calendar-milestone.mjs 对应场景，保留断言和证据输出。 */
...
const json = /* 调用 JSON.parse(await source(path)) 并返回调用结果。 */ async path => JSON.parse(await source(path))
...
const templateIds = [...templates.matchAll(/\{ id: '([^']+)', category:/g)].map(/* 返回 match[1] 的当前值。 */ match => match[1])
...
if (release === '26.08') {
  ...
  check('V2608-ACTION-PARITY', ['keyboard', 'mouse', 'touch', 'gesture', 'gamepad', 'virtualActions'].every(/* 调用 input.toLowerCase().includes(token.toLowerCase()) 并返回调用结果。 */ token => input.toLowerCase().includes(token.toLowerCase())) && panel.includes('virtual'), 'Keyboard, mouse, touch, gesture, gamepad and virtual controls remain connected to the action path.')
  check('V2608-PEN-LIFECYCLE', ['pressure', 'tiltX', 'tiltY', 'twist', 'eraser', 'pointercancel', 'visibilitychange'].every(/* 调用 `${input}\n${device}\n${modality}`.includes(token) 并返回调用结果。 */ token => `${input}\n${device}\n${modality}`.includes(token)), 'Pen expressiveness and cancellation/focus/page lifecycle release are implemented rather than display-only.')
...
if (release === '26.09') {
  ...
  check('V2609-WORKER-LIFECYCLE', ['lease', 'retireWorker', 'cancel', 'timeout'].every(/* 调用 jobs.includes(token) 并返回调用结果。 */ token => jobs.includes(token)) && worker.includes('lease'), 'Worker jobs use immutable leases and retire cancelled/timed-out workers before reuse.')
  ...
  check('V2609-SEMANTIC-MERGE', ['MAX_MERGE_DEPTH', 'MAX_MERGE_NODES', 'baseFingerprint', 'currentFingerprint', 'validateProjectDocument', 'cursor.splice'].every(/* 调用 workflow.includes(token) 并返回调用结果。 */ token => workflow.includes(token)), 'Semantic collaboration detects stale bases, bounds traversal, preserves deletion semantics and validates the canonical result.')
  check('V2609-REFERENCES', [largeRef, mergeRef].every(/* 比较 JSON.parse(text).engineVersion 与 machine，返回严格相等的判断结果。 */ text => JSON.parse(text).engineVersion === machine), 'Large-world and real semantic-conflict references use the current authority.')
}
...
if (release === '26.10') {
  ...
  check('V2610-REPRESENTATIVE-GAMES', [code, blocks, mixed].every(/* 比较 JSON.parse(text).engineVersion 与 machine，返回严格相等的判断结果。 */ text => JSON.parse(text).engineVersion === machine), 'Code, block and mixed-authoring representative games use the final authority.')
  ...
  const docText = await Promise.all(docs.map(/* 调用 source(`docs/${name}`) 并返回调用结果。 */ name => source(`docs/${name}`)))
  check('V2610-DOCUMENT-SET', docText.every(/* 比较 text.length 与 600，返回大于的判断结果。 */ text => text.length > 600) && docText.every(/* 调用 /pending|external|scope|local/i.test(text) 并返回调用结果。 */ text => /pending|external|scope|local/i.test(text)), 'The final API, support, migration, troubleshooting, reproducibility and qualification set is substantive and honest.', { documents: docs.length })
}
