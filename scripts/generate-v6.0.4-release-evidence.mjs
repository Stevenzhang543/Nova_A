/** 版本6.0.4：汇集发布报告与产物文件，生成带来源记录的发布证据。 */
...
const version='6.0.4',root=dirname(dirname(fileURLToPath(import.meta.url))),audits=join(root,'release-audits'),evidence=join(audits,`evidence-v${version}`),generatedAt=new Date().toISOString(),sha256=/* 调用 createHash('sha256').update(value).digest('hex') 并返回调用结果。 */ value=>createHash('sha256').update(value).digest('hex'),readJson=/* 调用 JSON.parse(await readFile(join(audits,name),'utf8')) 并返回调用结果。 */ async name=>JSON.parse(await readFile(join(audits,name),'utf8')),writeJson=/* 调用 writeFile(path,`${JSON.stringify(value,null,2)}\n`) 并返回调用结果。 */ (path,value)=>writeFile(path,`${JSON.stringify(value,null,2)}\n`)
...
const buildsPassed=builds.every(/* 比较 item.status 与 'passed'，返回严格相等的判断结果。 */ item=>item.status==='passed'),performanceCheck=verification.checks.find(/* 比较 item.id 与 'V604-GRAPH-PERFORMANCE'，返回严格相等的判断结果。 */ item=>item.id==='V604-GRAPH-PERFORMANCE')
