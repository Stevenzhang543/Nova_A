/** 功能回归脚本：执行 verify-v6.0.1-game.mjs 对应场景，保留断言和证据输出。 */
...
try {
  ...
  check('V601-TEMPLATE', templates.PROJECT_TEMPLATES.some(/* 比较 item.id 与 'mouse-knockout'，返回严格相等的判断结果。 */ item => item.id === 'mouse-knockout') && failures.length === 0, 'Launcher exposes a self-auditing Mouse Knockout template.', { failures })
  ...
  const scene = project.scenes[0], assets = project.assets, scripts = assets.filter(/* 比较 asset.assetType 与 'script'，返回严格相等的判断结果。 */ asset => asset.assetType === 'script')
  const scriptDiagnostics = scripts.flatMap(/** 结构说明（自动提取）：scripts.flatMap 回调；输入 asset；直接调用 map、diagnostics.filter、language.analyzeScript；返回表达式求值结果。 */ asset => language.analyzeScript(asset.source, 2).diagnostics.filter(/* 比较 item.severity 与 'error'，返回严格相等的判断结果。 */ item => item.severity === 'error').map(/** 结构说明（自动提取）：map 回调；输入 item；返回表达式求值结果。 */ item => ({ asset: asset.name, code: item.code, message: item.message })))
  ...
  const bodyRecord = /** 结构说明（自动提取）：bodyRecord；输入 handle、position、size、kinematic；直接调用 Float64Array；写入 record[…]；返回路径包含 record。 */ (handle, position, size, kinematic = false) => {
  ...
  physicsPositions.forEach(/** 结构说明（自动提取）：physicsPositions.forEach 回调；输入 position、index；直接调用 physicsRuntime.upsert_body、bodyRecord；返回表达式求值结果。 */ (position, index) => physicsRuntime.upsert_body(index + 1, index, bodyRecord(index + 1, position, index ? [1.25, 1.25] : [1.6, 1.6], index === 0)))
  ...
  for (const asset of scripts) {
    ...
    try {
      ...
      for (const functionName of functionNames) {
        ...
        wasmExecutions.push({ asset: asset.name, functionName, durationMs: performance.now() - startedAt, commands: execution.commands.map(/* 返回 command.type 的当前值。 */ command => command.type) })
  ...
  check('V601-WASM-SCRIPTS', wasmDiagnostics.length === 0 && wasmExecutions.some(/** 结构说明（自动提取）：wasmExecutions.some 回调；输入 run；直接调用 run.commands.filter；返回表达式求值结果。 */ run => run.functionName === 'start' && run.commands.filter(/* 比较 type 与 'spawnAt'，返回严格相等的判断结果。 */ type => type === 'spawnAt').length === 8) && wasmExecutions.some(/** 结构说明（自动提取）：wasmExecutions.some 回调；输入 run；直接调用 every；返回表达式求值结果。 */ run => run.functionName === 'on_timer' && ['targetDestroy', 'scoreAdd', 'targetSetUiText'].every(/* 调用 run.commands.includes(type) 并返回调用结果。 */ type => run.commands.includes(type))), 'The manager compiles and executes start and timer lifecycles through the same optimized WASM Rhai bridge used by Play and exported players.', { diagnostics: wasmDiagnostics, executions: wasmExecutions })
  const combined = scripts.map(/* 返回 asset.source 的当前值。 */ asset => asset.source).join('\n')
  check('V601-GAMEPLAY-CONTRACT', ['view_min_x(', 'view_max_x(', 'spawn_at(', 'timer_start(', 'query_group(', 'entity_destroy(', 'score_add(', 'ui_set_text_on(', 'entity_set_enabled('].every(/* 调用 combined.includes(api) 并返回调用结果。 */ api => combined.includes(api)) && (combined.match(/spawn_at\(/g) ?? []).length === 8, 'Spawn, timer, grouped bounds, score, UI and target-destruction contracts are all bound, with exactly eight authored target spawns.')
  ...
  const prefabAsset = assets.find(/* 比较 asset.name 与 'Knockout Target.nova-prefab'，返回严格相等的判断结果。 */ asset => asset.name === 'Knockout Target.nova-prefab'), prefab = JSON.parse(prefabAsset.source)
  const prefabRoot = prefab.bundle.entities[0], prefabKinds = new Set(prefabRoot.components.map(/* 返回 component.kind 的当前值。 */ component => component.kind))
  check('V601-PREFAB', prefab.prefabVersion === 2 && prefab.bundle.rootUuids.length === 1 && ['Transform2D', 'ShapeRenderer2D', 'RigidBody2D', 'BoxCollider2D'].every(/* 调用 prefabKinds.has(kind) 并返回调用结果。 */ kind => prefabKinds.has(kind)) && !prefabKinds.has('Script2D') && prefabRoot.groups?.includes('knockout-target'), 'Runtime target is a complete, grouped Prefab v2 physics object without redundant per-instance scripts.')
  const names = new Set(scene.entities.map(/* 返回 entity.name 的当前值。 */ entity => entity.name))
  const player = scene.entities.find(/* 比较 entity.name 与 'Mouse Player'，返回严格相等的判断结果。 */ entity => entity.name === 'Mouse Player'), playerKinds = new Set(player?.components?.map(/* 返回 component.kind 的当前值。 */ component => component.kind) ?? [])
  const mouseFollower = player?.components?.find(/* 比较 component.kind 与 'MouseFollower2D'，返回严格相等的判断结果。 */ component => component.kind === 'MouseFollower2D')?.data
  check('V601-SCENE-UI', ['Main Camera', 'Mouse Player', 'Game Manager', 'Game HUD', 'Score Text', 'Instruction Text', 'Congratulations Bar', 'Congratulations Text'].every(/* 调用 names.has(name) 并返回调用结果。 */ name => names.has(name)) && playerKinds.has('MouseFollower2D') && !playerKinds.has('Script2D') && mouseFollower?.maximumSpeed === 40 && scene.globalSettings.gravity === 0 && scene.globalSettings.tickRate === 60, 'Scene, physics-safe native fixed-step pointer component, physics, HUD and completion objects are complete.')
  ...
  const articleCounts = Object.fromEntries(['en', 'de', 'zh-CN'].map(/* 返回按声明顺序构造的数组 [locale, (manual.match(new RegExp(`<article data-lang="${locale}"`, 'g')) ?? []).length]。 */ locale => [locale, (manual.match(new RegExp(`<article data-lang="${locale}"`, 'g')) ?? []).length]))
  check('V601-MANUAL-LOCALES', Object.values(articleCounts).every(/* 比较 count 与 3，返回严格相等的判断结果。 */ count => count === 3) && manual.includes('const articles=()=>') && !manual.includes("const articles=[...document.querySelectorAll('article[data-lang]')]") && manual.indexOf('NOVA_V601_MOUSE_KNOCKOUT_START') < manual.indexOf('</main>') && manual.indexOf('NOVA_V6_TEACHING_START') < manual.indexOf('</main>'), 'HTML owns every old, task and reference article dynamically; generated content is inside the document before the controller runs.', { articleCounts })
  check('V601-MANUAL-TASK', ['en-v601-build', 'de-v601-build', 'zh-CN-v601-build', 'MouseFollower2D', 'Build &amp; Run', '构建并运行'].every(/* 调用 manual.includes(marker) 并返回调用结果。 */ marker => manual.includes(marker)), 'All locales contain the authored create→draw→component→code→configure→play→build tutorial and API examples.')
