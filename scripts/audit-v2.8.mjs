...
assert(packages.includes("version: '2.9.0'") && packages.includes('novaa.networking') && packages.includes("engineVersion = '3.0.0'"), 'official optional networking package is not versioned or v3 compatible')
...
assert(packageJson.includes('"version": "3.0.0"') && packageJson.includes('audit:v2.8'), 'package metadata/audit chain does not preserve v2.8')

for (const manual of [manualEn, manualDe, manualZh]) for (const topic of ['3.0.0', 'Production Lab', 'Replay', 'Networking']) {
  assert(manual.includes(topic), `localized manual lacks ${topic}`)
}
assert(manualHtml.includes('<title>Nova_A 3.0 Manual</title>') && manualHtml.includes('data-section="production"'), 'bookmarkable HTML manual lacks Production Lab documentation')
assert(readme.includes('3.0.0') && readmeZh.includes('3.0.0'), 'release README metadata is stale')
