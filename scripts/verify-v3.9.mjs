/** 功能回归脚本：执行 verify-v3.9.mjs 对应场景，保留断言和证据输出。 */
...
const write = /* 调用 writeFile(join(output, `v3.9.0-${name}.json`), `${JSON.stringify(value, null, 2)}\n`) 并返回调用结果。 */ (name, value) => writeFile(join(output, `v3.9.0-${name}.json`), `${JSON.stringify(value, null, 2)}\n`)
try {
  const [packageModule, buildModule, platformModule, teamModule, networkModule] = await Promise.all(['/src/runtime/packages.ts','/src/runtime/buildSettings.ts','/src/runtime/platformSupport.ts','/src/runtime/teamWorkflow.ts','/src/runtime/networking.ts'].map(/* 调用 server.ssrLoadModule(path) 并返回调用结果。 */ path => server.ssrLoadModule(path)))
  ...
  const installedItem = packageModule.packageState.installed.find(/* 比较 item.manifest.id 与 packageModule.OFFICIAL_OBJECT_POOL_PACKAGE_ID，返回严格相等的判断结果。 */ item => item.manifest.id === packageModule.OFFICIAL_OBJECT_POOL_PACKAGE_ID)
  ...
  const circularDependencyBlocked = !packageModule.applyPackageUpdate(cycleA.id) && packageModule.packageState.errors.some(/* 调用 message.includes('Circular package dependency') 并返回调用结果。 */ message => message.includes('Circular package dependency'))
  ...
  await write('lockfile-tests', { format: 'nova-lockfile-tests', version: 1, engineVersion: '3.9.0', generatedAt, online: JSON.parse(lockA), offline: JSON.parse(lockB), byteIdentical: lockA === lockB, dependencyHashesPresent: JSON.parse(lockA).every(/* 先计算 entry.sha256 && entry.signature；仅当其为真值时求右侧 entry.dependencies，返回短路求值结果。 */ entry => entry.sha256 && entry.signature && entry.dependencies), status: lockA === lockB ? 'passed' : 'failed' })
  ...
  const platformStatus = platformModule.PLATFORM_SUPPORT_MATRIX.find(/* 比较 item.id 与 'windows'，返回严格相等的判断结果。 */ item => item.id === 'windows')?.tier === 'tier-1' && platformModule.PLATFORM_SUPPORT_MATRIX.find(/* 比较 item.id 与 'web'，返回严格相等的判断结果。 */ item => item.id === 'web')?.tier === 'tier-1' && issues.some(/* 比较 item.code 与 'platform-unsupported'，返回严格相等的判断结果。 */ item => item.code === 'platform-unsupported')
  ...
  await write('automation-package-collaboration-tests', { format: 'nova-v3.9-integration-tests', version: 1, engineVersion: '3.9.0', generatedAt, build: { cacheMode: normalized.delivery.cacheMode, reports: normalized.delivery.sizeReport && normalized.delivery.dependencyReport, unsupportedMobileBlocked: issues.some(/* 比较 item.code 与 'platform-unsupported'，返回严格相等的判断结果。 */ item => item.code === 'platform-unsupported') }, packageSecurity: securityStatus, sourceControl: canonicalA === canonicalB, networkingGate: networkModule.NETWORKING_PACKAGE_GATE, status })
