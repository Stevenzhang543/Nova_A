...
/** 将参考项目固定为4.6格式，补齐资源数据库、交付设置和缺失导入散列元数据。 */ function normalizeProjectAssets(project) {
  project.engineVersion = '4.6.0'
...
/** 写出原生项目、期望输出、操作步骤与说明文档，并统一参考项目版本标记。 */ async function writeProjectBundle(slug, project, demonstrates) {
  ...
  await writeFile(join(directory, 'expected-output.json'), `${JSON.stringify({ engineVersion: '4.6.0', schema: 29, projectName: project.projectMetadata?.name ?? project.name ?? slug, minimumScenes: project.scenes?.length ?? 0, minimumEntities: entityCount, expectedValidation: slug === 'data-foundation-validation' ? 'repair-required' : 'pass', demonstrates }, null, 2)}\n`, 'utf8')
  ...
  const requiredPackages = project.packages?.installed?.filter(/* 返回 item.enabled 的当前值。 */ item => item.enabled).map(/** 将包清单标识和版本拼接为依赖说明。 */ item => `${item.manifest.id}@${item.manifest.version}`) ?? []
  ...
  await writeFile(readmePath, (await readFile(readmePath, 'utf8')).replace(/Engine \*\*4\.[345]\.0\*\*/, 'Engine **4.6.0**'), 'utf8')
}
...
try {
  ...
  for (const [slug, template, features] of physicsReferences) {
    const title = slug.split('-').map(/* 计算表达式 word[0].toUpperCase() + word.slice(1) 并返回结果，沿用操作数的原有类型规则。 */ word => word[0].toUpperCase() + word.slice(1)).join(' ')
...
fn on_signal(name, payload, source) { if name == "go" { navigation_set_target(4.0, 2.0); } }`, ['API v1 handles', 'input, physics, animation, audio, navigation, save and resources']]
  ...
  for (const [slug, title, source, features] of scriptReferences) {
    ...
    const script = (project.assets ?? []).find(/* 比较 asset.assetType 与 'script'，返回严格相等的判断结果。 */ asset => asset.assetType === 'script')
...
const stableUuid = /** 由种子散列生成确定性的参考对象标识。 */ seed => {
  const hex = createHash('sha256').update(seed).digest('hex').slice(0, 32)
  return `${hex.slice(0, 8)}-${hex.slice(8, 12)}-4${hex.slice(13, 16)}-8${hex.slice(17, 20)}-${hex.slice(20)}`
}
const authoringComponent = /** 创建带稳定标识、启用状态和数据的参考组件记录。 */ (seed, kind, data = {}) => ({ uuid: stableUuid(`${seed}:${kind}`), kind, enabled: true, removed: false, data })
const authoredEntity = /** 按对象种类构造变换及摄像机、文本、精灵或形状组件，附作者和预制体元数据。 */ (seed, kind, x, y, options = {}) => {
...
const authoringProject = /** 克隆基础项目并设置作者示例的版本、兼容性、场景实体和资源。 */ (slug, name, entities, assets = []) => {
...
const pixelEntities = [authoredEntity('pixel-camera', 'Camera', 0, 0, { pixelPerfect: true, orthographicSize: 6 }), ...Array.from({ length: 24 }, /** 按网格放置使用最近邻过滤的像素精灵示例。 */ (_, index) => authoredEntity(`pixel-${index}`, 'Sprite', index % 8 - 3.5, Math.floor(index / 8) - 1, { spriteAsset: `asset://${pixelAssetUuid}`, filterMode: 'Nearest', size: { x: 1, y: 1 } }))]
...
const resolutionEntities = [authoredEntity('resolution-camera', 'Camera', 0, 0, { orthographicSize: 10 }), ...Array.from({ length: 15 }, /** 按网格生成文本与缩放矩形用于分辨率示例。 */ (_, index) => authoredEntity(`resolution-${index}`, index % 3 === 0 ? 'WorldText' : 'Rectangle', (index % 5 - 2) * 3, (Math.floor(index / 5) - 1) * 3, { scale: { x: 2, y: 1 }, color: { r: 92 + index * 6, g: 145, b: 225 } }))]
...
const parallaxEntities = [authoredEntity('parallax-camera', 'Camera', 0, 0, { orthographicSize: 8 }), parallaxRoot, ...Array.from({ length: 30 }, /** 生成隶属于视差根的分层色块。 */ (_, index) => authoredEntity(`parallax-${index}`, 'Rectangle', (index % 10 - 5) * 4, Math.floor(index / 10) * 3 - 4, { parentUuid: parallaxRoot.uuid, color: { r: 65 + index * 3, g: 100 + index * 2, b: 170 + index } }))]
...
const nestedEntities = [authoredEntity('nested-camera', 'Camera', 0, 0), nestedRoot, ...Array.from({ length: 8 }, /** 生成具有父级和预制体来源身份的嵌套实例子对象。 */ (_, index) => authoredEntity(`nested-child-${index}`, index % 2 ? 'Sprite' : 'Rectangle', index - 4, -1, { parentUuid: nestedRoot.uuid, prefabAsset: 'asset://00000000-0000-4000-8000-000000000102', prefabInstanceUuid: stableUuid('instance:wheel'), prefabSourceUuid: stableUuid(`source:wheel:${index}`) }))]
...
const defaultReferenceSettings = /** 提供参考资源的纹理、音频及字体默认导入设置。 */ () => ({
...
const referenceAsset = /** 创建带稳定标识、源字节长度及合并导入设置的参考资源。 */ (seed, name, path, assetType, mimeType, source, settings = {}) => ({
...
const addComponent = /** 向实体追加作者组件并返回同一实体。 */ (entity, seed, kind, data) => { entity.components.push(authoringComponent(seed, kind, data)); return entity }
...
const lightingEntities = [
  ...
  ...Array.from({ length: 7 }, /** 生成具有阴影投射组件的灯光示例矩形。 */ (_, index) => addComponent(authoredEntity(`lighting-caster-${index}`, 'Rectangle', index * 2 - 6, index % 2 ? 1 : -1, { scale: { x: 1.2, y: 2.4 }, color: { r: 70 + index * 14, g: 105 + index * 8, b: 175 + index * 6 } }), `lighting-caster-${index}`, 'ShadowCaster2D', { layerMask: 0xffffffff, selfShadows: false, opacity: .85 }))
]
...
const materialEntities = [authoredEntity('material-camera', 'Camera', 0, 0, { orthographicSize: 7 }), ...Array.from({ length: 9 }, /** 按网格创建材质展示形状。 */ (_, index) => authoredEntity(`material-shape-${index}`, 'Rectangle', index % 3 * 3 - 3, Math.floor(index / 3) * 2.4 - 2.4, { color: { r: 110, g: 170, b: 255 }, scale: { x: 2.2, y: 1.5 } }))]
for (const entity of materialEntities) { const renderer = entity.components.find(/* 比较 component.kind 与 'ShapeRenderer2D'，返回严格相等的判断结果。 */ component => component.kind === 'ShapeRenderer2D'); if (renderer) renderer.data.material = materialReference }
...
textureCamera.components.find(/* 比较 component.kind 与 'Camera2D'，返回严格相等的判断结果。 */ component => component.kind === 'Camera2D').data.renderTexture = 'preview-target'
const textureProject = authoringProject('rendering-render-textures', 'Render Textures and Cameras', [textureCamera, authoredEntity('render-texture-composite', 'Camera', 5, 0, { name: 'Composite camera', orthographicSize: 9, priority: 1 }), ...Array.from({ length: 20 }, /** 按网格生成渲染目标摄像机的拍摄对象。 */ (_, index) => authoredEntity(`render-target-subject-${index}`, 'Rectangle', index % 5 * 2 - 4, Math.floor(index / 5) * 2 - 3, { color: { r: 75 + index * 5, g: 135, b: 220 } }))])
...
for (const entity of fontEntities) { const renderer = entity.components.find(/* 比较 component.kind 与 'TextRenderer2D'，返回严格相等的判断结果。 */ component => component.kind === 'TextRenderer2D'); if (renderer) { renderer.data.fontAsset = `asset://${fontAsset.uuid}`; renderer.data.fontSize = 1.25; renderer.data.maxWidth = 16 } }
...
/** 生成带渐入渐出包络的单声道十六位正弦波 WAV 数据地址。 */ function waveDataUrl(frequency = 440, seconds = .35, sampleRate = 22050) {
...
const audioSourceData = /** 生成空间音源默认播放、衰减和声部配置，并应用显式覆盖。 */ (clip, overrides = {}) => ({ audioClip: clip, volume: .7, pitch: 1, loop: true, autoplay: true, spatialBlend: 1, minDistance: 1, maxDistance: 14, bus: 'SFX', attenuationCurve: 'Inverse', customAttenuation: [{ distance: 0, gain: 1 }, { distance: 1, gain: 0 }], voicePriority: 50, polyphony: 4, randomPitch: .05, randomVolume: .08, virtualizeWhenLimited: true, streamOverride: 'ImportSetting', ...overrides })
...
const positionalEntities = [listenerCamera, ...[-6, 0, 6].map(/** 为指定位置对象添加使用示例音频及不同音高的空间音源。 */ (x, index) => addComponent(authoredEntity(`audio-source-${index}`, 'Rectangle', x, index % 2 ? 3 : -2, { name: `Spatial source ${index + 1}`, color: { r: 90, g: 165 + index * 20, b: 255 } }), `audio-source-${index}`, 'AudioSource', audioSourceData(`asset://${toneAsset.uuid}`, { pitch: .8 + index * .2 })))]
...
effectsProject.projectSettings.audio.mixer.buses.find(/* 比较 bus.id 与 'SFX'，返回严格相等的判断结果。 */ bus => bus.id === 'SFX').effects = [{ id: 'compressor', kind: 'Compressor', enabled: true, wet: 1, frequency: 1200, q: .7, threshold: -18, ratio: 4, time: .1, feedback: .2 }, { id: 'delay', kind: 'Delay', enabled: true, wet: .2, frequency: 1200, q: .7, threshold: -24, ratio: 4, time: .16, feedback: .22 }]
...
const releaseBuildSettings = /** 按目标平台生成参考发布构建及确定性交付配置。 */ target => ({
