/* 将公开操作的 AST 定义与已通过的具名用例关联，保留未覆盖操作及完整输入域覆盖的边界。 */
...
/* 遍历具名测试并要求报告中已有通过证据，再收集其调用的公开操作。 */
function scanTests(node){
 if(ts.isCallExpression(node)&&ts.isIdentifier(node.expression)&&node.expression.text==='test'&&ts.isStringLiteral(node.arguments[0])){
  const name=node.arguments[0].text;if(!report.checks.some(/* 先计算 check.name===name；仅当其为真值时求右侧 check.status==='passed'，返回短路求值结果。 */ check=>check.name===name&&check.status==='passed'))throw Error('Unexecuted case: '+name)
  /* 递归识别测试中模块别名的成员调用，将操作映射到具名测试与报告并去重。 */
  function calls(child){if(ts.isCallExpression(child)&&ts.isPropertyAccessExpression(child.expression)){const receiver=child.expression.expression;const alias=ts.isIdentifier(receiver)?receiver.text:ts.isPropertyAccessExpression(receiver)&&receiver.expression.getText(suite)==='opened.modules'?receiver.name.text:'';const source=aliases[alias];if(source){const id=source+'#'+child.expression.name.text,list=tested.get(id)||[];if(!list.some(/* 比较 item.case 与 name，返回严格相等的判断结果。 */ item=>item.case===name))list.push({suite:suitePath,report:'release-audits/v26.22-document-boundaries.json',case:name});tested.set(id,list)}}ts.forEachChild(child,calls)}calls(node.arguments[1])
...
for (const spec of [
 ...
 /* 识别指定测试入口的已执行用例，递归收集其函数、类成员和构造调用证据。 */
 function visit(node){if(ts.isCallExpression(node)&&ts.isIdentifier(node.expression)&&node.expression.text===spec.test&&ts.isStringLiteral(node.arguments[0])){
  const name=node.arguments[0].text;if(!evidence.checks.some(/* 先计算 row.name===name；仅当其为真值时求右侧 row.status==='passed'，返回短路求值结果。 */ row=>row.name===name&&row.status==='passed'))throw Error('Missing executed case '+name)
  const add=/* 将当前具名用例登记到公开操作映射，按套件与用例名称去重。 */ operation=>{const id=spec.source+'#'+operation,list=tested.get(id)||[];if(!list.some(/* 先计算 row.case===name；仅当其为真值时求右侧 row.suite===spec.suite，返回短路求值结果。 */ row=>row.case===name&&row.suite===spec.suite))list.push({suite:spec.suite,report:spec.report,case:name});tested.set(id,list)}
  /* 遍历测试语法节点，按配置识别函数调用、类成员及构造调用并登记操作。 */
...
if(eventEvidence.status!=='passed'||!eventEvidence.checks.length||eventEvidence.checks.some(/* 比较 check.status 与 'passed'，返回严格不等的判断结果。 */ check=>check.status!=='passed'))throw Error('Unpassed event-save evidence');
...
for(const path of paths){
 ...
 const exported=/* 判断声明是否显式导出，或其名称是否存在于本地导出集合中。 */ node=>node.modifiers?.some(/* 比较 mod.kind 与 ts.SyntaxKind.ExportKeyword，返回严格相等的判断结果。 */ mod=>mod.kind===ts.SyntaxKind.ExportKeyword)||node.name&&localExports.has(node.name.getText(source));
 const add=/* 登记公开操作的源码位置、种类和关联用例，并明确已有用例不代表完整输入域覆盖。 */ (name,node,kind)=>{const id=path+'#'+name,cases=tested.get(id)||[];operations.push({id,source:path,line:source.getLineAndCharacterOfPosition(node.getStart(source)).line+1,operation:name,kind,disposition:cases.length?'Executed named regression cases; full input-domain coverage is not implied.':'Pending named case or explicit non-authored exclusion',cases})}
 for(const node of source.statements){
  if(ts.isFunctionDeclaration(node)&&node.body&&exported(node))add(node.name?.text||'default',node,'exported function')
  if(ts.isClassDeclaration(node)&&exported(node))for(const member of node.members){if(member.modifiers?.some(/* 先计算 m.kind===ts.SyntaxKind.PrivateKeyword；仅当其为假值时求右侧 m.kind===ts.SyntaxKind.ProtectedKeyword，返回短路求值结果。 */ m=>m.kind===ts.SyntaxKind.PrivateKeyword||m.kind===ts.SyntaxKind.ProtectedKeyword))continue;if((ts.isMethodDeclaration(member)||ts.isGetAccessorDeclaration(member)||ts.isSetAccessorDeclaration(member)||ts.isConstructorDeclaration(member))&&member.body)add((node.name?.text||'default')+'.'+(ts.isConstructorDeclaration(member)?'constructor':member.name.getText(source)),member,ts.isGetAccessorDeclaration(member)?'getter':ts.isSetAccessorDeclaration(member)?'setter':'public class operation')}
