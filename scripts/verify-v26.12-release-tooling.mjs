...
const rejects = async (callback, pattern) => { await assert.rejects(callback, pattern); checks++ }
const put = async (root, path, contents) => { await mkdir(dirname(join(root, path)), { recursive: true }); await writeFile(join(root, path), contents) }
async function fixture(root) {
  ...
  const files = {
    ...
    'Cargo.lock': [...Array.from({ length: 7 }, (_, index) => `[[package]]\nname = "nova_${index}"\nversion = "${version}"\n`), `[[package]]\nname = "third_party"\nversion = "${version}"\n`].join('\n'),
...
function runSetter(root, release) { return execFileSync(process.execPath, [join(root, 'scripts/set-calendar-release.mjs'), `--release=${release}`], { encoding: 'utf8', windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] }) }
try {
  ...
  for (const release of ['26.00', '26.100', '26.1', '25.12', '../26.12', '26.12.0']) { assert.throws(() => releaseVersion(release)); checks++ }
  ...
  for (const release of ['26.12', '26.13', '26.14', '26.15', '26.16']) {
    ...
    check(Boolean(generated.gates.find(gate => gate.id === 'focus').blockedReason) === (release !== '26.12'), `Future focus remains blocked until its own implementation exists: ${release}`)
  }
  const pinnedPlan = generateReleasePlan('26.12', { node: 'C:/fixture/node.exe', pnpmEntry: 'C:/fixture/pnpm.cjs', pnpmBin: 'C:/fixture/bin' })
  check(pinnedPlan.gates.every(gate => gate.command.file === 'C:/fixture/node.exe') && !pinnedPlan.gates.some(gate => gate.blockedReason), 'Provided pinned tools produce an executable plan without running it')
  check(pinnedPlan.gates.find(gate => gate.id === 'stability').context.includes('not 1000 real editor'), 'Legacy synthetic stability scope is explicit in the actual plan')
  check(FOCUS_26_12.some(([script, args, report]) => script === 'scripts/verify-v26.12-api-signatures.mjs' && args.includes('--report=release-audits/v26.12-api-signatures.json') && report === 'release-audits/v26.12-api-signatures.json'), 'Current API overload execution is a mandatory recorded focus suite')
  check(FOCUS_26_12.some(([script, args, report]) => script === 'scripts/verify-v26.11-visual-roundtrip.mjs' && args.some(argument => argument.startsWith('--runtime=target/debug/examples/nova_script_test')) && report === 'release-audits/v26.11-visual-roundtrip.json'), 'Retained legacy graph focus requires actual native execution')
  ...
  assert.throws(() => summarizeProductEvidence([{ path: 'failed-gate.json', report: { status: 'failed' } }]), /missing or failed/); checks++
  ...
  assert.throws(() => runSetter(root, '26.13')); checks++
  ...
  const plan = { format: 'nova-release-qualification-plan', version: 1, release: '26.12', machineVersion: '26.12.0', releaseNotes: 'docs/notes.md', editLedger: 'docs/ledger.md', documentation: ['docs/notes.md'], gates: requiredGateIds.map(id => ({ id, category: userIds.has(id) ? 'user' : 'programmer', context: 'Disposable verifier fixture; no real UI or production release qualified.', command: { file: process.execPath, args: ['fixture-gate.mjs', id] }, reports: reportTargets[id] ? [{ path: `release-audits/${id}.json`, target: reportTargets[id], format: 'nova-release-test-fixture', version: 1, requireRelease: true, requireEngine: true }] : [], artifacts: id === 'native-build' ? artifactNames.map((name, index) => ({ name, path: artifactPaths[index] })) : [] })) }
  ...
  check((await releaseSourceInventory(snapshot.source)).some(file => file.path === 'src/untracked-new.ts'), 'Snapshot retains untracked authored source')
  check(!(await releaseSourceInventory(snapshot.source)).some(file => excludedReleaseSource(file.path)), 'Snapshot excludes generated/private/cache files')
  await rejects(() => createReleaseSnapshot(root, '26.12'), /Immutable release snapshot already exists/)
  await verifyReleaseSnapshot(snapshot.manifest, root); checks++
  const invalidPlan = structuredClone(plan); invalidPlan.gates.shift(); assert.throws(() => validateQualificationPlan(invalidPlan), /omits required gates/); checks++
  const invalidUser = structuredClone(plan); invalidUser.gates.find(gate => gate.id === 'windows').category = 'programmer'; assert.throws(() => validateQualificationPlan(invalidUser), /category/); checks++
  ...
  await rejects(() => verifyExecutedGates(root, runRoot), /Gate log changed/); await writeFile(join(runRoot, 'focus.log'), log)
  await put(root, 'dist/new-unqualified.js', 'modified'); await rejects(() => verifyExecutedGates(root, runRoot), /web assets changed/); await rm(join(root, 'dist/new-unqualified.js'))
  const artifact = await readFile(join(root, artifactPaths[2])); await put(root, artifactPaths[2], 'replaced native fixture'); await rejects(() => verifyExecutedGates(root, runRoot), /Qualified artifact changed/); await put(root, artifactPaths[2], artifact)
  ...
  await put(root, 'dist/unqualified.js', 'unexpected web payload'); await rejects(() => verifyPackagedQualification(generated.directory, snapshot.source, join(root, 'dist')), /Unqualified web asset/); await rm(join(root, 'dist/unqualified.js'))
  await rejects(() => generateSequentialReleaseEvidence(root, runRoot), /Immutable qualified evidence already exists/)
  ...
  await rejects(() => generateSequentialReleaseEvidence(root, stopped.runRoot), /incomplete/)
  await put(root, 'src/untracked-new.ts', 'changed after freeze'); await rejects(() => verifyReleaseSnapshot(snapshot.manifest, root), /Source changed/)
  ...
  const manifestRead = packager.split(/\r?\n/).find(line => /Get-Content -LiteralPath \$snapshotPath -Raw \| ConvertFrom-Json/.test(line))
  const manifestCheck = packager.split(/\r?\n/).find(line => line.includes("throw 'Frozen source identifies a different release.'"))
