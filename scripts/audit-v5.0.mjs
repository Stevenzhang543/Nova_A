/* 审计 5.0 的构建导出、包生态、协作健康、发布管线和多语言手册契约。 */
...
check('REL-PIPELINE', ['NOVA_RELEASE_PIPELINE','references:v5.0','audit:v5.0','verify:v5.0','evidence:v5.0','release:v5.0'].every(/* 调用 all.includes(value) 并返回调用结果。 */ value => all.includes(value)), 'The release pipeline and package scripts are declared.')
...
check('I18N-V50', ['en','de','zh'].every(/* 调用 sources['src/i18n.ts'].includes("releaseLabel:'Nova_A v5.0.0'") 并返回调用结果。 */ language => sources['src/i18n.ts'].includes("releaseLabel:'Nova_A v5.0.0'")), 'The v5.0 vocabulary is present in English, German and Chinese.')
