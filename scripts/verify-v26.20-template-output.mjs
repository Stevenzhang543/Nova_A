/** 验证脚本（v26.20-template-output）：组织对应功能与边界场景检查，断言行为并汇总验证结果。 */
...
const report = {
  format: 'nova-v26.20-template-output-verification',
  version: 1,
  release,
  engineVersion,
...
await writeFile(join(root, 'release-audits', 'v26.20-template-output.json'), `${JSON.stringify(report, null, 2)}\n`)
if (failed.length) { console.error(failed); process.exit(1) }
console.log(`Nova_A 26.20 template output verification passed: ${templateResults.length} templates, ${checks.length} checks.`)
