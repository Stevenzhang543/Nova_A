...
const context=resolveMilestoneAuditContext(import.meta.url,{release:'26.21',reportName:name})
...
await runMilestoneBrowserAudit(context,{name,height:1000},async a=>{
 ...
 const team=async()=>{await u.workspace('Manage');await a.click('.manage-body>nav button',6);await a.until("!!document.querySelector('.build-panel')");await a.click('.build-header nav button',4);await a.until("!!document.querySelector('.team-workflow')");if(!await a.evaluate("document.querySelector('.workflow-toggle input').checked"))await a.click('.workflow-toggle input')}
 await a.check('Make a real local edit and import a conflicting incoming project',async()=>{
  await u.entity('Checkpoint 1');await u.expandInspector();await u.field('[data-property-path="Transform.position"] input',-4)
  const incoming=JSON.parse(await readFile(join(context.repository,'reference-projects/projects/delivery-v2619-semantic-merge/project.nova'),'utf8'));incoming.scenes[0].entities.find(e=>e.name==='Checkpoint 1').components.find(c=>c.kind==='Transform2D').data.position.x=-2
  const path=join(a.evidence,'v26.21-incoming-conflict.nova');await writeFile(path,JSON.stringify(incoming));a.observations.push({name:'authored-incoming-conflict-fixture',file:path})
  await team();let chooser;const off=a.client.on('Page.fileChooserOpened',value=>chooser=value);await a.client.send('Page.setInterceptFileChooserDialog',{enabled:true});await u.activate('.incoming-picker button');for(let n=0;n<100&&!chooser;n++)await wait(50);assert.ok(chooser);await a.client.send('DOM.setFileInputFiles',{files:[path],backendNodeId:chooser.backendNodeId});off();await a.until("!!document.querySelector('.merge-conflict19')");console.log(await a.evaluate("JSON.stringify([...document.querySelectorAll('.team-grid,.changes-card,.semantic-merge')].map(e=>({class:e.className,height:e.clientHeight,scroll:e.scrollHeight,style:['height','minHeight','gridTemplateRows','gridAutoRows','overflow','contain'].map(k=>[k,getComputedStyle(e)[k]])})))"));await a.evaluate("document.querySelector('.merge-conflict19').scrollIntoView({block:'center'})");await a.capture('conflict-preview');a.observations.push({name:'actual-conflict-preview',text:await a.evaluate("document.querySelector('.semantic-merge').innerText")})
 })
 if(layout){
  const violations=[]
  await a.check('Team conflict cards remain contained and readable across locales themes scales and widths',async()=>{
   for(const locale of ['en','de','zh'])for(const scale of [1,1.5,2])for(const theme of [0,1])for(const width of [720,1280,1920]){
    ...
    if(metrics.navigationOverflow.length||metrics.clipped.length||metrics.cards.some(c=>c.scroll>c.height+3||c.scrollWidth>c.width+3)||!overlap.reachable||!overlap.focused)violations.push({state,...metrics,overlap})
    a.observations.push({name:state,...metrics,overlap});await a.capture(state);console.log('SURFACE '+state)
   }
   assert.deepEqual(violations,[])
  });return
 }
 await a.check('Choose incoming value, apply, undo and redo through visible editor controls',async()=>{
 ...
 await a.check('Save and reopen the exact merged document',async()=>{
  ...
  const file=join(a.evidence,'v26.21-merged-authored.nova');await writeFile(file,bytes);a.observations.push({name:'actual-merged-download',file,bytes:bytes.length});saved={file,document:JSON.parse(bytes)};await u.open(file);await u.entity('Checkpoint 1');await u.expandInspector();assert.equal((await u.position())[0],-2);await a.capture('merged-reopened')
 })
 await a.check('Export the merged game and collect the checkpoint at its changed position',async()=>{
  const exported=await exportDeliveryUser20(a,u,'merged',saved),server=fork(fileURLToPath(new URL('./lib/worldExportServer17.mjs',import.meta.url)),[exported.output],{stdio:['ignore','ignore','pipe','ipc'],windowsHide:true});let error='';server.stderr.on('data',data=>error+=data)
  try{const port=await new Promise((done,reject)=>{const timer=setTimeout(()=>reject(Error('Player server timed out '+error)),15000);server.once('message',value=>{clearTimeout(timer);done(value.port)});server.once('error',reject)});await a.client.send('Page.navigate',{url:'http://127.0.0.1:'+port+'/'});await a.until("!!document.querySelector('.player-root .render-canvas')",30000);await a.until("[...document.querySelectorAll('[data-game-ui-control]')].some(e=>e.getAttribute('aria-label')?.includes('Checkpoint 1/6'))",20000);await a.click('.overlay-canvas');await collectVisibleCheckpoint(a);await a.until("[...document.querySelectorAll('[data-game-ui-control]')].some(e=>e.getAttribute('aria-label')?.includes('Checkpoint 2/6'))",5000);await a.capture('merged-export-checkpoint-collected');a.observations.push({name:'exported-gameplay-at-edited-position',text:await a.evaluate("[...document.querySelectorAll('[data-game-ui-control]')].map(e=>e.getAttribute('aria-label')).join(' | ')")});await a.press('r');await a.until("[...document.querySelectorAll('[data-game-ui-control]')].some(e=>e.getAttribute('aria-label')?.includes('Checkpoint 1/6'))",5000)}finally{server.disconnect();await new Promise(done=>server.once('exit',done))}
 })
