/** 功能回归脚本：执行 verify-v26.21-delivery-user.mjs 对应场景，保留断言和证据输出。 */
...
await runMilestoneBrowserAudit(context,{name,height:1000},/** 在当前版本通过真实界面验证语义合并、布局、撤销重做、保存和导出玩法。 */ async a=>{
 ...
 await a.check('Make a real local edit and import a conflicting incoming project',/** 制作本地与传入项目的位置冲突，经文件选择器导入并记录当前版本合并预览。 */ async()=>{
 ...
 await a.check('Save and reopen the exact merged document',/** 结构说明（自动提取）：a.check 回调；无显式参数；直接调用 join、mkdir、a.client.send、a.evaluate、a.press 等；写入 bytes、saved；包含循环处理；等待异步结果。 */ async()=>{
