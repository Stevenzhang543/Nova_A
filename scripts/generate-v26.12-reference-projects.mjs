...
const release='26.12',engineVersion='26.12.0',verify=process.argv.includes('--verify-only'),serverOnly=process.argv.includes('--server-only'),hash=value=>createHash('sha256').update(value).digest('hex')
...
try{
  ...
  const [templates,sync,types,compiler]=await Promise.all(Object.keys(entries).map(name=>import(pathToFileURL(join(temporary,name+'.mjs')).href)))
  ...
  for(const mode of serverOnly?[]:['code','blocks','mixed']){
    ...
    for(const script of originalScripts){
      ...
      const stable=JSON.parse(JSON.stringify(graph,(_key,value)=>typeof value==='string'&&ids.has(value)?ids.get(value):value)),source=types.serializeGraphDocument(stable),graphId=uuid(seed+'/asset'),name=script.name.replace(/\.rhai$/,'.nova-graph')
