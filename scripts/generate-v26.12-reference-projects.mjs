/** 版本26.12：生成参考项目与对应资源，供功能演示和版本验证使用。 */
...
const root=dirname(dirname(fileURLToPath(import.meta.url))),temporary=await mkdtemp(join(tmpdir(),'nova-reference-2612-'))
const release='26.12',engineVersion='26.12.0',verify=process.argv.includes('--verify-only'),serverOnly=process.argv.includes('--server-only'),hash=/* 调用 createHash('sha256').update(value).digest('hex') 并返回调用结果。 */ value=>createHash('sha256').update(value).digest('hex')
...
try{
  assert.equal(JSON.parse(await readFile(join(root,'package.json'),'utf8')).version,engineVersion,'Select the26.12 candidate before generating its references.')
  const entries={templates:'projects/templates',sync:'visual/graphCodeSync',types:'visual/graphTypes',compiler:'visual/graphCompiler'}
  ...
  const [templates,sync,types,compiler]=await Promise.all(Object.keys(entries).map(/* 调用 import(pathToFileURL(join(temporary,name+'.mjs')).href) 并返回调用结果。 */ name=>import(pathToFileURL(join(temporary,name+'.mjs')).href)))
  ...
  for(const mode of serverOnly?[]:['code','blocks','mixed']){
    const id='creator-v2612-'+mode+'-game',title='Nova 26.12 Coin Trail — '+mode,project=templates.createTemplateProject('coin-trail',title)
    ...
    for(const script of originalScripts){
      ...
      const stable=JSON.parse(JSON.stringify(graph,/* 根据 typeof value==='string'&&ids.has(value) 的真假，分别返回 ids.get(value) 或 value。 */ (_key,value)=>typeof value==='string'&&ids.has(value)?ids.get(value):value)),source=types.serializeGraphDocument(stable),graphId=uuid(seed+'/asset'),name=script.name.replace(/\.rhai$/,'.nova-graph')
  ...
  const server=JSON.parse(await readFile(join(root,'reference-projects/projects/server-v2610-headless-authority/project.nova'),'utf8')),id='server-v2612-headless-authority',title='Nova 26.12 Headless Authority',directory=join(root,'reference-projects/projects',id)
  ...
  console.log((verify?'Verified':'Generated')+(serverOnly?' the 26.12 authority only; ':' three deterministic 26.12 games and one retained authority; ')+converted+' shipped Rhai scripts converted exactly.')
