...
assert(format.includes('CURRENT_FORMAT_VERSION: u32 = 22') && format.includes('CURRENT_ENGINE_VERSION: &str = "3.0.0"'), 'Rust format authority is not schema 22 / current engine 3.0')
assert(project.includes('NOVA_PROJECT_SCHEMA_VERSION = 22') && project.includes("NOVA_ENGINE_VERSION = '3.0.0'"), 'frontend format authority is not schema 22 / current engine 3.0')
...
for (const metadata of [packageJson, cargo, tauriCargo, tauriConfig]) assert(metadata.includes('3.0.0'), 'release metadata is not synchronized to 3.0.0')

for (const [manual, topics] of [[manualEn, ['3.0.0', 'Build Settings', 'telemetry', 'rollback']], [manualDe, ['3.0.0', 'Build Settings', 'Telemetrie', 'Rückrollkopie']], [manualZh, ['3.0.0', '构建设置', '遥测', '回滚']]]) for (const topic of topics) assert(manual.toLocaleLowerCase().includes(topic.toLocaleLowerCase()), `localized manual lacks ${topic}`)
assert(manualHtml.includes('<title>Nova_A 3.0 Manual</title>') && manualHtml.includes('data-section="shipping"') && manualHtml.includes('#en-v29'), 'bookmarkable multilingual HTML manual lacks v2.9 shipping documentation')
assert(readme.includes('3.0.0') && readme.includes('schema 22') && readmeZh.includes('3.0.0') && readmeZh.includes('Schema 22'), 'release README metadata/documentation is stale')
