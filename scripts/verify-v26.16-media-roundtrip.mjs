/** 功能回归脚本：执行 verify-v26.16-media-roundtrip.mjs 对应场景，保留断言和证据输出。 */
...
const m=opened.modules,check=/** 结构说明（自动提取）：check；输入 name、fn；直接调用 fn、checks.push、console.log、String、console.error；等待异步结果。 */ async(name,fn)=>{try{await fn();checks.push({name,status:'passed'});console.log('PASS '+name)}catch(error){checks.push({name,status:'failed',error:error.stack??String(error)});console.error('FAIL '+name+': '+error.message)}}
try{
 const declarations=new Map(),aliases=new Map()
 for(const file of['runtime/animation','runtime/rigging','runtime/timeline']){const source=ts.createSourceFile(file,await readFile(opened.locate(file),'utf8'),ts.ScriptTarget.Latest,true);for(const node of source.statements){if(ts.isInterfaceDeclaration(node))declarations.set(node.name.text,node);if(ts.isTypeAliasDeclaration(node)&&ts.isUnionTypeNode(node.type)&&node.type.types.every(/* 先计算 ts.isLiteralTypeNode(n)；仅当其为真值时求右侧 ts.isStringLiteral(n.literal)，返回短路求值结果。 */ n=>ts.isLiteralTypeNode(n)&&ts.isStringLiteral(n.literal)))aliases.set(node.name.text,node.type.types.map(/* 返回 n.literal.text 的当前值。 */ n=>n.literal.text))}}
 await check('Every declared serializable media interface field has an explicit fixture and every public enum value is covered',/** 检查媒体示例覆盖全部接口字段、枚举、控制器条件运算符和约束类型。 */ ()=>{
  const examples=interfaceExamples(fixture)
  for(const[name,values]of Object.entries(examples)){const declaration=declarations.get(name);assert.ok(declaration,name);const fields=declaration.members.map(/* 调用 member.name?.getText() 并返回调用结果。 */ member=>member.name?.getText()).sort();assert.ok(values.length,name);for(const value of values)assert.deepEqual(Object.keys(value).sort(),fields,name);observations.push({interface:name,fields,examples:values.length})}
  for(const[name,values]of Object.entries(enumCoverage))assert.deepEqual([...values].sort(),[...aliases.get(name)].sort(),name)
  assert.deepEqual(new Set(fixture.controller.transitions.flatMap(/** 提取当前状态转换中每个条件的运算符。 */ v=>v.conditions.map(/* 返回 c.operator 的当前值。 */ c=>c.operator))),new Set(['==','!=','>','<','>=','<=','trigger']))
  assert.deepEqual(fixture.rig.constraints.map(/* 返回 v.type 的当前值。 */ v=>v.type),['RotationLimit','CopyRotation','PositionLimit'])
 })
 for(const[type,document]of Object.entries(fixture))await check(type+': every raw field survives actual lossless authoring validation and draft encode/decode',/** 验证媒体草稿检查和编解码保持文档一致，且不修改原始输入。 */ ()=>{
 ...
 await check('Actual project serializer, WASM migration and component hydration retain all media asset fields and component authoring values',/** 创建完整媒体资源与组件字段，经 WASM 项目序列化重开后检查组件及资源保留。 */ ()=>{
  ...
  const serialized=m.physics.getSceneJSON(),authored=JSON.parse(serialized).scenes.flatMap(/* 返回 s.entities 的当前值。 */ s=>s.entities).find(/* 比较 e.uuid 与 owner.uuid，返回严格相等的判断结果。 */ e=>e.uuid===owner.uuid)
  observations.push({componentAuthoring:authored.components.filter(/* 调用 ['Animator','Skeleton2D','TimelinePlayer'].includes(c.kind) 并返回调用结果。 */ c=>['Animator','Skeleton2D','TimelinePlayer'].includes(c.kind))})
  assert.equal(m.physics.loadProject(wasm.module.migrate_project_json(serialized)),true)
  const after=JSON.parse(m.physics.getSceneJSON()).scenes.flatMap(/* 返回 s.entities 的当前值。 */ s=>s.entities).find(/* 比较 e.uuid 与 owner.uuid，返回严格相等的判断结果。 */ e=>e.uuid===owner.uuid)
  assert.deepEqual([...after.components].sort(/* 调用 a.uuid.localeCompare(b.uuid) 并返回调用结果。 */ (a,b)=>a.uuid.localeCompare(b.uuid)),[...authored.components].sort(/* 调用 a.uuid.localeCompare(b.uuid) 并返回调用结果。 */ (a,b)=>a.uuid.localeCompare(b.uuid)))
  for(const[type,document]of Object.entries(fixture)){const asset=m.assets.assetState.records.find(/* 比较 a.assetType 与 type，返回严格相等的判断结果。 */ a=>a.assetType===type);assert.ok(asset,type);assert.deepEqual(JSON.parse(m.assets.readTextAsset(asset.uuid)),document)}
 ...
 await check('Actual deterministic NovaPak asset closure, unpacking, WASM migration and production reopening retain every field',/** 验证媒体项目两次确定性打包字节一致，NovaPak 解包与 WASM 重开保留全部资源及场景。 */ async()=>{
 ...
 await check('Unsupported bone masks, serialized skin bind poses and submachine documents are refused without mutating drafts',/** 验证不支持的骨骼遮罩、绑定姿态和嵌套状态机字段产生定位诊断且原文不变。 */ ()=>{
  for(const[type,key,value]of[['animationMask','bones',['root']],['skin','bindPoses',[{boneId:'root',matrix:[1,0,0,1,0,0]}]],['controller','submachines',[{id:'nested'}]]]){const document={...structuredClone(fixture[type]),[key]:value},before=structuredClone(document),result=m.authoring.inspectAnimationDraft(type,document);assert.ok(result.issues.some(/* 比较 i.path 与 '$.'+key，返回严格相等的判断结果。 */ i=>i.path==='$.'+key),JSON.stringify(result.issues));assert.deepEqual(document,before)}
 })
 await check('Timeline clip zero/negative rates are refused atomically; signed/zero player transport speed survives hydration',/** 验证片段非正播放率被拒绝，同时项目中的播放器零速与负速可保留。 */ ()=>{
  for(const rate of[0,-1]){const document=structuredClone(fixture.timeline);document.tracks[0].clips[0].playbackRate=rate;const before=structuredClone(document);assert.ok(m.authoring.inspectAnimationDraft('timeline',document).issues.some(/* 调用 i.path.endsWith('playbackRate') 并返回调用结果。 */ i=>i.path.endsWith('playbackRate')));assert.deepEqual(document,before)}
  for(const speed of[0,-2]){const source=JSON.parse(m.physics.getSceneJSON()),owner=source.scenes.flatMap(/* 返回 s.entities 的当前值。 */ s=>s.entities).find(/* 比较 e.uuid 与 uuid(100)，返回严格相等的判断结果。 */ e=>e.uuid===uuid(100)),player=owner.components.find(/* 比较 c.kind 与 'TimelinePlayer'，返回严格相等的判断结果。 */ c=>c.kind==='TimelinePlayer');player.data.speed=speed;assert.equal(m.physics.loadProject(wasm.module.migrate_project_json(JSON.stringify(source))),true);assert.equal(m.physics.physicsState.world.entities.find(/* 比较 e.uuid 与 uuid(100)，返回严格相等的判断结果。 */ e=>e.uuid===uuid(100)).getComponent('TimelinePlayer').speed,speed)}
 })
 await check('Runtime-only pose samples, clocks, callback queues and caches do not appear in serialized media documents',/** 检查媒体规范化后的字段集合与骨架可求解性，记录未实现能力及验证范围边界。 */ ()=>{
