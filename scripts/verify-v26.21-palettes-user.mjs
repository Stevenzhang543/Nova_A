/** 功能回归脚本：执行 verify-v26.21-palettes-user.mjs 对应场景，保留断言和证据输出。 */
...
const layout=process.argv.includes('--layout'),name=layout?'palette-layout':'palette-user',context=resolveMilestoneAuditContext(import.meta.url,{release:'26.21',reportName:name})
...
const sections=spec.split(/## \d+\. /).slice(1),expected=sections.map(/* 调用 [...section.matchAll(/`(#[A-Fa-f0-9]{6})`/g)].slice(0,6).map(x=>x[1].toLowerCase()) 并返回调用结果。 */ section=>[...section.matchAll(/`(#[A-Fa-f0-9]{6})`/g)].slice(0,6).map(/* 调用 x[1].toLowerCase() 并返回调用结果。 */ x=>x[1].toLowerCase()));assert.equal(expected.length,5)
await runMilestoneBrowserAudit(context,{name,height:1000},/** 结构说明（自动提取）：runMilestoneBrowserAudit 回调；输入 a；直接调用 worldUserControls17、u.open、join、settings、a.check；等待异步结果。 */ async a=>{
 const u=worldUserControls17(a);await u.open(join(context.repository,'reference-projects/projects/creator-v2621-code-game/project.nova'))
 const settings=/** 打开管理工作区设置的外观页并等待调色板控件出现。 */ async()=>{await u.workspace('Manage');await a.click('.manage-body>nav button',1);await a.until("!!document.querySelector('[data-audit=color-palette]')");await a.click('.settings-search nav button',0)}
 await settings()
 await a.check('Every supplied palette uses exact semantic colors through visible Settings controls',/** 逐个选择调色板，核对每个语义颜色、明暗主题及面板动画时长。 */ async()=>{
 ...
 if(layout){await a.check('Palette Settings labels controls and colors remain readable across 135 combinations',/** 遍历语言、界面缩放、调色板和宽度，检查调色板设置标签裁切及选择控件可达性。 */ async()=>{
  const failures=[]
 ...
 await a.check('Palette text and primary-action color pairs meet the declared contrast budget',/** 计算各调色板正文、次要文字和主按钮的实际颜色对比度，并检查其达到阈值。 */ async()=>{
 ...
 await a.check('High contrast and reduced motion remain user-selectable without losing palettes',/** 验证高对比度和减少动画开关可独立操作，且不会丢失当前调色板选择。 */ async()=>{
 ...
 await a.check('Each localized context-help button opens its actual bundled lesson',/** 结构说明（自动提取）：a.check 回调；无显式参数；直接调用 a.select、a.click、a.until、JSON.stringify、a.capture；包含循环处理；等待异步结果。 */ async()=>{
  for(const locale of['en','de','zh']){await a.select('.settings-page select:has(option[value="de"]):has(option[value="zh"])',locale);await a.click('[data-audit=qualification-help]');const anchor=(locale==='zh'?'zh-CN':locale)+'-v2621-production';await a.until(`document.querySelector('.manual-viewer iframe')?.contentDocument?.getElementById(${JSON.stringify(anchor)})!==null&&document.querySelector('.manual-viewer iframe')?.contentWindow?.location.hash===${JSON.stringify('#'+anchor)}`);await a.capture('help-'+locale);await a.click('.manual-viewer button.close')};await a.select('.settings-page select:has(option[value="de"]):has(option[value="zh"])','en')
 })
 await a.check('Palette choices persist and light/dark switching remembers each selection',/** 验证明暗主题分别记住调色板选择，且重新加载页面后保留当前调色板。 */ async()=>{
 ...
 await a.check('Reset Settings restores both palette defaults',/** 结构说明（自动提取）：a.check 回调；无显式参数；直接调用 u.open、join、settings、a.evaluate、a.click 等；包含循环处理；等待异步结果。 */ async()=>{await u.open(join(context.repository,'reference-projects/projects/creator-v2621-code-game/project.nova'));await settings();
