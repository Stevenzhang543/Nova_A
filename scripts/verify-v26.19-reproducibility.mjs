/** 功能回归脚本：执行 verify-v26.19-reproducibility.mjs 对应场景，保留断言和证据输出。 */
...
const context=resolveMilestoneAuditContext(import.meta.url,{release:'26.19',reportName:'reproducibility'})
...
const root=await realpath(context.sourceRoot),run=join(root,'.cache','delivery19-builds',randomUUID()),clean=join(run,'clean-source'),moved=join(run,'moved checkout with spaces'),executions=[]
...
try{
 ...
 assert.equal((await readFile(join(root,'release-audits/v26.19-repro-pnpm-version.log'),'utf8')).trim(),'10.30.0')
