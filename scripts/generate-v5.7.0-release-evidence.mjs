/** 版本5.7.0：汇集发布报告与产物文件，生成带来源记录的发布证据。 */
...
const readJson = /* 调用 JSON.parse(await readFile(join(audits, name), 'utf8')) 并返回调用结果。 */ async name => JSON.parse(await readFile(join(audits, name), 'utf8')), writeJson = /* 调用 writeFile(path, `${JSON.stringify(value, null, 2)}\n`) 并返回调用结果。 */ (path, value) => writeFile(path, `${JSON.stringify(value, null, 2)}\n`), sha256 = /* 调用 createHash('sha256').update(value).digest('hex') 并返回调用结果。 */ value => createHash('sha256').update(value).digest('hex')
...
if ([product, verification, layout].some(/* 比较 report.status 与 'passed'，返回严格不等的判断结果。 */ report => report.status !== 'passed')) throw new Error('Product, world and layout audits must pass before evidence generation.')
...
await writeJson(join(evidence, 'external/gates.json'), { format: 'nova-external-certification-gates', version: 1, release: '5.7.0', generatedAt, gates: ['publisher signing', 'independent clean-machine install and portable launch', 'cross-host Linux/macOS builds', 'real wall-clock soak', 'independent 10,000-agent hardware profiling'].map(/** 为指定外部资格生成待外部验证且不声称通过的条目。 */ name => ({ name, status: 'pending-external', claimed: false })) })
...
const environment = { id: `${platform}-${arch}-node${versions.node}`, os: platform, architecture: arch, node: versions.node }, entries = await Promise.all((await filesUnder(evidence)).sort().map(/** 记录证据文件路径、散列、长度和来源。 */ async path => { const contents = await readFile(path); return { path: relative(evidence, path).replaceAll('\\', '/'), sha256: sha256(contents), bytes: (await stat(path)).size, source: commit, tool: 'generate-v5.7.0-release-evidence.mjs', environment: environment.id } }))
