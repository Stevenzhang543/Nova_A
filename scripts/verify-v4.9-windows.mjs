/** 功能回归脚本：执行 verify-v4.9-windows.mjs 对应场景，保留断言和证据输出。 */
...
const paths = {
  ...
  msi: join(root, 'src-tauri', 'target', 'release', 'bundle', 'msi', 'Nova_A_4.9.0_x64_en-US.msi'),
  setup: join(root, 'src-tauri', 'target', 'release', 'bundle', 'nsis', 'Nova_A_4.9.0_x64-setup.exe')
}
const artifacts = {}
...
const profile = await mkdtemp(join(tmpdir(), 'nova-a-v49-native-smoke-'))
...
try {
  ...
  const report = {
    format: 'nova-v4.9-windows-smoke', version: 1, engineVersion: '4.9.0', generatedAt, host: `${process.platform}-${process.arch}`,
  ...
  await writeFile(join(root, 'release-audits', 'v4.9.0-windows-smoke.json'), `${JSON.stringify(report, null, 2)}\n`)
  if (status !== 'passed') process.exitCode = 1
  console.log(`Nova_A v4.9 Windows smoke ${status}.`)
} finally {
