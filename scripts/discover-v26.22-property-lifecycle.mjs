/** 汇总属性编辑生命周期证据，区分已执行保留检查、运行时派生排除及尚待验证项。 */
...
const discovery=JSON.parse(await readFile('release-audits/v26.22-component-corpus.json','utf8')),probes=new Map(discovery.inventory.map(/* 返回按声明顺序构造的数组 [row.component+'.'+row.field,row]。 */ row=>[row.component+'.'+row.field,row]))
const componentHistory=JSON.parse(await readFile('release-audits/v26.22-component-history.json','utf8'));if(componentHistory.status!=='passed')throw Error('Component history evidence must pass');const historyByCase=new Map(componentHistory.cases.map(/* 返回按声明顺序构造的数组 [row.index,row]。 */ row=>[row.index,row]));
const rows=[],sources=[]
/** 结合发现和历史报告构造字段证据；运行时派生字段注明排除，未证实导出效果不标记通过。 */ function componentLifecycle(probe){
 if(!probe)return{}
 const excluded=probe.disposition?.startsWith('runtime/derived exclusion:')
 const history=probe.cases?.length&&probe.cases.every(/* 比较 historyByCase.get(index)?.status 与 'passed'，返回严格相等的判断结果。 */ index=>historyByCase.get(index)?.status==='passed')?'release-audits/v26.22-component-history.json named transaction/whole-component Undo/field Redo cases '+probe.cases.join(', '):probe.cases?.length&&probe.cases.every(/* 返回 historyByCase.get(index)?.disposition 的当前值。 */ index=>historyByCase.get(index)?.disposition)?'Constructor identity replacement excluded from in-place property editing; creation/hydration retained in canonical corpus':probe.history;
 const verified=probe.cases?.length>0&&probe.cases.every(/* 比较 discovery.cases[index]?.status 与 'passed'，返回严格相等的判断结果。 */ index=>discovery.cases[index]?.status==='passed')
...
for(const [family,paths] of Object.entries(families))for(const path of paths){
 ...
 const owners=source.statements.filter(ts.isFunctionDeclaration).filter(/* 返回 n.name 的当前值。 */ n=>n.name).map(/* 返回 n.name.text 的当前值。 */ n=>n.name.text)
...
for(const alias of entityReport.aliases){
 const row=rows.find(/* 比较 row.id 与 'src/world/Entity.ts#Entity.'+alias.field，返回严格相等的判断结果。 */ row=>row.id==='src/world/Entity.ts#Entity.'+alias.field),owner=rows.find(/* 比较 row.id 与 'src/world/components.ts#'+alias.canonicalDeclaration+'.'+alias.canonicalField.split('.')[0]，返回严格相等的判断结果。 */ row=>row.id==='src/world/components.ts#'+alias.canonicalDeclaration+'.'+alias.canonicalField.split('.')[0]);
 if(!row||!owner||!alias.checks.length||!alias.checks.every(/* 调用 entityReport.checks.some(check=>check.name===name&&check.status==='passed'&&check.covers.includes(row.id)) 并返回调用结果。 */ name=>entityReport.checks.some(/* 先计算 check.name===name&&check.status==='passed'；仅当其为真值时求右侧 check.covers.includes(row.id)，返回短路求值结果。 */ check=>check.name===name&&check.status==='passed'&&check.covers.includes(row.id))))throw Error('Invalid Entity ownership evidence '+alias.field);
...
for(const field of ['priority','deterministicSeed','kind','callback']){
 const ids=field==='deterministicSeed'?['src/runtime/eventSheets.ts#EventSheetDocument.deterministicSeed','script-assets#EventSheet.deterministicSeed']:['src/runtime/eventSheets.ts#ObjectEventHandler.'+field,'script-assets#EventSheet.handlers.0.'+field],checks=eventReport.checks.filter(/* 比较 check.field 与 field，返回严格相等的判断结果。 */ check=>check.field===field);
 if(!checks.length||checks.some(/* 比较 check.status 与 'passed'，返回严格不等的判断结果。 */ check=>check.status!=='passed'))throw Error('Missing event rejection cases '+field);
 for(const id of ids){const row=rows.find(/* 比较 row.id 与 id，返回严格相等的判断结果。 */ row=>row.id===id);if(!row)throw Error('Missing event field '+id);row.eventValidationEvidence={report:eventPath,cases:checks.map(/* 返回 check.name 的当前值。 */ check=>check.name),scope:'Authored Save rejects these named input domains before changing whole-project bytes or asset generation. Legacy parse, UI and runtime effect contracts remain separate.'};row.lifecycle.validation=eventPath+' '+checks.map(/* 返回 check.name 的当前值。 */ check=>check.name).join(' / ')+'; '+row.eventValidationEvidence.scope;eventValidationLinks++;}
}
let constructorIdentityLinks=0;
for(const row of rows.filter(/* 先计算 row.family==='components'&&row.field==='kind'&&row.readOnly；仅当其为真值时求右侧 !['Component2D','ComponentBase'].includes(row.declaration)，返回短路求值结果。 */ row=>row.family==='components'&&row.field==='kind'&&row.readOnly&&!['Component2D','ComponentBase'].includes(row.declaration))){
 const cases=discovery.cases.flatMap(/* 根据 check.component===row.declaration&&check.status==='passed' 的真假，分别返回 [index] 或 []。 */ (check,index)=>check.component===row.declaration&&check.status==='passed'?[index]:[]);if(!cases.length)throw Error('Missing constructor corpus '+row.id);
...
const sceneOwnerReports=Object.fromEntries(await Promise.all(['scene-ownership','script-assets-corpus','component-order'].map(/* 返回按声明顺序构造的数组 [name,JSON.parse(await readFile('release-audits/v26.22-'+name+'.json','utf8'))]。 */ async name=>[name,JSON.parse(await readFile('release-audits/v26.22-'+name+'.json','utf8'))])));
declarationEvidence.sceneOwners=linkSceneOwnerEvidence22(rows,sceneOwnerReports);
let fixedVectorLinks=0;
for(const row of rows){const links=row.canonicalEvidence?.links;if(row.type!=='Vec2'||links?.length!==2)continue;const base=links[0].id.slice(0,-1);if(!links.every(/* 调用 link.id.startsWith(base) 并返回调用结果。 */ link=>link.id.startsWith(base))||new Set(links.map(/* 调用 link.id.slice(-1) 并返回调用结果。 */ link=>link.id.slice(-1))).size!==2||!links.every(/* 调用 ['x','y'].includes(link.id.slice(-1)) 并返回调用结果。 */ link=>['x','y'].includes(link.id.slice(-1))))continue;
 ...
 for(const key of ['mutation','validation','history','persistence','export'])row.lifecycle[key]='Fixed coordinate owners: '+links.map(/* 计算表达式 link.id+' — '+link.lifecycle[key] 并返回结果，沿用操作数的原有类型规则。 */ link=>link.id+' — '+link.lifecycle[key]).join(' / ');
...
for(const row of rows){
 const links=row.canonicalEvidence?.links||[],aliasOwner=row.entityAliasEvidence&&rows.find(/* 比较 candidate.id 与 row.entityAliasEvidence.owner，返回严格相等的判断结果。 */ candidate=>candidate.id===row.entityAliasEvidence.owner),owners=aliasOwner?[aliasOwner]:links.map(/** 按链接标识找到对应属性记录。 */ link=>rows.find(/* 比较 candidate.id 与 link.id，返回严格相等的判断结果。 */ candidate=>candidate.id===link.id)).filter(Boolean);
 ...
 for(const stage of ['validation','history','persistence','export']){
  ...
  if(!owners.every(/* 先计算 owner.lifecycle[stage]；仅当其为真值时求右侧 !String(owner.lifecycle[stage]).startsWith('pending')，返回短路求值结果。 */ owner=>owner.lifecycle[stage]&&!String(owner.lifecycle[stage]).startsWith('pending')))continue;
  row.lifecycle[stage]=(row.disposition.startsWith('Authored container')?'Populated child-field evidence only; arbitrary structural variants are not qualified. ':'Canonical owner evidence; no additional independent execution. ')+owners.map(/* 计算表达式 owner.id+' ['+stage+']' 并返回结果，沿用操作数的原有类型规则。 */ owner=>owner.id+' ['+stage+']').join(' / ');canonicalStageLinks++;
 }
}
...
for(const row of rows){
 ...
 row.review={field:row.id,classification:row.disposition,owner:row.lifecycle.mutation,stages:Object.fromEntries(reviewStages.map(/** 为每个生命周期阶段记录证据及明确的验证范围限制。 */ stage=>[stage,{evidence:row.lifecycle[stage]||'No independent '+stage+' evidence attached',status:!row.lifecycle[stage]||/pending|unqualified/i.test(typeof row.lifecycle[stage]==='string'?row.lifecycle[stage]:JSON.stringify(row.lifecycle[stage]))?'explicit evidence limit':'scoped evidence or exclusion'}])),runtimeScope:'Observable effects are reviewed by named executable family in reports/v26.22-runtime-family-evidence.json; stored-value equality alone is not a runtime pass. Unsupported effects remain in docs/GAP_REGISTER_26_22.md'};
}
const fieldReview={namedFields:rows.length,unclassified:0,canonicalStageLinks,evidenceLimits:Object.fromEntries(reviewStages.map(/** 统计各阶段仍存在明确证据限制的字段数量。 */ stage=>[stage,rows.filter(/* 比较 row.review.stages[stage].status 与 'explicit evidence limit'，返回严格相等的判断结果。 */ row=>row.review.stages[stage].status==='explicit evidence limit').length])),meaning:'Every enumerated declaration/leaf has a named disposition. Stage limits and structural/input-domain gaps remain explicit; this is not a universal field/runtime pass.'};
...
console.log(JSON.stringify({declarations:rows.length,constructorIdentityLinks,fixedVectorLinks,declarationEvidence,primitiveValidationLinks,entityAliasLinks,eventValidationLinks,families:Object.fromEntries(Object.keys(families).map(/** 按属性家族统计发现记录数量。 */ family=>[family,rows.filter(/* 比较 r.family 与 family，返回严格相等的判断结果。 */ r=>r.family===family).length]))}))
