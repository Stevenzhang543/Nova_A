/** 版本26.13：生成参考项目与对应资源，供功能演示和版本验证使用。 */
...
const root=dirname(dirname(fileURLToPath(import.meta.url))),temporary=await mkdtemp(join(tmpdir(),'nova-reference-2613-'))
const release='26.13',engineVersion='26.13.0',verify=process.argv.includes('--verify-only'),hash=/* 调用 createHash('sha256').update(value).digest('hex') 并返回调用结果。 */ value=>createHash('sha256').update(value).digest('hex')
...
try{
  assert.equal(JSON.parse(await readFile(join(root,'package.json'),'utf8')).version,engineVersion,'Select the26.13 candidate before generating its references.')
  ...
  for(const mode of ['code','blocks','mixed']){
    const id='creator-v2613-'+mode+'-game',title='Nova 26.13 Coin Trail — '+mode,project=templates.createTemplateProject('coin-trail',title)
    ...
    await output(join(directory,'README.md'),'# '+title+'\n\nPublic release **26.13** · Engine **26.13.0** · Project Format 2/schema 29.\n\nAuthoring: '+authoring+'. The same Coin Trail scene, input map and gameplay source are used by all three variants. Only the blocks variant attaches the graph asset directly. Both modes use the same runtime module resolver and command path.\n\n'+actions.map(/* 计算表达式 (index+1)+'. '+item.action+' **Expected:** '+item.expected 并返回结果，沿用操作数的原有类型规则。 */ (item,index)=>(index+1)+'. '+item.action+' **Expected:** '+item.expected).join('\n\n')+'\n\nSee [the26.12 language contract and26.13 workspace lesson](../../../docs/VERSION_26_12_LANGUAGE.md) and the English/German/Chinese offline manual. A generated reference is a repeatable test input; qualification results are recorded separately.\n')
  }
  const server=JSON.parse(await readFile(join(root,'reference-projects/projects/server-v2610-headless-authority/project.nova'),'utf8')),id='server-v2613-headless-authority',title='Nova 26.13 Headless Authority',directory=join(root,'reference-projects/projects',id)
  ...
  console.log((verify?'Verified':'Generated')+' three deterministic 26.13 games and one retained authority; '+converted+' shipped Rhai scripts converted exactly.')
