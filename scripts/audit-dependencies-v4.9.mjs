...
const report = {
  format: 'nova-v4.9-dependency-advisory-audit', version: 1, engineVersion: '4.9.0', generatedAt: new Date().toISOString(),
...
await writeFile(join(root, 'release-audits', 'v4.9.0-dependency-audit.json'), `${JSON.stringify(report, null, 2)}\n`)
if (run.status !== 0) { console.error(run.stderr || run.stdout); process.exit(run.status || 1) }
console.log('Nova_A v4.9 dependency advisory audit passed: no High/Critical advisory blocks.')
