/* 审计 4.1 的窗口、工作区、停靠、上下文工具、排版及新手引导契约。 */
...
const files = /* 调用 Promise.all(paths.map(read)) 并返回调用结果。 */ async paths => Promise.all(paths.map(read))
...
check('V410-WORKSPACES', ['design','script','animation','ui','debug','manage'].every(/* 调用 workspaces.includes(`id: '${id}'`) 并返回调用结果。 */ id => workspaces.includes(`id: '${id}'`)) && bar.includes("preset.id !== 'custom'"), 'The six public workspaces are represented directly; named custom layouts stay in the layout manager.')
check('V410-PROFILES', ['beginner','designer','programmer','ui-designer','profiler','release-engineer'].every(/* 调用 workspaces.includes(`id: '${id}'`) 并返回调用结果。 */ id => workspaces.includes(`id: '${id}'`)), 'All six named default profiles are declared.')
check('V410-DOCKING', ['floatingPanels','splitDocking','bottomPanelPinned','panelOrder','bottomTabOrder'].every(/* 调用 workspaces.includes(value) 并返回调用结果。 */ value => workspaces.includes(value)) && shell.includes("dropPanel('floating')"), 'Docking, floating, splitting, pinning, rearrangement and persistence fields are wired.')
check('V410-MANAGE', ['PackageManagerPanel','ProjectHealthPanel','BuildSettingsPanel','RenderingPanel','SettingsPanel'].every(/* 调用 manage.includes(value) 并返回调用结果。 */ value => manage.includes(value)) && !bottom.includes('PackageManagerPanel') && !bottom.includes('BuildSettingsPanel'), 'Global policy tools moved from the transient dock to Manage.')
...
check('V410-TYPOGRAPHY', ['Nunito Sans Variable','Noto Sans SC Variable','JetBrains Mono Variable'].every(/* 调用 css.includes(value) 并返回调用结果。 */ value => css.includes(value)) && main.includes('@fontsource-variable/nunito-sans') && !css.includes('accent-color: #ef4444'), 'Declared variable fonts and non-danger audio controls are used.')
check('V410-DESIGN-TOKENS', ['--type-body','--space-2','--radius-control','--focus-ring','--selection-bg','--disabled-opacity','--warning','--danger'].every(/* 调用 css.includes(value) 并返回调用结果。 */ value => css.includes(value)), 'Core typography, spacing, radius, focus, selection, disabled, warning and error tokens exist.')
check('V410-DISCOVERY', ['quickOpen','globalSearch','contextSearch','commandPalette'].every(/* 调用 shortcuts.includes(value) 并返回调用结果。 */ value => shortcuts.includes(value)) && palette.includes('stableControlInventory') && bar.includes('context-title'), 'Search modes, stable command discovery and current context are connected.')
...
check('V410-LIFECYCLE', ['stable','beta','experimental','internal'].every(/* 调用 lifecycle.includes(`'${value}'`) 并返回调用结果。 */ value => lifecycle.includes(`'${value}'`)) && lifecycle.includes('import.meta.env.DEV'), 'Lifecycle flags include Stable/Beta/Experimental/Internal and suppress Internal outside development.')
...
check('V410-TASKS', ['logs','resources','cancel','retry'].every(/* 调用 feedback.includes(value) 并返回调用结果。 */ value => feedback.includes(value)), 'Task Center records progress, cancellation, retry, logs and resource links.')
check('V410-READINESS', ['blocked','ready-warnings','ready','in-progress'].every(/* 调用 build.includes(value) 并返回调用结果。 */ value => build.includes(value)), 'All four build-readiness states are declared.')
