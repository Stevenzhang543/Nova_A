/* 审计 4.0 的版本冻结、升级预检、原子迁移、发布通道、模板和手册契约。 */
...
const check = /* 调用 checks.push({ name, status: status ? 'passed' : 'failed', detail }) 并返回调用结果。 */ (name, status, detail) => checks.push({ name, status: status ? 'passed' : 'failed', detail })
...
check('upgrade preflight', ['preflight', 'packageProblems', 'engineUpgradeRequired', 'validation'].every(/* 调用 upgrade.includes(value) 并返回调用结果。 */ value => upgrade.includes(value)), 'Pre-open compatibility report covers engine, format, packages and validation.')
...
check('release channels and crash privacy', ['stable', 'beta', 'development', 'crashReportingOptIn', 'uploaded: false', 'exportCrashReportPackage', 'releaseHealthSnapshot'].every(/* 调用 support.includes(value) 并返回调用结果。 */ value => support.includes(value)) && status.includes('crashPrivacyNotice'), 'Channels, health and local opt-in crash packaging are connected.')
...
check('manual and onboarding', manual.includes('<title>Nova_A 4.0 Manual</title>') && ['en-v40','de-v40','zh-CN-v40'].every(/* 调用 manual.includes(value) 并返回调用结果。 */ value => manual.includes(value)) && readme.includes('CREATE_EXPORT_SMALL_GAME_4_0.md'), 'Multilingual manual and guided path are published.')
...
const statusValue = checks.every(/* 比较 item.status 与 'passed'，返回严格相等的判断结果。 */ item => item.status === 'passed') ? 'passed' : 'failed'
const report = { format: 'nova-v4-production-baseline-audit', version: 1, engineVersion: '4.0.0', projectSchema: 29, generatedAt: new Date().toISOString(), severity0Open: 0, severity1Open: statusValue === 'passed' ? 0 : checks.filter(/* 比较 item.status 与 'failed'，返回严格相等的判断结果。 */ item => item.status === 'failed').length, checks, status: statusValue }
await writeFile(join(root, 'release-audits', 'v4.0.0-production-baseline-audit.json'), `${JSON.stringify(report, null, 2)}\n`)
if (statusValue !== 'passed') { console.error(checks.filter(/* 比较 item.status 与 'failed'，返回严格相等的判断结果。 */ item => item.status === 'failed')); process.exit(1) }
