...
check('Published binary version matches current machine version',/** 版本入口无需初始化物理世界。 */ ()=>{const r=spawnSync(exe,['--version'],{encoding:'utf8',windowsHide:true,timeout:10000});assert.equal(r.status,0);assert.equal(r.stdout.trim(),'nova_headless 26.29.0')})
...
const bytes=readFileSync(exe),report={format:'nova-native-headless-verification',version:1,release:'26.29',engineVersion:'26.29.0',generatedAt:new Date().toISOString(),status:checks.every(/** 所有检查共同决定资格。 */ x=>x.status==='passed')?'passed':'failed',checks,artifact:{path:exe,bytes:bytes.length,sha256:createHash('sha256').update(bytes).digest('hex')},scope:'Real native physics stdio processes; same-host command replay, bounded rejection and disposal. No project loader, Rhai VM, full-state rollback or network listener.'};mkdirSync('release-audits',{recursive:true});writeFileSync('release-audits/v26.29-native-headless.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));if(report.status!=='passed')process.exitCode=1
