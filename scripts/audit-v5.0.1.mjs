/* 审计 5.0.1 的字体、颜色、动画、导航、弹出层、本地化及非原生对话框布局。 */
...
check('UX-TYPE', ['--type-caption: calc(12px','--type-dense: calc(13px','--type-body: calc(14px','--type-section: calc(16px','--type-page: calc(22px'].every(/* 调用 sources['src/assets/main.css'].includes(value) 并返回调用结果。 */ value => sources['src/assets/main.css'].includes(value)), 'The multilingual typography floor and semantic type roles are centralized.')
check('UX-COLOR', ['--accent-secondary','--creation','--creation-soft','--bg-canvas','--border-strong',":root[data-theme='light']"].every(/* 调用 sources['src/assets/main.css'].includes(value) 并返回调用结果。 */ value => sources['src/assets/main.css'].includes(value)), 'Dark/light surface hierarchy and role colors are centralized.')
...
check('NAV-HIERARCHY', ['workspace-list','history-controls','layout-menu','command-menu','toggleEditorPanel','toggleFocusMode','openPalette'].every(/* 调用 workspace.includes(value) 并返回调用结果。 */ value => workspace.includes(value)), 'Workspace, history, layout, focus, management and command/search paths remain wired.')
...
check('TOOLS-COMPLETE', ['transformTools','authoringTools','shapeTools','transformActions','snapOptions','guidesAndRulers','cameraOverlay','alignSelection','distributeSelection','mirrorSelection','groupSelection'].every(/* 调用 toolbar.includes(value) 并返回调用结果。 */ value => toolbar.includes(value)), 'Transform, edit, draw, arrange, snapping, guide, camera and grouping capabilities remain reachable.')
check('TOOLS-GROUPS', ['authoring-menu','action-grid','snap-menu','guide-menu','authoring-popover','viewport-popover'].every(/* 调用 toolbar.includes(value) 并返回调用结果。 */ value => toolbar.includes(value)), 'Secondary scene controls are organized in named popovers.')
...
check('I18N-ACCESSIBLE-NAMES', ['generatedAriaLabel','generatedDisabledReason','characterData','refreshTextOwner'].every(/* 调用 sources['src/runtime/controlRegistry.ts'].includes(value) 并返回调用结果。 */ value => sources['src/runtime/controlRegistry.ts'].includes(value)), 'Registry-generated accessible names and disabled reasons follow live language changes.')

const sourceNames = (await readdir(join(root, 'src'), { recursive: true })).filter(/* 调用 /\.(vue|ts)$/.test(name) 并返回调用结果。 */ name => /\.(vue|ts)$/.test(name))
const sourceFiles = await Promise.all(sourceNames.map(/* 返回按声明顺序构造的数组 [name, await readFile(join(root, 'src', name), 'utf8')]。 */ async name => [name, await readFile(join(root, 'src', name), 'utf8')]))
const literalKeys = new Set(sourceFiles.flatMap(/* 从源码中提取直接以字符串字面量调用 t 的翻译键。 */ ([, source]) => [...source.matchAll(/\bt\(\s*['"]([^'"]+)['"]/g)].map(/* 返回 match[1] 的当前值。 */ match => match[1])))
const missingLiteralKeys = [...literalKeys].filter(/* 返回 new RegExp(`(?:^|[,{\\s])${key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*:`).test(i18n) 的逻辑取反结果。 */ key => !new RegExp(`(?:^|[,{\\s])${key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*:`).test(i18n))
...
check('DIALOGS-NATIVE-FREE', sources['src/components/ConfirmDialog.vue'].includes('role="alertdialog"') && sources['src/store/dialog.ts'].includes('requestConfirmation') && !sourceFiles.some(/* 调用 /\b(?:window\.)?(?:confirm|alert|prompt)\s*\(/.test(source) 并返回调用结果。 */ ([, source]) => /\b(?:window\.)?(?:confirm|alert|prompt)\s*\(/.test(source)), 'App-native confirmation is present and source code does not call browser confirm/alert/prompt.')
check('SPECIFICATION', ['First-user audit','Information architecture','Typography','Dark palette','Motion system','Localization and accessibility','Required audit matrix','Build and release checks','Do not remove a feature'].every(/* 调用 sources['instructions.txt'].toLowerCase().includes(value.toLowerCase()) 并返回调用结果。 */ value => sources['instructions.txt'].toLowerCase().includes(value.toLowerCase())), 'instructions.txt is the authoritative complete 5.0.1 UX specification.')
check('DOCUMENTATION', ['5.0.1','Layout','Commands','Authoring tools','View settings'].every(/* 调用 sources['docs/UX_GUIDE_5_0_1.md'].includes(value) 并返回调用结果。 */ value => sources['docs/UX_GUIDE_5_0_1.md'].includes(value)) && ['MANUAL.en.md','MANUAL.de.md','MANUAL.zh-CN.md'].every(/* 调用 sources[`manual/${name}`].includes('5.0.1') 并返回调用结果。 */ name => sources[`manual/${name}`].includes('5.0.1')), 'README, UX guide and three Markdown manuals describe the new organization.')
