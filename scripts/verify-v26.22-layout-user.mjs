...
await withBrowserAudit({release:'26.22',name:'layout-user',development:process.argv.includes('--development'),expectedRelease:process.argv.includes('--development')?JSON.parse(await readFile('package.json','utf8')).version.split('.').slice(0,2).join('.'):'26.22',width:1366,height:768},async a=>{
 ...
 await a.check('Focused numeric draft survives resize, saves its exact value, and survives maximize/restore',async()=>{
  ...
  let saved;for(let n=0;n<100&&!saved;n++){try{saved=JSON.parse(await readFile(join(a.profile,'project.nova'),'utf8'))}catch{await wait(100)}}assert.ok(saved,'Actual saved file');const player=saved.scenes.flatMap(s=>s.entities).find(e=>e.name==='Player');assert.equal(player.components.find(c=>c.kind==='Transform2D').data.position.x,value)
 ...
 await a.check('Hierarchy and Inspector float and redock through visible workspace controls',async()=>{
 ...
 await a.check('Navigation names and paired profiler budgets remain readable in all locales at 100/150/200 percent',async()=>{
