/** 功能回归脚本：执行 verify-v6.0.3-windows.mjs 对应场景，保留断言和证据输出。 */
...
if (process.platform !== 'win32') throw new Error('The v6.0.3 Windows verifier must run on Windows.')
const root = dirname(dirname(fileURLToPath(import.meta.url)))
const editor = join(root, 'src-tauri/target/release/nova_a.exe')
const msi = join(root, 'src-tauri/target/release/bundle/msi/Nova_A_6.0.3_x64_en-US.msi')
const setup = join(root, 'src-tauri/target/release/bundle/nsis/Nova_A_6.0.3_x64-setup.exe')
const project = join(root, 'reference-projects/projects/creator-v603-template-export-accessibility/project.nova')
const output = join(root, 'release-audits/game-output-v6.0.3')
...
/** 结构说明（自动提取）：launchSmoke；输入 path、duration；直接调用 spawn、dirname、Date.now、child.once、Promise 等；等待异步结果。 */ async function launchSmoke(path, duration) {
  ...
  child.once('exit', /** 结构说明（自动提取）：child.once 回调；输入 code、signal；写入 exit。 */ (code, signal) => { exit = { code, signal } }); child.once('error', /** 结构说明（自动提取）：child.once 回调；输入 value；直接调用 String；写入 error。 */ value => { error = value instanceof Error ? value.message : String(value) })
