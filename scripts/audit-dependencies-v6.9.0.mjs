/* 依赖审计 v6.9.0：核对本地包声明、锁文件和完整性记录，输出带锁文件哈希的证据；在线安全公告资格单独保留。 */
...
const declared = Object.keys({ ...(pkg.dependencies ?? {}), ...(pkg.devDependencies ?? {}), ...(pkg.optionalDependencies ?? {}) }).sort(), missing = declared.filter(/* 先计算 !lockSource.includes(`  ${name}:`) && !lockSource.includes(`'${name}':`)；仅当其为真值时求右侧 !lockSource.includes(`/${name}@`)，返回短路求值结果。 */ name => !lockSource.includes(`  ${name}:`) && !lockSource.includes(`'${name}':`) && !lockSource.includes(`/${name}@`))
const checks = [
