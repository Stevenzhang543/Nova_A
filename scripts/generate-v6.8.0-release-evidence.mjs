/** 版本6.8.0：汇集发布报告与产物文件，生成带来源记录的发布证据。 */
...
await writeJson(join(evidence,'external/gates.json'), { format:'nova-external-certification-gates',version:1,release:version,generatedAt,gates:['live dependency advisory registry lookup','real low-end hardware matrix','publisher identity and signing','independent clean-machine install/launch/upgrade/repair/uninstall','second-machine byte reproduction','matching-host platform builds','independent accessibility review','real 72-hour editor/player soak'].map(/** 为指定外部资格生成待外部验证且不声称通过的条目。 */ name=>({name,status:'pending-external',claimed:false})) })
...
const entries = await Promise.all((await filesUnder(evidence)).sort().filter(/* 返回 path.endsWith('evidence-manifest.json') 的逻辑取反结果。 */ path=>!path.endsWith('evidence-manifest.json')).map(/** 读取证据文件，返回相对路径、内容散列、字节数及提交、工具和环境身份。 */ async path=>{const contents=await readFile(path);return{path:relative(evidence,path).replaceAll('\\','/'),sha256:sha256(contents),bytes:contents.length,source:commit,tool:'generate-v6.8.0-release-evidence.mjs',environment:environment.id}}))
