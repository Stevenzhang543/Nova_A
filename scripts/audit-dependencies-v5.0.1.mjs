/* 依赖审计 v5.0.1：通过当前固定包管理器查询高危及以上安全公告，解析结果并写入版本报告。 */
...
const report = {
  format: 'nova-v5.0.1-dependency-advisory-audit', version: 1, engineVersion: '5.0.1', generatedAt: new Date().toISOString(),
...
await writeFile(join(root, 'release-audits', 'v5.0.1-dependency-audit.json'), `${JSON.stringify(report, null, 2)}\n`)
if (run.status !== 0) { console.error(run.stderr || run.stdout); process.exit(run.status || 1) }
console.log('Nova_A v5.0.1 dependency advisory audit passed: no High/Critical advisory blocks.')
