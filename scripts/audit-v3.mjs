...
assert(pkg.version === '3.0.0' && tauri.version === '3.0.0', 'Application/Tauri version is not 3.0.0.')
assert(projectFormat.includes("NOVA_ENGINE_VERSION = '3.0.0'") && projectFormat.includes('NOVA_PROJECT_SCHEMA_VERSION = 22') && projectFormat.includes('NOVA_MINIMUM_SCHEMA_VERSION = 5'), 'TypeScript format authority must remain schema 22 / schemas 5–22 / engine 3.0.0.')
assert(rustFormat.includes('CURRENT_ENGINE_VERSION: &str = "3.0.0"') && rustFormat.includes('CURRENT_FORMAT_VERSION: u32 = 22') && rustFormat.includes('MINIMUM_SUPPORTED_FORMAT_VERSION: u32 = 5'), 'Rust format authority does not match the frozen v3 contract.')
...
assert(cli.includes("const ENGINE_VERSION = '3.0.0'") && cli.includes("['windows', 'linux', 'macos', 'web']"), 'Build CLI 1 is not current or lacks the documented targets.')
...
assert(JSON.stringify(inputs.publicSchemas) === JSON.stringify(Array.from({ length: 18 }, (_, index) => index + 5)), 'Migration golden inputs do not cover every schema 5–22.')
assert(expected.targetSchema === 22 && expected.targetEngine === '3.0.0', 'Migration golden output is not schema 22 / engine 3.0.0.')
...
assert(packages.includes("engine: '>=2.9.0 <4.0.0'") && packages.includes("engineVersion = '3.0.0'"), 'Official optional packages are not compatible with the v3 engine range.')
