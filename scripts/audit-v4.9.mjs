/* 审计 4.9 的构建分级、导出来源、包信任、发布管线、隐私及文档契约。 */
...
].map(/* 返回按声明顺序构造的数组 [path, await read(path)]。 */ async path => [path, await read(path)])))
...
check('BLD-TIERS', ['Windows','Web','tier-1','Linux','macOS','Experimental','unavailable','buildHosts','minimumSystem','lastQualified'].every(/* 调用 all.includes(value) 并返回调用结果。 */ value => all.includes(value)), 'Platform tiers, host requirements, availability and evidence are explicit.')
check('BLD-EXPORT', ['exportTemplate','stripUnusedAssets','compression','debugSymbols','crashSymbols','contentManifest','releaseChannel','signingHook','notarizationHook','deploymentMode','cleanMachineJob'].every(/* 调用 all.includes(value) 并返回调用结果。 */ value => all.includes(value)), 'Export templates, content, delivery, crash/symbol and external hook settings are connected.')
check('BLD-PROVENANCE', ['nova-build-provenance','nova-sbom','nova-deployment-manifest','webDeploymentHeaders','compareBuildProvenance','inputsHash','outputsHash','cacheKey'].every(/* 调用 all.includes(value) 并返回调用结果。 */ value => all.includes(value)), 'Build provenance, SBOM, manifests, web headers and deterministic comparison are implemented.')
check('PKG-FAIL-CLOSED', ['publisherVerified','verifyPackageArchive','packageInstallReview','executionAllowed','grantedPermissions','quarantinePackage','rollbackPackage'].every(/* 调用 all.includes(value) 并返回调用结果。 */ value => all.includes(value)), 'Package execution is gated by compatibility, provenance, integrity and permissions.')
check('PKG-ECOSYSTEM', ['registries','offlineMirror','resolvePackageLockfile','verifyPackageCache','licenseUrl','vulnerabilityPolicy','certification','dependencyHashes'].every(/* 调用 all.includes(value) 并返回调用结果。 */ value => all.includes(value)), 'Registry, lock, offline, cache, license, vulnerability, dependency and certification metadata exist.')
check('HLT-TEAM', ['semanticProjectComparison','novaIgnoreFile','codeOwnersFile','ownership','taskLinks','changeNotes','sharedBuildPresets','binaryLocks','networkOperations'].every(/* 调用 all.includes(value) 并返回调用结果。 */ value => all.includes(value)), 'Optional local-first source-control/team workflow is wired.')
check('HLT-RELEASE', ['releaseReadinessGate','projectSchemaGate','packageSecurityGate','documentationGate','cleanMachineMatrix','rcObservation','RELEASE_CANDIDATE_FREEZE'].every(/* 调用 all.includes(value) 并返回调用结果。 */ value => all.includes(value)), 'Project Health exposes the release-readiness gates and direct fixes.')
check('REL-PIPELINE', ['NOVA_RELEASE_PIPELINE','references:v4.9','audit:v4.9','verify:v4.9','evidence:v4.9','release:v4.9'].every(/* 调用 all.includes(value) 并返回调用结果。 */ value => all.includes(value)), 'The release pipeline and package scripts are declared.')
check('BLD-CLI', sources['scripts/nova-cli.mjs'].includes("ENGINE_VERSION = '4.9.0'") && sources['scripts/nova-cli.mjs'].includes('CLI_VERSION = 1') && ['host or CI runner','nova-build-provenance','nova-content-manifest','CycloneDX','implicitNetworkOperation'].every(/* 调用 sources['scripts/nova-export.mjs'].includes(value) 并返回调用结果。 */ value => sources['scripts/nova-export.mjs'].includes(value)), 'Build CLI 1 is current, host-safe and emits release metadata.')
...
check('I18N-V49', ['en','de','zh'].every(/* 调用 sources['src/i18n.ts'].includes("releaseLabel:'Nova_A v4.9.0'") 并返回调用结果。 */ language => sources['src/i18n.ts'].includes("releaseLabel:'Nova_A v4.9.0'")), 'The v4.9 vocabulary is present in English, German and Chinese.')
