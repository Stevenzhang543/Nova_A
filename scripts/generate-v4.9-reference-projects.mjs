...
function refreshProject(project) {
  ...
  for (const asset of project.assets ?? []) if (typeof asset?.source === 'string' && asset.source.includes('"engine"')) asset.source = asset.source.replace('<5.0.0', '<6.0.0')
}
const fixtures = [
  ['build-v49-platform-matrix', 'empty', 'Build 4.9 Platform Matrix', ['Windows Tier 1', 'Web Tier 1', 'explicit unavailable targets', 'export presets', 'content manifest']],
  ['build-v49-release-pipeline', 'build-automation', 'Build 4.9 Release Pipeline', ['provenance', 'SBOM', 'web headers', 'release channel', 'clean-machine job', 'build comparison']],
  ['package-v49-extension-sdk', 'package-authoring', 'Package 4.9 Extension SDK', ['publisher identity', 'checksums', 'permissions', 'license', 'API 2 certification', 'offline mirror', 'rollback']],
  ['collaboration-v49-local-team', 'source-control-workflow', 'Collaboration 4.9 Local Team', ['Git status', 'semantic diff', 'ownership', 'CODEOWNERS', 'task links', 'change notes', 'binary lock guidance']],
...
function updateProject(project, slug, title, features) {
  ...
  project.projectSettings.build.releaseEngineering = {
    release: '4.9.0', schema: 29, rhaiApi: 2, pluginApi: 2, packageManifest: 1, buildCli: 1,
...
for (const [slug, source, title, features] of fixtures) {
  ...
  await writeFile(join(directory, 'expected-output.json'), json({
    engineVersion: '4.9.0', schema: 29, projectName: title, expectedValidation: 'pass', features,
  ...
  await writeFile(join(directory, 'test-controls.json'), json({
    engineVersion: '4.9.0', open: 'Project Manager > Open project.nova',
...
for (const entry of entries) {
  ...
  try {
    ...
    await writeFile(readmePath, readme.replace(/Engine \*\*\d+\.\d+\.\d+\*\*/g, 'Engine **4.9.0**'))
    for (const name of ['expected-output.json', 'test-controls.json']) {
      try {
        const path = join(directory, name)
        const document = JSON.parse(await readFile(path, 'utf8'))
        document.engineVersion = '4.9.0'
...
console.log('Generated five Nova_A v4.9 release-system references and refreshed all reference metadata.')
