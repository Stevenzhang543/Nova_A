/** 功能回归脚本：执行 verify-v26.15-template-ui-user.mjs 对应场景，保留断言和证据输出。 */
...
await withBrowserAudit({release:'26.15',name:'template-ui-user',width:1600,height:1000},/** 结构说明（自动提取）：withBrowserAudit 回调；输入 audit；直接调用 check；包含循环处理；等待异步结果。 */ async audit=>{
 const {evaluate,until,click,clickText,fill,client,check,capture,observations,evidence}=audit
 const selectEntity=/** 结构说明（自动提取）：selectEntity；输入 name；直接调用 click、until、fill、evaluate、JSON.stringify 等；等待异步结果。 */ async name=>{await click('.workspace-list button',0);await until("!!document.querySelector('.hierarchy-header input[type=search]')");await fill('.hierarchy-header input[type=search]',name);const index=await evaluate("[...document.querySelectorAll('.entity-item .name')].findIndex(e=>{const c=e.cloneNode(true);c.querySelector('small')?.remove();return c.textContent.trim()==="+JSON.stringify(name)+"})");assert.ok(index>=0,'Visible hierarchy object '+name);await click('.entity-item .name',index);await until("document.querySelector('.inspector-header')?.textContent.includes("+JSON.stringify(name)+")")}
 for(const id of['ui-showcase','responsive-ui','audio-lab']){
  await check(id+': actual launcher, Text caption,68/100 progress and None panel layout are visible in Inspector',/** 结构说明（自动提取）：check 回调；无显式参数；直接调用 until、fill、click、evaluate、audit.press 等；等待异步结果。 */ async()=>{
  ...
  await check(id+': actual Play draws a localized caption and68% progress at the authored positions',/** 结构说明（自动提取）：check 回调；无显式参数；直接调用 click、until、clickText、assert.equal、evaluate 等；写入 frame、image、metrics；包含循环处理；等待异步结果。 */ async()=>{
   ...
   for(let attempt=0;attempt<60;attempt++){
    ...
    const ratio=blue.length/(end-start);metrics={captionPixels:positions.length,captionWidth:positions.length?Math.max(...positions.map(/* 返回 p[0] 的当前值。 */ p=>p[0]))-Math.min(...positions.map(/* 返回 p[0] 的当前值。 */ p=>p[0]))+1:0,progressRatio:ratio,fillPixel:auditPixel(image,barX+button.width*.25,barY),emptyPixel:auditPixel(image,barX+button.width*.85,barY),scale}
