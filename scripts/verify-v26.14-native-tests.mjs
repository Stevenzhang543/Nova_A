...
async function check(name, test) { await test(); checks.push({ name, status: 'passed' }) }
await check('suite hooks execute once; each case receives isolated initialized export state', async () => {
  ...
  assert.ok(report.results.every(test => test.fixture === 'seeded' && test.seed === 42)); assert.equal(coverage.functionRate, 1)
})
await check('test failures preserve after_each and after_all; cleanup failures remain visible', async () => {
...
await check('failed before_each skips body but always attempts case and suite teardown', async () => {
...
await check('failed before_all runs suite teardown and never begins case fixtures', async () => {
...
await check('skip-only suite never initializes or runs hooks and has no claimed coverage', async () => {
...
await check('actual compiler discovery finds compact/multiline functions and ignores comment/string lookalikes', async () => {
  const { report, coverage } = await run('discovery', '/* fn test_phantom() {}\n// @test skip\nfn test_second() {} */\nlet text = `fn test_fake() {}`;\nfn test_first() {} fn test_second() {}\nfn\ntest_third\n() {}')
  assert.equal(report.passed, 3); assert.equal(report.skipped, 0); assert.deepEqual(report.results.map(test => test.name), ['test_first', 'test_second', 'test_third']); assert.deepEqual(coverage.files[0].executableFunctions, ['test_first', 'test_second', 'test_third'])
})
await check('compile failure is a failed report entry with no fabricated execution', async () => {
...
await check('retries require explicit infrastructure flag and retain cleanup for every attempt', async () => {
