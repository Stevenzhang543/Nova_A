/** 验证脚本（v26.06-template-output）：组织对应功能与边界场景检查，断言行为并汇总验证结果。 */
...
try {
  ...
  check('OUTPUT-ALL-TEMPLATES', templateResults.length === 20 && templateResults.every(/* 先计算 result.web === 'passed'；仅当其为真值时求右侧 result.windowsPortable === 'passed'，返回短路求值结果。 */ result => result.web === 'passed' && result.windowsPortable === 'passed'), 'Every registered launcher template exports through the supported Web folder and Windows portable packaging paths.', { templateResults })
