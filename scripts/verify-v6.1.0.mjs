/** 验证脚本（v6.1.0）：组织对应功能与边界场景检查，断言行为并汇总验证结果。 */
...
try{
  ...
  const paths=await Promise.all(['package.json','pnpm-workspace.yaml','src-tauri/tauri.conf.json','src/runtime/editorWindow.ts','src/App.vue','src/layout/EditorLayout.vue','src/components/WorldCanvas.vue','src/runtime/profiler.ts','src/renderer/renderGraph.ts','src/assets/main.css','scripts/nova-export.mjs','scripts/package-release.ps1'].map(/* 返回按声明顺序构造的数组 [path,await text(path)]。 */ async path=>[path,await text(path)]))
  const source=Object.fromEntries(paths),activeSource=paths.map(/* 返回 value 的当前值。 */ ([,value])=>value).join('\n')
  ...
  check('V610-VISUAL-LANGUAGE',['-apple-system','Segoe UI Variable Text','Noto Sans SC Variable','--font-display','semantic surface','backdrop blur'].every(/* 调用 source['src/assets/main.css'].includes(marker) 并返回调用结果。 */ marker=>source['src/assets/main.css'].includes(marker))&&!source['src/components/WorldCanvas.vue'].includes('imageSmoothingQuality = \'low\''),'System-first typography, semantic materials, restrained live glass, and high-quality canvas smoothing are present.')
