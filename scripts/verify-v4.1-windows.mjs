/** 功能回归脚本：执行 verify-v4.1-windows.mjs 对应场景，保留断言和证据输出。 */
...
const paths = {
  ...
  msi: join(root, 'src-tauri', 'target', 'release', 'bundle', 'msi', 'Nova_A_4.1.0_x64_en-US.msi'),
  setup: join(root, 'src-tauri', 'target', 'release', 'bundle', 'nsis', 'Nova_A_4.1.0_x64-setup.exe')
}
...
const profile = await mkdtemp(join(tmpdir(), 'nova-a-v41-native-smoke-'))
let child
let earlyExit = null
try {
  ...
  const report = { format: 'nova-windows-smoke', version: 1, engineVersion: '4.1.0', generatedAt, host: `${process.platform}-${process.arch}`, isolatedProfile: true, artifacts, launchSeconds: 10, stayedAlive, earlyExit, checks, cleanMachineLifecycle: 'pending external disposable-VM gate', publisherSigning: 'pending signing identity', systemMutationPerformedByAudit: false, status }
  await mkdir(join(root, 'release-audits'), { recursive: true })
  await writeFile(join(root, 'release-audits', 'v4.1.0-windows-smoke.json'), `${JSON.stringify(report, null, 2)}\n`)
  console.log(`Nova_A v4.1 Windows smoke ${status}: portable stayed alive for 10 seconds; EXE/MSI/NSIS hashes captured.`)
