...
async function standardResult(request){
  ...
  if(method==='textDocument/rename'){const name=wordAt(source,params.position),updates=index.rename(name,String(params.newName??''),{uri,offset:offsetAt(source,params.position)});return{changes:Object.fromEntries([...updates].map(([target,text])=>[target,[{range:{start:{line:0,character:0},end:{line:(index.document(target)?.source.split(/\r?\n/).length??1)+1,character:0}},newText:text}]]))}}
