/** 功能回归脚本：执行 verify-v6.0.2-windows.mjs 对应场景，保留断言和证据输出。 */
...
if (process.platform !== 'win32') throw new Error('The v6.0.2 Windows verifier must run on Windows.')
const root = dirname(dirname(fileURLToPath(import.meta.url)))
const editor = join(root, 'src-tauri/target/release/nova_a.exe')
const msi = join(root, 'src-tauri/target/release/bundle/msi/Nova_A_6.0.2_x64_en-US.msi')
const setup = join(root, 'src-tauri/target/release/bundle/nsis/Nova_A_6.0.2_x64-setup.exe')
const project = join(root, 'reference-projects/projects/creator-v602-interaction-export-audit/project.nova')
const output = join(root, 'release-audits/game-output-v6.0.2')
...
const game = join(output, 'Mouse Knockout.exe'), bytes = await readFile(game)
...
const gameSmoke = await launchSmoke(game, 5_000)
const editorSmoke = await launchSmoke(editor, 3_000)
...
/** 结构说明（自动提取）：launchSmoke；输入 path、duration；直接调用 spawn、dirname、Date.now、child.once、Promise 等；等待异步结果。 */ async function launchSmoke(path, duration) {
  const child = spawn(path, [], { cwd: dirname(path), windowsHide: true, stdio: 'ignore' })
  const startedAt = Date.now(); let exit = null, error = ''
  child.once('exit', /** 结构说明（自动提取）：child.once 回调；输入 code、signal；写入 exit。 */ (code, signal) => { exit = { code, signal } })
  child.once('error', /** 结构说明（自动提取）：child.once 回调；输入 value；直接调用 String；写入 error。 */ value => { error = value instanceof Error ? value.message : String(value) })
  await new Promise(/* 调用 setTimeout(resolve, duration) 并返回调用结果。 */ resolve => setTimeout(resolve, duration))
  const stayedAlive = exit === null && !error
