/* 汇总 26.07 的网络多进程、包安全、回滚、服务及发布报告一致性。 */
...
const vuePaths = (await filesUnder(join(root, 'src'))).filter(/* 调用 path.endsWith('.vue') 并返回调用结果。 */ path => path.endsWith('.vue')).sort()
const vue = await Promise.all(vuePaths.map(/* 返回相对于仓库根目录的规范路径及对应 UTF-8 源码。 */ async path => ({ path: relative(root, path).replaceAll('\\', '/'), source: await readFile(path, 'utf8') })))
check('V2607-AUDIT-ALL-SURFACES', vue.length >= 65 && vue.every(/* 先计算 item.source.includes('<template')；仅当其为真值时求右侧 item.source.includes('<script')，返回短路求值结果。 */ item => item.source.includes('<template') && item.source.includes('<script')), 'Every Vue surface was enumerated for the programmer-facing source audit.', { vueFiles: vue.length })
...
const reports = Object.fromEntries(await Promise.all(reportFiles.map(/* 返回按声明顺序构造的数组 [name, await report(name)]。 */ async name => [name, await report(name)])))
const authorityIssues = Object.entries(reports).flatMap(/* 检查报告状态与引擎、公开发布版本是否匹配，返回发现的不一致项。 */ ([name, value]) => {
  const issues = []
...
const processMatrixPassed = [2, 4, 8].every(/* 验证指定实例数恰好对应一条已通过且有收发确认的多进程回归记录。 */ count => processRegression.processMatrix?.filter(/* 筛选实例数、客户端数和 RPC 收发确认均符合要求的已通过进程记录。 */ item => item?.instances === count && item?.status === 'passed' && item?.clientProcesses === count - 1 && item?.rpcReceived > 0 && item?.acknowledged > 0).length === 1)
...
check('V2607-AUDIT-WEB-OUTPUT', ['includeWebDynamicEntry','isTauriManifestEntry','projectPackageEnabled','includeWebDynamicEntry(project, child)'].every(/* 调用 exporter.includes(token) 并返回调用结果。 */ token => exporter.includes(token)), 'CLI web export traverses static dependencies but conditionally includes optional dynamic runtime chunks and excludes Tauri-only dynamic entries from browser output.')
check('V2607-AUDIT-MULTI-INSTANCE', ['launch_network_instances','network_instance_status','stop_network_instance','stop_network_instances','validate_network_player_build','nova-build-report.json','embedded_package(executable)','runtime_project_document','network_policy_from_project','network_instance_launcher_requires_a_reported_embedded_player','endpoint: host_endpoint.clone()','bind_address'].every(/* 调用 native.includes(token) 并返回调用结果。 */ token => native.includes(token)) && ['openInstanceLogs','openInstanceInspector','stopNetworkInstance','instance.endpoint','instance.bindAddress','selectedInstance.endpoint','selectedInstance.bindAddress'].every(/* 调用 panel.includes(token) 并返回调用结果。 */ token => panel.includes(token)), 'Multi-instance cards expose isolated lifecycle commands and the actual per-process endpoint/bind address, while native launch requires a current reported embedded player and matching runtime policy before any executable starts.')
