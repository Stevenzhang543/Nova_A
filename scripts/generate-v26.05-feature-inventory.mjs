...
const root=dirname(dirname(fileURLToPath(import.meta.url))),output=join(root,'docs/FEATURE_INVENTORY_26_05.md'),compiled=await mkdtemp(join(tmpdir(),'nova-v2605-inventory-'))
...
try{
  ...
  lines.push('## 26.05 focus','','The canonical Rendering operation now includes a production checklist shared with Build, bounded texture residency, material and Canvas2D fallback review, numbered fixed-frame capture with integer audio sample ownership, and Balanced/Low-end semantic parity. Existing animation curves/dope sheets/state machines/blend trees/rigging, audio buses/effects/spatialization/snapshots, lights/shadows/normal maps, particles/trails, post effects, pixel-art/high-DPI paths and skippable timelines remain connected to their existing runtime and export routes.','')
