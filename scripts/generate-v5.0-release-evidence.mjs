/** 版本5.0：汇集发布报告与产物文件，生成带来源记录的发布证据。 */
...
await writeJson(join(tree, 'build', 'reproducibility.json'), {
  ...
  localDeterministicComparison: reports.runtime.checks?.find(/* 比较 item.id 与 'BLD-REPRODUCIBILITY'，返回严格相等的判断结果。 */ item => item.id === 'BLD-REPRODUCIBILITY')?.status ?? 'not-run', tenConsecutiveCleanBuilds: reports.cleanBuilds,
...
await writeJson(join(tree, 'build', 'clean-machine-matrix.json'), {
  ...
  jobs: ['MSI install + launch', 'NSIS install + launch', 'portable launch', '4.9 upgrade to 5.0', 'repair', 'uninstall and residue review', 'hosted web launch'].map(/** 生成带空证据引用的待外部资格条目。 */ id => ({ id, status: 'pending-external', evidence: '' })),
...
await writeJson(join(tree, 'packages', 'security-matrix.json'), {
  ...
  ].map(/** 提取5.0实际运行用例状态并记录报告引用，缺失时未运行。 */ ([name, id]) => ({ name, status: reports.runtime.checks?.find(/* 比较 item.id 与 id，返回严格相等的判断结果。 */ item => item.id === id)?.status ?? 'not-run', evidence: `release-audits/v5.0.0-verification.json#${id}` })),
