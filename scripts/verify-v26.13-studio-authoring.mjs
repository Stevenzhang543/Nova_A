/** 验证脚本（v26.13-studio-authoring）：组织对应功能与边界场景检查，断言行为并汇总验证结果。 */
import assert from 'node:assert/strict'
import {withBrowserAudit,wait} from './lib/browserUserAudit.mjs'

await withBrowserAudit({release:'26.13',name:'studio-authoring'},/** 通过工作区实际操作验证脚本草稿、响应布局、图布线撤销、路由持久化及冲突恢复。 */ async audit=>{
  ...
  await check('Create a disposable project and source script through real controls',/** 通过启动界面及键盘创建项目和脚本，并核对编辑器发布版本。 */ async()=>{
  ...
  await check('Measured full layout preserves selected identity and viewport and is one Undo operation',/** 拖动后执行完整布局，验证选择与视口保持且一次撤销重做恢复对应位置。 */ async()=>{
  ...
  await check('Selected layout leaves every unselected position unchanged',/** 验证仅排列选中节点时未选节点位置、视口和选择均保持。 */ async()=>{
  ...
  await check('Saved-source changes expose a recovered graph conflict and require an explicit decision',/** 验证图注释草稿与外部已保存源码冲突时需要明确处理，并可放弃草稿重新加载。 */ async()=>{
  ...
  await check('Event Sheet changes survive workspace unmount and validated Save clears recovery',/** 验证事件表无效草稿跨工作区保留，修复并保存后清除修改及恢复状态。 */ async()=>{
