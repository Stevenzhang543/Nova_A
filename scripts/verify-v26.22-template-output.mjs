/** 验证脚本（v26.22-template-output）：组织对应功能与边界场景检查，断言行为并汇总验证结果。 */
...
const release = '26.22', development = process.argv.includes('--development')
if (!development) assert.equal(engineVersion, '26.22.0', 'Template output qualification requires actual 26.22 source')
...
const report = {
  format: 'nova-v26.22-template-output-verification',
  version: 1,
  release,
  engineVersion,
  development,
...
await writeFile(join(root, 'release-audits', 'v26.22-template-output.json'), `${JSON.stringify(report, null, 2)}\n`)
if (failed.length) { console.error(failed); process.exit(1) }
console.log(`Nova_A 26.22 template output verification passed: ${templateResults.length} templates, ${checks.length} checks.`)
