/** 版本26.15：生成参考项目与对应资源，供功能演示和版本验证使用。 */
...
const root=dirname(dirname(fileURLToPath(import.meta.url))),temporary=await mkdtemp(join(tmpdir(),'nova-reference-2615-'))
const release='26.15',engineVersion='26.15.0',verify=process.argv.includes('--verify-only'),hash=/* 调用 createHash('sha256').update(value).digest('hex') 并返回调用结果。 */ value=>createHash('sha256').update(value).digest('hex')
...
try{
  assert.equal(JSON.parse(await readFile(join(root,'package.json'),'utf8')).version,engineVersion,'Select the26.15 candidate before generating its references.')
  ...
  for(const mode of ['code','blocks','mixed']){
    const id='creator-v2615-'+mode+'-game',title='Nova 26.15 Coin Trail — '+mode,project=templates.createTemplateProject('coin-trail',title)
  ...
  const server=JSON.parse(await readFile(join(root,'reference-projects/projects/server-v2610-headless-authority/project.nova'),'utf8')),id='server-v2615-headless-authority',title='Nova 26.15 Headless Authority',directory=join(root,'reference-projects/projects',id)
  ...
  console.log((verify?'Verified':'Generated')+' three deterministic 26.15 games and one retained authority; '+converted+' shipped Rhai scripts converted exactly.')
