...
try {
  console.log('v5.0 verification: loading platform/build contracts')
  ...
  console.log('v5.0 verification: loading package/team contracts')
  ...
  try { await novaPak.createNovaPak(JSON.stringify({ engineVersion: '5.0.0', assets: [maliciousAsset], scenes: [], projectSettings: { build: { developmentBuild: false } } }), [maliciousAsset], 'scene') } catch (error) { maliciousArchiveRejected = error instanceof Error && error.message.includes('Unsafe package path') }
...
console.log(`Nova_A v5.0 runtime verification passed: ${checks.length} checks.`)
process.exit(0)
