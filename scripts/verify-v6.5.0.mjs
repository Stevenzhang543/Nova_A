/** 功能回归脚本：执行 verify-v6.5.0.mjs 对应场景，保留断言和证据输出。 */
...
const colliderSource = /** 结构说明（自动提取）：colliderSource；输入 shapeModel、vertices、shapes；返回表达式求值结果。 */ (shapeModel, vertices = [], shapes = []) => ({
...
const child = /** 创建启用的碰撞形状默认配置，并允许传入属性覆盖几何和碰撞参数。 */ (id, kind, offset, size, properties = {}) => ({ id, kind, offset, rotation: 0, size, radius: Math.min(size.x, size.y) * .5, points: [], enabled: true, sensor: false, physicsLayer: 0, collisionMask: 0xffff_ffff, oneWay: false, oneWayNormal: { x: 0, y: 1 }, ...properties })
...
try {
  ...
  check('V650-EXACT-COMPOUND', compoundA.shapes.length === 3 && new Set(compoundA.shapes.map(/* 返回 item.id 的当前值。 */ item => item.id)).size === 3 && JSON.stringify(compoundA) === JSON.stringify(compoundB), 'Compound children remain independent, stable and deterministic rather than becoming one envelope.')
  ...
  const areas = compoundA.shapes.map(/* 调用 geometry.solverShapeArea(shape, { x: 2, y: .5 }) 并返回调用结果。 */ shape => geometry.solverShapeArea(shape, { x: 2, y: .5 })), totalArea = areas.reduce(/* 计算表达式 sum + area 并返回结果，沿用操作数的原有类型规则。 */ (sum, area) => sum + area, 0), totalMass = totalArea * 2
  const compoundInertia = compoundA.shapes.reduce(/* 计算表达式 sum + geometry.solverShapeInertia(shape, { x: 2, y: .5 }, totalMass * areas[index] / totalArea) 并返回结果，沿用操作数的原有类型规则。 */ (sum, shape, index) => sum + geometry.solverShapeInertia(shape, { x: 2, y: .5 }, totalMass * areas[index] / totalArea), 0)
  ...
  const bounded = rendering.normalizeRenderingSettings({ qualityVolumes: Array.from({ length: 80 }, /** 结构说明（自动提取）：Array.from 回调；输入 _、index；返回表达式求值结果。 */ (_, index) => ({ id: `v-${index}`, enabled: true, center: { x: 0, y: 0 }, size: { x: Infinity, y: -1 }, priority: Infinity, preset: 'Ultra' })) })
  check('V650-QUALITY-BOUNDS', bounded.qualityVolumes.length === 64 && bounded.qualityVolumes.every(/* 先计算 Number.isFinite(item.size.x) && item.size.x >= .001；仅当其为真值时求右侧 Number.isFinite(item.priority)，返回短路求值结果。 */ item => Number.isFinite(item.size.x) && item.size.x >= .001 && Number.isFinite(item.priority)), 'Malformed quality volumes are finite and the authoring list is bounded to 64.')
  ...
  const sources = Object.fromEntries(await Promise.all(paths.map(/* 返回按声明顺序构造的数组 [path, await source(path)]。 */ async path => [path, await source(path)])))
