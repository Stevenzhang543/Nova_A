/** 功能回归脚本：执行 verify-v4.9-reference-ci.mjs 对应场景，保留断言和证据输出。 */
...
projectFiles.sort()
const temporary = await mkdtemp(join(tmpdir(), 'nova-a-v49-reference-ci-'))
const results = []
...
try {
  for (let index = 0; index < projectFiles.length; index++) {
    ...
    const metadataOk = document.engineVersion === '4.9.0' && document.formatVersion === 29 && document.manifest?.engineCompatibility?.maximumExclusive === '6.0.0'
...
const report = { format: 'nova-v4.9-reference-project-ci', version: 1, engineVersion: '4.9.0', generatedAt, projectCount: results.length, targets: ['web','windows'], packageExample: 'package-v49-extension-sdk/project.nova', pluginExample: 'reference-projects/plugins/hello-plugin/plugin.json', results, status: failed.length ? 'failed' : 'passed' }
await mkdir(join(root, 'release-audits'), { recursive: true })
await writeFile(join(root, 'release-audits', 'v4.9.0-reference-ci.json'), `${JSON.stringify(report, null, 2)}\n`)
if (failed.length) { console.error(`Reference CI failed for ${failed.length}/${results.length} projects.`); process.exit(1) }
console.log(`Nova_A v4.9 reference CI passed: ${results.length} projects × Windows/Web.`)
