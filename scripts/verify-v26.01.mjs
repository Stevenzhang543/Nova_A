/** 验证脚本（v26.01）：组织对应功能与边界场景检查，断言行为并汇总验证结果。 */
...
try {
  ...
  check('V2601-COMPATIBILITY-SEAL', normalized.engineCompatibility.maximumExclusive === '27.0.0' && manifest.manifestCompatibility(normalized).compatible && preview.supported && preview.requiresMigration && preview.targetSchema === 29 && preview.migrationSteps.some(/* 调用 step.name.includes('calendar-version') 并返回调用结果。 */ step => step.name.includes('calendar-version')), 'Legacy reviewed ceilings migrate to <27.0.0 as metadata only, with preview and frozen schema.', { warnings: preview.warnings })
  const ids = templates.PROJECT_TEMPLATES.map(/* 返回 item.id 的当前值。 */ item => item.id), counts = Object.fromEntries(templates.PROJECT_TEMPLATE_CATEGORIES.map(/** 统计模板目录中指定分类的模板数量。 */ category => [category, templates.PROJECT_TEMPLATES.filter(/* 比较 item.category 与 category，返回严格相等的判断结果。 */ item => item.category === category).length]))
...
const failed = checks.filter(/* 比较 item.status 与 'failed'，返回严格相等的判断结果。 */ item => item.status === 'failed'), report = { format: 'nova-v26.01-verification', version: 1, release: '26.01', engineVersion: '26.1.0', generatedAt: new Date().toISOString(), perspectives: ['versioning','compatibility','templates','binding','documentation','user'], checks, severity0Open: failed.length, severity1Open: 0, status: failed.length ? 'failed' : 'passed' }
