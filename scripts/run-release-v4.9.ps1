# 历史 4.9 发布流程：依次执行版本、构建、审计与发布步骤，失败立即停止。
[CmdletBinding()]
param(
...
try {
  ...
  Invoke-ReleaseCommand 'Generate v4.9 references' 'pnpm' @('run', 'references:v4.9')
  Invoke-ReleaseCommand 'Complete BLD/PKG/HLT/regression catalog' 'pnpm' @('run', 'audit')
  if (-not $SkipOnlineAdvisoryAudit) { Invoke-ReleaseCommand 'Dependency vulnerability advisory scan' 'pnpm' @('run', 'security:v4.9') }
  ...
  if (-not $SkipLayout) { Invoke-ReleaseCommand 'Pinned Chromium layout qualification' 'pnpm' @('run', 'qualify:v4.9:layout') }
  if (-not $SkipNativeBuild) {
    Invoke-ReleaseCommand 'Windows native installers' 'pnpm' @('tauri', 'build')
    Invoke-ReleaseCommand 'Windows isolated startup' 'pnpm' @('run', 'verify:v4.9:windows')
  }
  Invoke-ReleaseCommand 'All reference projects on Tier-1 targets' 'pnpm' @('run', 'references:verify:v4.9')
  Invoke-ReleaseCommand 'Structured release evidence' 'pnpm' @('run', 'evidence:v4.9')
  ...
  $report = [ordered]@{
    format = 'nova-v4.9-release-pipeline-run'; version = 1; engineVersion = $releaseVersion
    generatedAt = [DateTime]::UtcNow.ToString('o'); status = $status; stages = @($commands)
    externalGates = @('minimum 14-day RC observation', '24-hour soak', 'two-machine reproducibility', 'clean-machine install lifecycle', 'publisher signing/notarization', 'external browser/hardware matrix', 'independent verifier')
  } | ConvertTo-Json -Depth 8
  [IO.File]::WriteAllText((Join-Path $auditDirectory 'v4.9.0-release-pipeline-run.json'), "$report`n", [Text.UTF8Encoding]::new($false))
}
