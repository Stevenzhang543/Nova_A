/** 验证脚本（v6.0.4）：组织对应功能与边界场景检查，断言行为并汇总验证结果。 */
...
try{
  ...
  const logNode=graph.nodes.find(/* 比较 node.type 与 'api.log_info'，返回严格相等的判断结果。 */ node=>node.type==='api.log_info'),messagePin=logNode?.pins.find(/* 比较 pin.key 与 'message'，返回严格相等的判断结果。 */ pin=>pin.key==='message')
  const edited=source.replace('Hello from Nova Visual Script','Changed from linked Rhai')
  const synchronized=sync.applyLinkedRhaiSource(graph,edited),updatedLog=synchronized.graph.nodes.find(/* 比较 node.uuid 与 logNode?.uuid，返回严格相等的判断结果。 */ node=>node.uuid===logNode?.uuid),updatedMessage=updatedLog?.pins.find(/* 比较 pin.key 与 'message'，返回严格相等的判断结果。 */ pin=>pin.key==='message')
  ...
  const customResult=sync.applyLinkedRhaiSource(synchronized.graph,custom),moduleNode=customResult.graph.nodes.find(/* 比较 node.type 与 'code.module'，返回严格相等的判断结果。 */ node=>node.type==='code.module')
  const roundTrip=types.parseGraphDocument(types.serializeGraphDocument(customResult.graph)),roundTripModule=roundTrip.nodes.find(/* 比较 node.type 与 'code.module'，返回严格相等的判断结果。 */ node=>node.type==='code.module')
  ...
  const [native,editor,app,assets]=await Promise.all(['src-tauri/src/lib.rs','src/components/VisualGraphEditor.vue','src/App.vue','src/assets/AssetDatabase.ts'].map(/* 调用 readFile(join(root,path),'utf8') 并返回调用结果。 */ path=>readFile(join(root,path),'utf8')))
...
const failed=checks.filter(/* 比较 item.status 与 'failed'，返回严格相等的判断结果。 */ item=>item.status==='failed'),report={format:'nova-v6.0.4-verification',version:1,engineVersion:'6.0.4',generatedAt:new Date().toISOString(),perspectives:['backend','visual-scripting','script-authoring','performance','build','portable-game'],checks,severity0Open:failed.length,severity1Open:0,status:failed.length?'failed':'passed'}
