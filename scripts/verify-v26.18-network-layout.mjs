/** 功能回归脚本：执行 verify-v26.18-network-layout.mjs 对应场景，保留断言和证据输出。 */
...
await runMilestoneBrowserAudit(context,{name:'network-layout'},/** 审查网络面板授权、复制、协议和编排编辑，以及多种布局下的字段可用性。 */ async a=>{
 ...
 const network=/** 切换到设计工作区的网络面板并将其最大化以便操作。 */ async()=>{await u.workspace('Design');await u.activate('.bottom-panel .panel-tab',5);await until("!!document.querySelector('.network-studio')");if(await evaluate("document.querySelector('[data-panel-maximize=bottom]')?.getAttribute('aria-pressed')!=='true'"))await u.activate('[data-panel-maximize=bottom]');};
 ...
 await a.check('Invalid numeric drafts preserve authored values and valid values remain editable',/** 验证非法通道数值回退并显示错误，修正为有效值后清除错误。 */ async()=>{
 ...
 await a.check('Interest coordinates accept fractional world positions',/** 验证网络编排坐标保留小数而不会被错误截断。 */ async()=>{await u.activate('#network-studio-tab-orchestration');await a.fill('.coordinate-row input','1.25');assert.equal(await evaluate("document.querySelector('.coordinate-row input').value"),'1.25')});
 const violations=[];
 await a.check('All six Network Studio tabs remain readable across locale/theme/scale/width',/** 遍历语言、缩放、主题、宽度和网络标签页，检查标签尺寸、文本裁切及最后控件可操作性。 */ async()=>{
 for(const locale of(smoke?['en']:['en','de','zh']))for(const scale of(smoke?[1]:[1,1.5,2]))for(const theme of(smoke?[0]:[0,1]))for(const width of(smoke?[720]:[720,1280,1920])){
  ...
  for(const variant of['session-host','session-client','session-server','protocol','replication','orchestration','simulation','diagnostics']){
   ...
   if(metrics.clipped.length||metrics.fields.some(/* 先计算 f.width<80；仅当其为假值时求右侧 !f.label，返回短路求值结果。 */ f=>f.width<80||!f.label)||reach&&(!reach.focused||!reach.visible||!reach.reachable))violations.push({name,...metrics,reach});observations.push({name,...metrics,reach});await capture(name);console.log('SURFACE '+name);
  }
 }
