/** 功能回归脚本：执行 verify-v26.22-hierarchy-user.mjs 对应场景，保留断言和证据输出。 */
...
await withBrowserAudit({release:'26.22',name:'hierarchy-user',development:process.argv.includes('--development'),expectedRelease:process.argv.includes('--development')?JSON.parse(await readFile('package.json','utf8')).version.split('.').slice(0,2).join('.'):'26.22',height:1000,commandTimeoutMs:profileLoad?30000:120000},/** 结构说明（自动提取）：withBrowserAudit 回调；输入 a；直接调用 worldUserControls17、performance.now、a.client.send、u.open、writeFile 等；等待异步结果；包含显式抛错路径。 */ async a=>{
