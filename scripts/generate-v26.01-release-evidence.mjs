...
const reports = {
  product: await readJson('v26.01-product-audit.json'),
  verification: await readJson('v26.01-verification.json'),
  visual: await readJson('v26.01-visual-roundtrip.json'),
  interactions: await readJson('v26.01-user-interactions.json'),
  layout: await readJson('v26.01-layout-browser.json'),
  templates: await readJson('template-catalog-verification.json'),
  performance: await readJson('v26.01-benchmarks.json'),
  stability: await readJson('v26.01-stability-smoke.json')
}

for (const [source, target] of [
  ['v26.01-product-audit.json', 'runtime/product-audit.json'],
  ['v26.01-verification.json', 'runtime/verification.json'],
  ['v26.01-visual-roundtrip.json', 'runtime/visual-roundtrip.json'],
  ['v26.01-user-interactions.json', 'runtime/user-interactions.json'],
  ['template-catalog-verification.json', 'runtime/template-catalog.json'],
  ['v26.01-layout-browser.json', 'layout/layout-browser.json'],
  ['v26.01-benchmarks.json', 'performance/benchmarks.json'],
  ['v26.01-stability-smoke.json', 'performance/stability-local.json']
...
for (const name of [
  'VERSIONING_2026.md', 'FEATURE_INVENTORY_26_01.md', 'COMPETITIVE_REVIEW_26_01.md',
  'ROADMAP_26_01_TO_26_10.md', 'TEMPLATE_LIBRARY_26_01.md', 'VISUAL_SCRIPTING_26_01.md',
  'UI_LAYOUT_AUDIT_26_01.md', 'COMPATIBILITY.md', 'STABLE_CONTRACTS.md', 'KNOWN_LIMITATIONS.md'
...
const entries = await Promise.all((await filesUnder(evidence)).sort()
  ...
  .map(async path => {
    ...
    return {
      ...
      source: commit, tool: 'generate-v26.01-release-evidence.mjs', environment: environment.id
    }
  }))
