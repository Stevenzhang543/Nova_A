/** 功能回归脚本：执行 verify-v7.0.0.mjs 对应场景，保留断言和证据输出。 */
...
try {
  ...
  const complete = readiness.length === learning.CREATOR_LEARNING_GUIDES.length && readiness.every(/** 结构说明（自动提取）：readiness.every 回调；输入 item；直接调用 dimensions.every；返回表达式求值结果。 */ item => dimensions.length === 7 && dimensions.every(/** 结构说明（自动提取）：dimensions.every 回调；输入 dimension；直接调用 allowed.has；返回表达式求值结果。 */ dimension => item.dimensions[dimension] && allowed.has(item.dimensions[dimension].status) && item.dimensions[dimension].route && item.dimensions[dimension].detail))
  ...
  check('V700-MIGRATION-PREVIEW', preview.supported && preview.requiresMigration && preview.sourceSchema === 29 && preview.targetSchema === 29 && preview.targetEngine === version && preview.migrationSteps.some(/* 调用 step.name.includes('compatibility metadata seal') 并返回调用结果。 */ step => step.name.includes('compatibility metadata seal')) && preview.preflight.some(/* 先计算 item.id === 'backup'；仅当其为真值时求右侧 item.status === 'passed'，返回短路求值结果。 */ item => item.id === 'backup' && item.status === 'passed'), 'A 6.9/schema-29 project receives a metadata-only, backed-up compatibility migration preview.', { warnings: preview.warnings })
  ...
  check('V700-FUTURE-BLOCK', !futurePreview.supported && futurePreview.preflight.some(/* 比较 item.status 与 'blocked'，返回严格相等的判断结果。 */ item => item.status === 'blocked'), 'Future schemas fail closed before editor state is modified.')
  ...
  check('V700-REFERENCES', references.every(/** 结构说明（自动提取）：references.every 回调；输入 project；返回表达式求值结果。 */ project => project.engineVersion === version && project.projectFormatMajor === 2 && project.formatVersion === 29 && project.manifest.engineCompatibility.maximumExclusive === '8.0.0') && referencePackage.engine === '>=6.0.0 <8.0.0' && referencePackage.provenance.includes('v700'), 'Both v7 guided references and their reusable package use the frozen format and reviewed <8 compatibility ceiling.')
} finally {
...
console.log(`Nova_A v7.0.0 verification passed: ${checks.length} stable-platform checks across ${checks.find(/* 比较 item.id 与 'V700-FEATURE-READINESS'，返回严格相等的判断结果。 */ item => item.id === 'V700-FEATURE-READINESS')?.metrics.features ?? 0} public features.`)
