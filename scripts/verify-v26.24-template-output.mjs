/** 26.24 保留回归：复用 26.23 已审计的 template-output 检查，在当前版本执行并单独记录证据。 */
...
const release = '26.24', development = process.argv.includes('--development')
if (!development) assert.equal(engineVersion, '26.24.0', 'Template output qualification requires actual 26.24 source')
...
const report = {
  format: 'nova-v26.24-template-output-verification',
  version: 1,
  release,
  engineVersion,
  development,
  ...
  scope: { launcherTemplates: templateResults.length, webRuntime: 'Actual current production dist copied into each export; rendering is qualified by the separate browser library and export-user suites.', targets: ['web', 'windows-portable-structural'], realPlayerLaunch: 'not-executed-by-this-headless-check' },
  checks,
  severity0Open: 0,
...
await writeFile(join(root, 'release-audits', 'v26.24-template-output.json'), `${JSON.stringify(report, null, 2)}\n`)
if (failed.length) { console.error(failed); process.exit(1) }
console.log(`Nova_A 26.24 template output verification passed: ${templateResults.length} templates, ${checks.length} checks.`)
