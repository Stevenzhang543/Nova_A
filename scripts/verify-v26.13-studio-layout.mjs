...
try{
  ...
  assert.equal(JSON.stringify(occupied),occupiedBefore);assert.ok(occupied.every(rect=>placed.x>=rect.x+rect.width+24||placed.x+commentSize.width+24<=rect.x||placed.y>=rect.y+rect.height+24||placed.y+commentSize.height+24<=rect.y));assert.deepEqual(placeGraphComment([], {x:25,y:35},commentSize),{x:25,y:35})
  ...
  for(const point of [{x:NaN,y:1},{x:1,y:Infinity},{x:1_000_001,y:0}]){assert.throws(()=>changeWireReroute(edge,0,point));assert.throws(()=>insertWireReroute(edge,point,ends));assert.equal(JSON.stringify(edge),before)}
  assert.throws(()=>changeWireReroute(edge,-1,{x:0,y:0}),/no longer exists/);assert.throws(()=>changeWireReroute(edge,2,{x:0,y:0}),/no longer exists/)
  assert.throws(()=>insertWireReroute({...edge,reroutes:Array.from({length:64},()=>({x:1,y:2}))},{x:2,y:3},ends),/64/)
  assert.throws(()=>insertWireReroute(edge,{x:2,y:3},ends,100_000),/100,000/);assert.equal(JSON.stringify(edge),before)
  ...
  await writeFile(join(root,'release-audits/v26.13-studio-layout.json'),JSON.stringify({status:'passed',generatedAt:new Date().toISOString(),scope:'Production pure pane/geometry/wire helpers; no browser geometry or integrated editor interaction claim.',checks:groups.map(name=>({name,status:'passed'}))},null,2)+'\n')
