...
await runMilestoneBrowserAudit(context,{name,height:1000},async a=>{
 ...
 await a.check('Export the merged game and collect the checkpoint at its changed position',async()=>{
  const exported=await exportDeliveryUser19(a,u,'merged',saved),server=fork(fileURLToPath(new URL('./lib/worldExportServer17.mjs',import.meta.url)),[exported.output],{stdio:['ignore','ignore','pipe','ipc'],windowsHide:true});let error='';server.stderr.on('data',data=>error+=data)
  try{const port=await new Promise((done,reject)=>{const timer=setTimeout(()=>reject(Error('Player server timed out '+error)),15000);server.once('message',value=>{clearTimeout(timer);done(value.port)});server.once('error',reject)});await a.client.send('Page.navigate',{url:'http://127.0.0.1:'+port+'/'});await a.until("!!document.querySelector('.player-root .render-canvas')",30000);await a.until("[...document.querySelectorAll('[data-game-ui-control]')].some(e=>e.getAttribute('aria-label')?.includes('Checkpoint 1/6'))",20000);await a.click('.overlay-canvas');await a.press('d',0,850);await a.press('w',0,330);await a.until("[...document.querySelectorAll('[data-game-ui-control]')].some(e=>e.getAttribute('aria-label')?.includes('Checkpoint 2/6'))",5000);await a.capture('merged-export-checkpoint-collected');a.observations.push({name:'exported-gameplay-at-edited-position',text:await a.evaluate("[...document.querySelectorAll('[data-game-ui-control]')].map(e=>e.getAttribute('aria-label')).join(' | ')")});await a.press('r');await a.until("[...document.querySelectorAll('[data-game-ui-control]')].some(e=>e.getAttribute('aria-label')?.includes('Checkpoint 1/6'))",5000)}finally{server.disconnect();await new Promise(done=>server.once('exit',done))}
 })
