/** 版本5.4.0：汇集发布报告与产物文件，生成带来源记录的发布证据。 */
...
const writeJson=/* 调用 writeFile(path,`${JSON.stringify(value,null,2)}\n`) 并返回调用结果。 */ (path,value)=>writeFile(path,`${JSON.stringify(value,null,2)}\n`),readJson=/* 调用 JSON.parse(await readFile(join(audits,name),'utf8')) 并返回调用结果。 */ async name=>JSON.parse(await readFile(join(audits,name),'utf8')),sha256=/* 调用 createHash('sha256').update(value).digest('hex') 并返回调用结果。 */ value=>createHash('sha256').update(value).digest('hex')
...
if([product,verification,layout].some(/* 比较 report.status 与 'passed'，返回严格不等的判断结果。 */ report=>report.status!=='passed'))throw new Error('Product, gameplay and browser-layout audits must pass before v5.4 evidence generation.')
...
await writeJson(join(evidence,'external/gates.json'),{format:'nova-external-certification-gates',version:1,release:'5.4.0',generatedAt,gates:['publisher signing','independent clean-machine install and portable launch','cross-host Linux/macOS builds','real wall-clock soak'].map(/** 为指定外部资格生成待外部验证且不声称通过的条目。 */ name=>({name,status:'pending-external',claimed:false}))})
/** 递归枚举普通文件供证据归档。 */ async function filesUnder(directory){const output=[];for(const entry of await readdir(directory,{withFileTypes:true})){const path=join(directory,entry.name);if(entry.isDirectory())output.push(...await filesUnder(path));else if(entry.isFile())output.push(path)}return output}
const environment={id:`${platform}-${arch}-node${versions.node}`,os:platform,architecture:arch,node:versions.node},entries=await Promise.all((await filesUnder(evidence)).sort().map(/** 读取证据文件并记录路径、散列、长度及工具环境。 */ async path=>{const source=await readFile(path);return{path:relative(evidence,path).replaceAll('\\','/'),sha256:sha256(source),bytes:(await stat(path)).size,source:commit,tool:'generate-v5.4.0-release-evidence.mjs',environment:environment.id}}))
