...
try {
  ...
  Invoke-ReleaseCommand 'Generate v5.0 references' 'pnpm' @('run', 'references:v5.0')
  Invoke-ReleaseCommand 'Complete BLD/PKG/HLT/regression catalog' 'pnpm' @('run', 'audit')
  if (-not $SkipOnlineAdvisoryAudit) { Invoke-ReleaseCommand 'Dependency vulnerability advisory scan' 'pnpm' @('run', 'security:v5.0') }
  ...
  if (-not $SkipLayout) { Invoke-ReleaseCommand 'Pinned Chromium layout qualification' 'pnpm' @('run', 'qualify:v5.0:layout') }
  if (-not $SkipNativeBuild) {
    Invoke-ReleaseCommand 'Windows native installers' 'pnpm' @('tauri', 'build')
    Invoke-ReleaseCommand 'Windows isolated startup' 'pnpm' @('run', 'verify:v5.0:windows')
  }
  Invoke-ReleaseCommand 'All reference projects on Tier-1 targets' 'pnpm' @('run', 'references:verify:v5.0')
  Invoke-ReleaseCommand 'Ten consecutive clean Tier-1 payload builds' 'pnpm' @('run', 'reproducibility:v5.0')
  Invoke-ReleaseCommand 'Pinned performance baselines' 'pnpm' @('run', 'benchmark:v5.0')
  Invoke-ReleaseCommand 'Bounded stability/fault smoke' 'pnpm' @('run', 'stability:v5.0')
  Invoke-ReleaseCommand 'Structured release evidence' 'pnpm' @('run', 'evidence:v5.0')
  ...
  $report = [ordered]@{
    format = 'nova-v5.0-release-pipeline-run'; version = 1; engineVersion = $releaseVersion
  ...
  [IO.File]::WriteAllText((Join-Path $auditDirectory 'v5.0.0-release-pipeline-run.json'), "$report`n", [Text.UTF8Encoding]::new($false))
}
