/** 版本26.03：汇集发布报告与产物文件，生成带来源记录的发布证据。 */
...
const reports = {
  product: await readJson('v26.03-product-audit.json'),
  verification: await readJson('v26.03-verification.json'),
  interactions: await readJson('v26.03-user-interactions.json'),
  layout: await readJson('v26.03-layout-browser.json'),
  templates: await readJson('template-catalog-verification.json'),
  history: await readJson('v26.03-history-verification.json'),
  security: await readJson('v26.03-dependency-audit.json'),
  windows: await readJson('v26.03-windows-smoke.json'),
  performance: await readJson('v26.03-benchmarks.json'),
  stability: await readJson('v26.03-stability-smoke.json')
}

for (const [source, target] of [
  ['v26.03-product-audit.json', 'runtime/product-audit.json'],
  ['v26.03-verification.json', 'runtime/verification.json'],
  ['v26.03-user-interactions.json', 'runtime/user-interactions.json'],
  ['template-catalog-verification.json', 'runtime/template-catalog.json'],
  ['v26.03-history-verification.json', 'runtime/migration-history.json'],
  ['v26.03-dependency-audit.json', 'runtime/dependency-audit.json'],
  ['v26.03-windows-smoke.json', 'build/windows-smoke.json'],
  ['v26.03-layout-browser.json', 'layout/layout-browser.json'],
  ['v26.03-benchmarks.json', 'performance/benchmarks.json'],
  ['v26.03-stability-smoke.json', 'performance/stability-local.json']
...
for (const name of [
  'VERSIONING_2026.md', 'FEATURE_INVENTORY_26_01.md', 'FEATURE_INVENTORY_26_02.md', 'FEATURE_INVENTORY_26_03.md',
  ...
  'VISUAL_GRAPH_PERFORMANCE_26_02.md', 'LANGUAGE_DEBUGGING_26_03.md', 'UI_LAYOUT_AUDIT_26_01.md', 'UI_LAYOUT_AUDIT_26_02.md', 'UI_LAYOUT_AUDIT_26_03.md',
  'RELEASE_NOTES_26_03.md', 'COMPATIBILITY.md', 'STABLE_CONTRACTS.md', 'KNOWN_LIMITATIONS.md'
...
const entries = await Promise.all((await filesUnder(evidence)).sort()
  ...
  .map(/** 记录证据相对路径、字节数、散列及源码和工具来源。 */ async path => {
    ...
    return {
      ...
      source: commit, tool: 'generate-v26.03-release-evidence.mjs', environment: environment.id
    }
  }))
...
if (!localQualificationComplete) throw new Error('The Nova_A 26.03 local evidence tree is incomplete; release packaging is blocked.')
