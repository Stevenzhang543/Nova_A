/** 功能回归脚本：执行 verify-v26.20-palettes-user.mjs 对应场景，保留断言和证据输出。 */
...
const layout=process.argv.includes('--layout'),name=layout?'palette-layout':'palette-user',context=resolveMilestoneAuditContext(import.meta.url,{release:'26.20',reportName:name})
...
await runMilestoneBrowserAudit(context,{name,height:1000},/** 通过设置界面检查全部调色板语义颜色、布局、对比度、辅助功能、帮助和持久化。 */ async a=>{
 const u=worldUserControls17(a);await u.open(join(context.repository,'reference-projects/projects/creator-v2620-code-game/project.nova'))
 ...
 await a.check('Reset Settings restores both palette defaults',/** 通过实际重置设置按钮恢复默认深色与浅色调色板。 */ async()=>{await u.open(join(context.repository,'reference-projects/projects/creator-v2620-code-game/project.nova'));await settings();
