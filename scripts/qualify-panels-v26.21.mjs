...
if (!['26.21', '26.22', '26.23', '26.24'].includes(qualificationVersion)) throw Error('Unsupported panel audit release')
...
try {
  ...
  client.on('Log.entryAdded', /** 收集浏览器日志中的错误级别条目。 */ event => { if (event.entry?.level === 'error') consoleErrors.push(event.entry.text + (event.entry.url ? " | " + event.entry.url : "")) })
  ...
  const report = { format: `nova-v${qualificationVersion}-layout-qualification`, version: 1, release: qualificationVersion, engineVersion: qualificationEngineVersion, generatedAt: new Date().toISOString(), development, qualifiedRelease: null, scope: 'Visible DOM geometry plus field-capacity findings; shortFields and visible navigation label clipping are included in the pass status. Conditional-state coverage requires separate reports. Development runs do not qualify a release.', preflight, palette, browser: browser.product, graphics, languages: ['en','de','zh'], matrix:requiredMatrix.length?requiredMatrix:isV41?{viewports:['1366x768','1920x1080','2560x1440','3840x2160'],scales:[100,125,150,175,200],catalogs:['SHELL','LCH','HLT','BLD']}:undefined,requiredTextResults,stableControls,screenshots,results,consoleErrors: seriousConsoleErrors }
  ...
  console.log(`Nova_A v${qualificationVersion} layout geometry ${development ? 'development audit' : 'audit'} passed: ${results.length - 1} panel/viewport states in three languages; ${screenshots.length} captures.`)
} finally {
...
/** 等待面板过渡稳定，再测量可见控件溢出和重叠、外壳边界及工具栏；同时验证展开的完整选项和字段字符容量，保存真实结果和稳定控件标识。 */ async function recordLayoutOne(cdp, collection, name) {
  ...
  const detail = await evaluate(cdp, `(() => {
    ...
    const selectTextWidth = node => { const text=(node.selectedOptions?.[0]?.textContent||'').trim(); if(!text)return 0; const style=getComputedStyle(node),canvas=document.createElement('canvas'),context=canvas.getContext('2d'); if(!context)return 0; context.font=style.fontStyle+' '+style.fontWeight+' '+style.fontSize+' '+style.fontFamily; return context.measureText(text).width+parseFloat(style.paddingLeft||'0')+parseFloat(style.paddingRight||'0')+28 }
    const selectedValueDetails = [], verifiedSelectDetails = new Set(), originalFocus = document.activeElement
    for (const node of [...document.querySelectorAll('select')].filter(visible)) {
      if (selectTextWidth(node)<=node.clientWidth+1) continue
      node.dispatchEvent(new PointerEvent('pointerover',{bubbles:true}))
      const hint=document.querySelector('[data-nova-select-detail]'), r=hint?.getBoundingClientRect(),text=(node.selectedOptions[0]?.textContent||'').trim()
      const complete=!!hint&&!hint.hidden&&hint.textContent===text&&r.width>0&&r.left>=0&&r.right<=innerWidth&&r.top>=0&&r.bottom<=innerHeight&&hint.scrollWidth<=hint.clientWidth+1&&hint.scrollHeight<=hint.clientHeight+1
      selectedValueDetails.push({control:description(node),text,clipped:true,fullValueVisible:complete})
      if(complete)verifiedSelectDetails.add(node)
      node.dispatchEvent(new PointerEvent('pointerout',{bubbles:true}))
    }
    originalFocus?.focus?.({preventScroll:true})
    const overflow = [...document.querySelectorAll('button,summary,label,input,select,.camera-overlay,.segmented')].filter(visible).filter(node => { const style=getComputedStyle(node),horizontal=node.tagName==='SELECT'?selectTextWidth(node)>node.clientWidth+1&&!verifiedSelectDetails.has(node):(node.scrollWidth>node.clientWidth+1)&&style.overflowX==='visible',vertical=['BUTTON','SUMMARY'].includes(node.tagName)||node.matches('.camera-overlay,.segmented')?(node.scrollHeight>node.clientHeight+1)&&style.overflowY==='visible':false; return horizontal||vertical }).slice(0,40).map(node => ({ node:description(node), text:(node.textContent||'').trim().slice(0,90), client:[node.clientWidth,node.clientHeight], scroll:[node.scrollWidth,node.scrollHeight] }))
    const overlap = []
    ...
    const railLabels=[...document.querySelectorAll('.sidebar button strong')].filter(visible).filter(node=>getComputedStyle(node).clipPath==='none').filter(node=>{const style=getComputedStyle(node);return node.scrollWidth>node.clientWidth+1||node.scrollHeight>node.clientHeight+1||style.overflowWrap==='anywhere'||style.wordBreak==='break-word'}).map(node=>({text:node.textContent,width:node.clientWidth,height:node.clientHeight}))
    ...
    return {palette:document.documentElement.dataset.palette,viewport:[innerWidth,innerHeight],document:[document.documentElement.scrollWidth,document.documentElement.scrollHeight],required,contained,railLabels,selectedValueDetails,overflow,overlap:overlap.slice(0,40),verticalText,toolbarState,controls}
  })()`)
  // This is a capacity finding, separate from geometric containment. It cannot
  // certify semantic bindings or imply that every conditional form was visited.
  detail.fieldReadability = await evaluate(cdp, `(() => {
  ...
  detail.shortFields = detail.fieldReadability.filter(/* 比较 field.characters + .1 与 field.required，返回小于的判断结果。 */ field => field.characters + .1 < field.required)
  for (const control of detail.controls ?? []) if (control.testId) stableControlsById.set(control.testId, control)
  const passed = detail.railLabels.length === 0 && detail.shortFields.length === 0 && detail.palette === palette && detail.contained && detail.overflow.length === 0 && detail.overlap.length === 0 && detail.verticalText.length === 0 && (!detail.toolbarState || detail.toolbarState.childOverflows === 0)
