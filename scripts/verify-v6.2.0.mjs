/** 功能回归脚本：执行 verify-v6.2.0.mjs 对应场景，保留断言和证据输出。 */
...
try{
  ...
  const load=/* 调用 import(`${pathToFileURL(join(compiled,`${name}.mjs`)).href}?v=${Date.now()}`) 并返回调用结果。 */ name=>import(`${pathToFileURL(join(compiled,`${name}.mjs`)).href}?v=${Date.now()}`),[contracts,formats]=await Promise.all(['contracts','formats'].map(load))
  ...
  check('V620-FAIL-CLOSED',!missing.valid&&missing.diagnostics.some(/* 比较 item.code 与 'NOVA-CONTRACT-REQ'，返回严格相等的判断结果。 */ item=>item.code==='NOVA-CONTRACT-REQ'),'Missing declared requirements fail closed with a specific diagnostic.')
  const nondeterministic=contracts.parseScriptContract('// @nova deterministic\nfn update(dt) { let x = mouse_x(); }')
  check('V620-DETERMINISM',!nondeterministic.valid&&nondeterministic.diagnostics.some(/* 比较 item.code 与 'NOVA-CONTRACT-007'，返回严格相等的判断结果。 */ item=>item.code==='NOVA-CONTRACT-007'),'Deterministic contracts reject host-dependent API usage.')
  const wrongThread=contracts.parseScriptContract('// @nova strict\nfn update(dt) { apply_force(1.0, 0.0); }')
  check('V620-STRICT-THREAD',!wrongThread.valid&&wrongThread.diagnostics.some(/* 比较 item.code 与 'NOVA-CONTRACT-008'，返回严格相等的判断结果。 */ item=>item.code==='NOVA-CONTRACT-008'),'Strict contracts reject fixed-step mutations from rendered-frame callbacks.')
  ...
  check('V620-DIAGNOSTICS',!malformed.valid&&new Set(malformed.diagnostics.map(/* 返回 item.code 的当前值。 */ item=>item.code)).size>=3,'Malformed flags, requirement kinds and budgets remain separately actionable.')
  const sources=Object.fromEntries(await Promise.all(['src/runtime/GameplayRuntime.ts','src/runtime/productionValidation.ts','src/components/ScriptStudio.vue','src/components/ProjectHealthPanel.vue','src/editor/scriptTemplates.ts','src/i18n.ts','instructions.txt','docs/ROADMAP_6_2_TO_7_0.md','docs/FEATURE_INVENTORY_6_2.md'].map(/* 返回按声明顺序构造的数组 [path,await text(path)]。 */ async path=>[path,await text(path)])))
...
/** 结构说明（自动提取）：forEachLanguage；输入 source、key；直接调用 every。 */ function forEachLanguage(source,key){return ['Object.assign(en','Object.assign(de','Object.assign(zh'].every(/** 结构说明（自动提取）：every 回调；输入 marker；直接调用 source.lastIndexOf、includes、source.slice。 */ marker=>{const at=source.lastIndexOf(marker);return at>=0&&source.slice(at,at+5000).includes(`${key}:`)})}
