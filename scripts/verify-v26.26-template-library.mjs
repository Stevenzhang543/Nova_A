/** 功能回归脚本：执行 verify-v26.26-template-library.mjs 对应场景，保留断言和证据输出。 */
...
const captureRoot = resolve(process.env.NOVA_TEMPLATE_PREVIEWS || join(root,'release-audits/v26.26-template-previews'))
...
await withBrowserAudit({release:'26.26',name:'template-library',root,development:process.env.NOVA_AUDIT_DEVELOPMENT==='1',expectedRelease:process.env.NOVA_AUDIT_EXPECTED_RELEASE || '26.26',width:1600,height:1000}, /** 结构说明（自动提取）：withBrowserAudit 回调；输入 audit；直接调用 client.on、client.send、click、until、evaluate 等；包含循环处理；等待异步结果。 */ async audit => {
