/** 功能回归脚本：执行 verify-v26.23-template-library.mjs 对应场景，保留断言和证据输出。 */
// Retained forty-template runtime audit, updated for explicit New and focus-contained creation.
...
const captureRoot = resolve(process.env.NOVA_TEMPLATE_PREVIEWS || join(root,'release-audits/v26.23-template-previews'))
...
await withBrowserAudit({release:'26.23',name:'template-library',root,development:process.env.NOVA_AUDIT_DEVELOPMENT==='1',expectedRelease:process.env.NOVA_AUDIT_EXPECTED_RELEASE || '26.23',width:1600,height:1000}, /** 结构说明（自动提取）：withBrowserAudit 回调；输入 audit；直接调用 client.on、client.send、click、until、evaluate 等；包含循环处理；等待异步结果。 */ async audit => {
  ...
  await click('.quick-actions .new-project')
  ...
  await check('Localized requirements, controls/results, search reset, preview and real help navigation',/** 结构说明（自动提取）：check 回调；无显式参数；直接调用 evaluate、click、until、audit.select、assert.match 等；包含循环处理；等待异步结果。 */ async()=>{
    for(const locale of ['en','de','zh']){
      if(await evaluate("!!document.querySelector('.creation-card')")){await click('.creation-title button');await until("!document.querySelector('.creation-card')")}
      await audit.select('.manager-header select',locale);await click('.quick-actions .new-project');await until("!!document.querySelector('.creation-card')")
    ...
    await click('.creation-title button');await until("!document.querySelector('.creation-card')");await audit.select('.manager-header select','en');await click('.quick-actions .new-project');await until("!!document.querySelector('.creation-card')");await click('.template-results button');await audit.capture('library-discovery')
  })
  ...
  for (const id of ids.filter(/* 先计算 !only；仅当其为假值时求右侧 only.includes(id)，返回短路求值结果。 */ id=>!only||only.includes(id))) {
    await check(`${id}: select visible launcher card, create, Play, rendered output, Console, Stop`,/** 结构说明（自动提取）：check 回调；无显式参数；直接调用 until、evaluate、click、fill、assert.equal 等；等待异步结果。 */ async()=>{
      await until("!!document.querySelector('.project-manager')")
      if(!await evaluate("!!document.querySelector('.creation-card')")){await click('.quick-actions .new-project');await until("!!document.querySelector('.creation-card')")}
