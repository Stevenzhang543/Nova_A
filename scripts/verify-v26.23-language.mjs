/** 功能回归脚本：执行 verify-v26.23-language.mjs 对应场景，保留断言和证据输出。 */
...
const results=[];
for(const[name,source,expected]of cases){const vm=new wasm.WasmScriptRuntime();try{const execution=JSON.parse(vm.execute_json(source,'start',JSON.stringify({entity:'semantics23'})));const actual=execution.logs.map(/* 返回 x.message 的当前值。 */ x=>x.message);const native=spawnSync(resolve('target/debug/examples/runtime_bridge.exe'),{input:JSON.stringify({source,function:'start',context:{entity:'semantics23'}}),encoding:'utf8',windowsHide:true,timeout:15000});assert.equal(native.status,0);const output=JSON.parse(native.stdout);if(output.error)throw Error(output.error);const nativeLogs=output.value.logs.map(/* 返回 x.message 的当前值。 */ x=>x.message);assert.deepEqual(actual,expected);assert.deepEqual(nativeLogs,actual);assert.deepEqual(output.value.properties,execution.properties);results.push({name,status:'passed',logs:actual})}catch(error){results.push({name,status:'failed',error:String(error)})}finally{vm.free()}}
