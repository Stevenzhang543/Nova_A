...
const screenshotRoot = join(evidenceRoot, 'screenshots', 'v3.2.0')
...
if (!edgePath) throw new Error('Microsoft Edge is required for the v3.2 editor qualification run.')
await mkdir(screenshotRoot, { recursive: true })

const previewServer = await preview({ root, logLevel: 'silent', preview: { host: '127.0.0.1', port: 4173, strictPort: true } })
const debugPort = await freePort()
const profile = await mkdtemp(join(tmpdir(), 'nova-a-edge-v32-'))
...
try {
  ...
  await key(client, 'F11', 'F11')
  const browserFullscreenEntered = await waitForExpression(client, 'Boolean(document.fullscreenElement)', 5_000, false)
  await key(client, 'F11', 'F11')
  const browserFullscreenExited = await waitForExpression(client, '!document.fullscreenElement', 5_000, false)
  results.push({ name: 'F11 fullscreen shortcut', status: browserFullscreenEntered && browserFullscreenExited ? 'passed' : 'failed', detail: 'The global F11 route entered and exited the browser fullscreen fallback.' })
