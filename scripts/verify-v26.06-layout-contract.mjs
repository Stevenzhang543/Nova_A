/** 验证脚本（v26.06-layout-contract）：组织对应功能与边界场景检查，断言行为并汇总验证结果。 */
...
/** 递归收集指定目录下所有非目录条目的路径。 */ async function filesUnder(directory) { const result = []; for (const entry of await readdir(directory, { withFileTypes: true })) { const path = join(directory, entry.name); entry.isDirectory() ? result.push(...await filesUnder(path)) : result.push(path) } return result }
const files = (await filesUnder(join(root, 'src'))).filter(/* 调用 path.endsWith('.vue') 并返回调用结果。 */ path => path.endsWith('.vue')).sort()
const surfaces = await Promise.all(files.map(/** 读取源码并将相对项目路径中的分隔符统一为正斜杠。 */ async path => ({ path: relative(root, path).replaceAll('\\', '/'), source: await readFile(path, 'utf8') })))
...
check('V2606-LAYOUT-SURFACE-INVENTORY', files.length >= 65 && surfaces.every(/* 先计算 item.source.includes('<template')；仅当其为真值时求右侧 item.source.includes('<script')，返回短路求值结果。 */ item => item.source.includes('<template') && item.source.includes('<script')), 'Every Vue surface is enumerated and structurally authored.', { vueSurfaces: files.length })
