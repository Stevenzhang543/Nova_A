/** 26.26 保留回归：复用 26.23 已审计的 template-output 检查，在当前版本执行并单独记录证据。 */
import assert from 'node:assert/strict'
// Retained 26.20 regression implementation, executed against actual 26.26 source.
...
const release = '26.26', development = process.argv.includes('--development')
if (!development) assert.equal(engineVersion, '26.26.0', 'Template output qualification requires actual 26.26 source')
...
const report = {
  format: 'nova-v26.26-template-output-verification',
  version: 1,
  release,
  engineVersion,
  development,
...
await writeFile(join(root, 'release-audits', 'v26.26-template-output.json'), `${JSON.stringify(report, null, 2)}\n`)
if (failed.length) { console.error(failed); process.exit(1) }
console.log(`Nova_A 26.26 template output verification passed: ${templateResults.length} templates, ${checks.length} checks.`)
