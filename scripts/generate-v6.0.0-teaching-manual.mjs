...
function htmlFor(locale, guides, localizedLearningGuide) {
  ...
  return `<article data-lang="${locale}"${locale === 'en' ? '' : ' hidden'} class="v6-teaching"><section id="${locale}-v60"><div class="hero"><span class="eyebrow">Nova_A 6.0.0 · Project Format 2/schema 29 · external certification honestly pending</span><h1>${escapeHtml(l.heading)}</h1><p>${escapeHtml(l.intro)}</p><div class="links">${toc}</div></div>${panels}</section></article>`
}
...
try {
  ...
  let html = original.replaceAll('Nova_A 5.9 Manual', 'Nova_A 6.0 Manual').replaceAll('5.9.0 Offline Documentation', '6.0.0 Offline Teaching Manual').replaceAll('Engine 5.9.0', 'Engine 6.0.0')
