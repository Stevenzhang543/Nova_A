/** 功能回归脚本：执行 verify-v4.9.mjs 对应场景，保留断言和证据输出。 */
...
Object.defineProperty(globalThis, 'navigator', { configurable: true, value: { platform: 'Win32', hardwareConcurrency: 8, userAgent: 'Nova_A v4.9 verification', mediaDevices: { /** 提供不注册监听器的测试事件接口。 */ addEventListener(){}, /** 提供无需移除监听器的测试事件接口。 */ removeEventListener(){}, /* 返回按声明顺序构造的数组 []。 */ async enumerateDevices(){ return [] } } } })
...
try {
  console.log('v4.9 verification: loading platform/build contracts')
  ...
  console.log('v4.9 verification: loading package/team contracts')
  ...
  const input = { engineVersion: '4.9.0', projectId: 'fixture', target: 'web', architecture: 'x86_64', profile: 'release', releaseChannel: 'stable', settings: webSettings, packages: [], deterministic: true, files: [{ path: 'player.js', sha256: 'a'.repeat(64), bytes: 12 }] }
  ...
  const update = packages.normalizePackageManifest({ ...official, version: '4.9.1', sha256: 'e'.repeat(64), signature: `nova-official-v1:${'e'.repeat(64)}`, permissions: [...official.permissions, 'filesystem.write'] })
  ...
  const cycleA = packages.normalizePackageManifest({ ...official, id: 'com.example.cycle-a', name: 'Cycle A', version: '4.9.0', sha256: 'a'.repeat(64), signature: `nova-official-v1:${'a'.repeat(64)}`, dependencies: { 'com.example.cycle-b': '^4.9.0' }, dependencyHashes: { 'com.example.cycle-b': 'b'.repeat(64) } })
  const cycleB = packages.normalizePackageManifest({ ...official, id: 'com.example.cycle-b', name: 'Cycle B', version: '4.9.0', sha256: 'b'.repeat(64), signature: `nova-official-v1:${'b'.repeat(64)}`, dependencies: { 'com.example.cycle-a': '^4.9.0' }, dependencyHashes: { 'com.example.cycle-a': 'a'.repeat(64) } })
  ...
  try { await novaPak.createNovaPak(JSON.stringify({ engineVersion: '4.9.0', assets: [maliciousAsset], scenes: [], projectSettings: { build: { developmentBuild: false } } }), [maliciousAsset], 'scene') } catch (error) { maliciousArchiveRejected = error instanceof Error && error.message.includes('Unsafe package path') }
  ...
  console.log('v4.9 verification: closing isolated module host')
...
const report = { format: 'nova-v4.9-runtime-verification', version: 1, engineVersion: '4.9.0', generatedAt: new Date().toISOString(), catalogs: ['BLD','PKG','HLT'], checks, severity0Open: 0, severity1Open: failed.length, status: failed.length ? 'failed' : 'passed' }
await writeFile(join(output, 'v4.9.0-verification.json'), `${JSON.stringify(report, null, 2)}\n`)
if (failed.length) { console.error(failed); process.exit(1) }
console.log(`Nova_A v4.9 runtime verification passed: ${checks.length} checks.`)
process.exit(0)
