/* 审计 6.7.0 的输入运行时、Android 安全与资格边界、可访问性及平台证据。 */
...
check('V670-WINDOWS', windows.status === 'passed' && windows.artifacts?.length === 4 && windows.artifacts.every(/* 先计算 item.bytes > 0；仅当其为真值时求右侧 item.sha256?.length === 64，返回短路求值结果。 */ item => item.bytes > 0 && item.sha256?.length === 64), 'Windows editor and exported no-code touch game launch smoke passes.', { artifacts: windows.artifacts?.length ?? 0 })
