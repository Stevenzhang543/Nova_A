/** 功能回归脚本：执行 verify-v26.22-material-graph-user.mjs 对应场景，保留断言和证据输出。 */
...
await withBrowserAudit({release:'26.22',name:'material-graph-user',development:process.argv.includes('--development'),expectedRelease:JSON.parse(await readFile('package.json','utf8')).version.split('.').slice(0,2).join('.'),width:1366,height:900},/** 结构说明（自动提取）：withBrowserAudit 回调；输入 a；直接调用 worldUserControls17、u.open、graph、a.check；等待异步结果。 */ async a=>{
 ...
 await a.check('Unsaved material graph edits survive asset switches and panel replacement',/** 结构说明（自动提取）：a.check 回调；无显式参数；直接调用 u.field、a.select、a.clickText、assert.equal、a.evaluate 等；等待异步结果。 */ async()=>{
 ...
 await a.check('Typed scalar and vector uniforms retain rejected input and save expressions without removing sliders',/** 结构说明（自动提取）：a.check 回调；无显式参数；直接调用 a.click、a.until、u.field、assert.equal、a.evaluate 等；等待异步结果。 */ async()=>{
  ...
  await blockedStudioSave22(a);const read=/* 返回 JSON.parse(decodeURIComponent(doc.assets.find(x=>x.uuid===ids[0]).source.split(',').slice(1).join(','))).uniforms 的当前值。 */ doc=>JSON.parse(decodeURIComponent(doc.assets.find(/* 比较 x.uuid 与 ids[0]，返回严格相等的判断结果。 */ x=>x.uuid===ids[0]).source.split(',').slice(1).join(','))).uniforms;
 ...
 await a.check('Malformed advanced JSON survives preview edits and corrected JSON saves intact',/** 结构说明（自动提取）：a.check 回调；无显式参数；直接调用 a.click、u.field、assert.equal、a.evaluate、wait 等；等待异步结果。 */ async()=>{
  ...
  const material=JSON.parse(decodeURIComponent(saved.document.assets.find(/* 比较 x.uuid 与 ids[0]，返回严格相等的判断结果。 */ x=>x.uuid===ids[0]).source.split(',').slice(1).join(',')));
  assert.equal(material.uniforms.auditScalar,7);assert.ok(material.fragment.includes('// preserve advanced draft'));
 })
 await a.check('Typed material numbers stay readable and reachable across three languages, scales and viewport widths',/** 结构说明（自动提取）：a.check 回调；无显式参数；直接调用 a.viewport、u.workspace、a.click、a.until、a.select 等；包含循环处理；等待异步结果。 */ async()=>{
  const failures=[];
  for(const locale of ['en','de','zh'])for(const scale of [1,1.5,2]){
   ...
   for(const width of [1024,1366,1920]){
    ...
    a.observations.push({name:'material-uniform-reachability',locale,scale,width,...reachability});assert.ok(reachability.paneHeight>=80,JSON.stringify({locale,scale,width,...reachability}));assert.ok(reachability.results.every(/* 返回 r.reachable 的当前值。 */ r=>r.reachable),JSON.stringify({locale,scale,width,...reachability}));
