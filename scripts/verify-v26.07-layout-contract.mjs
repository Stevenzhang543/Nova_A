/** 验证脚本（v26.07-layout-contract）：组织对应功能与边界场景检查，断言行为并汇总验证结果。 */
...
/** 递归遍历目录并返回所有非目录条目路径。 */ async function filesUnder(directory) { const files = []; for (const entry of await readdir(directory, { withFileTypes: true })) { const path = join(directory, entry.name); entry.isDirectory() ? files.push(...await filesUnder(path)) : files.push(path) } return files }
const vueFiles = (await filesUnder(join(root, 'src'))).filter(/* 调用 path.endsWith('.vue') 并返回调用结果。 */ path => path.endsWith('.vue')).sort()
const vue = await Promise.all(vueFiles.map(/** 读取源码并将相对项目路径中的分隔符统一为正斜杠。 */ async path => ({ path: relative(root, path).replaceAll('\\', '/'), source: await readFile(path, 'utf8') })))
const [css, panel, dock, i18n, contract] = await Promise.all(['src/assets/main.css','src/components/NetworkStudioPanel.vue','src/components/EditorBottomPanel.vue','src/i18n.ts','docs/UI_LAYOUT_AUDIT_26_07.md'].map(/* 调用 readFile(join(root, path), 'utf8') 并返回调用结果。 */ path => readFile(join(root, path), 'utf8')))
check('V2607-LAYOUT-ALL-SURFACES', vue.length >= 65 && vue.every(/* 先计算 item.source.includes('<template')；仅当其为真值时求右侧 item.source.includes('<script')，返回短路求值结果。 */ item => item.source.includes('<template') && item.source.includes('<script')), 'The layout contract enumerates every Vue surface rather than only Network Studio.', { vueFiles: vue.length })
...
check('V2607-LAYOUT-LAUNCH-REASON', panel.includes('id="multi-instance-prerequisite"') && panel.includes(':aria-describedby="!canLaunchInstances') && launchReasonKeys.every(/* 比较 (i18n.match(new RegExp(`${key}:`, 'g')) ?? []).length 与 3，返回严格相等的判断结果。 */ key => (i18n.match(new RegExp(`${key}:`, 'g')) ?? []).length === 3), 'The disabled native multi-instance action exposes one contained, programmatically associated and fully localized corrective reason.')
...
check('V2607-LAYOUT-LOCALES', ['networkStudio:', "networkStudio:'Netzwerkstudio'", "networkStudio:'网络工作室'"].every(/* 调用 i18n.includes(token) 并返回调用结果。 */ token => i18n.includes(token)), 'The primary Network Studio label exists in English, German and Chinese.')
check('V2607-LAYOUT-MATRIX', ['1024×640','1024×768','1366×768','1920×1080','2560×1440','80%', '100%', '125%', '150%', 'English, German, and Chinese'].every(/* 调用 contract.includes(token) 并返回调用结果。 */ token => contract.includes(token)), 'The documented browser matrix covers every required viewport, UI scale and locale.')
